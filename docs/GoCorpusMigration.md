...
#### Amendment 2026-09-13 — the `-stdlib` tag default, the `DELETE-DESELECTED` bar, and the two solutions

Four procedure lines from the first reconvert-and-build run at the target release. They amend this
step; they do not revise the bullets above. The day's other H5 amendment is the seed-list sub-bullet
above (`src/gen` and `src/Directory.Build.props`), and these read with it.

**1. The reconvert's tag set is the converter's `-stdlib` DEFAULT — never pass `-tags` to the H5
reconvert.** `resolveBuildTags` (`src/go2cs/commandLineOptions.go`) applies `purego` and
`math_big_pure_go` **only when `-tags` was not passed**, and an explicit `-tags` — `-tags=` included —
overrides it verbatim; the step's record therefore quotes the converter's own printed tag line, not the
command line that invoked it. *Derivation:* the first step-2 run tonight emitted WITH the default (the printed line
`Applying build tags: purego,math_big_pure_go (default; ...)`), yet the deletion pass classified five
purego and generic variants as `DELETE-DESELECTED` and removed them — the pass carried a SECOND tag
resolution of its own that did not know the converter's default. Two components must not resolve
tags twice: the converter is the authority on what it selected, and the deletion pass derives
"deselected" from the staging root it wrote (present in the corpus, principal present at the target,
absent from the emission), never from tag logic of its own. The asm-backed twins such a mis-selection
would leave in place convert to throwing stubs that COMPILE.

**2. Every `DELETE-DESELECTED` row H5c reports is explained by a SELECTION change between the two
`GOROOT`s under the converter's own tag set, or the run is REFUSED before `-Apply`.** The explanation
gate evaluates the principal's selection at the source SDK tree and at the target SDK tree with ONE tag
set — the one the converter PRINTED for this run (`go list -tags <that set>` at each tree) — and a row
whose principal is selected at BOTH is a live file the pass must not delete; a row selected at the source
and not at the target is a genuine deselection, whatever its constraint text did. ⚠ Constraint TEXT is
not selection: `grep '^//go:build'` at the two trees compares one of the inputs (Go also moves the arch
list and the default `GOEXPERIMENT` set), and on the first population measured it read four of six rows
wrong — three live `|| purego` files whose negated arch list grew, and the one genuine deletion
(`internal/goexperiment/exp_aliastypeparams_off.go`) whose text did not change because the flip is in the
experiment defaults. *Derivation:* the same event as item 1 — the deletion pass called `go list` with no
`-tags` and an emptied `GOFLAGS`, so it re-resolved selection without the default and deleted five live
files; the corrected pass takes the select/deselect decision from the EMISSION and runs this gate only as
the explanation of a candidate deletion, with the tag set read from the converter's printed line rather
than mirrored as a constant.

**3. The generated stdlib solution is REGENERATED by the reconvert, and the regenerated file is
CARRIED into the corpus commit.** `-stdlib` writes `go2cs-stdlib.slnx` at the output root over the
packages it actually converted (`src/go2cs/solutionGenerator.go`; the committed `src/go2cs-stdlib.slnx`
is adopted from that output VERBATIM), while the overlay above moves `.cs`, `.csproj` and `README.md`
only — so the regenerated solution has to be carried from the staging root by hand or the hop ships the
outgoing release's project set. An H5c slnx arm is a GUARD on the generated file, never the mechanism
that produces it. *Derivation:* the build wall after the overlay — MSB3202, on project files the target
release no longer has — was the committed stale solution, and it cleared once the regenerated one was
carried.

**4. The hand-maintained converter-dev solution is NOT regenerated, so at H5 it is edited by hand.**
`src/go2cs.slnx` has no generator: at the hop its `Project` entries follow moved packages to their
successors and drop packages the release emptied, in the checkpoint commit. A text guard asserting that
every `Project Path` in it resolves on disk is a converter-suite seat. *Derivation:* two entries went
stale AT the hop (`runtime/internal/{math,sys}` → `internal/runtime/{math,sys}`) and a third when a
package's last file was deleted, and the solution then failed to LOAD — which is why nothing caught
them, a solution that cannot be opened runs no gate.
