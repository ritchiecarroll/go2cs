...
**2. Every `DELETE-DESELECTED` row H5c reports is explained by a build-tag change between the two
`GOROOT`s, or the run is REFUSED before `-Apply`.** Read the two principals' constraints directly —
`grep '^//go:build'` at the source SDK tree and at the target SDK tree — and record the change beside
the row; the class means *Go stopped selecting this file at the target*, so a row whose principal
carries the same constraints at both releases is the pass reporting the CONVERSION's tags rather than
the release's, and deleting it deletes a live file. *Derivation:* the same event — every deselection in
that run was a tag-set artifact and the class named nothing about the target release.
