# NuGet 1.24.13.1 release announcement: DRAFT (repaired)
...
- **Piece 2**: `docs/README.md`, the featured NEWS block (replaced whole; guarded by
  `src/check-roster-format.ps1` §2e).
...
outside the standard library, and each stays restorable at 1.23.12.3 and is never unlisted. This is
also the first release under the project's current licensing: the `go.<pkg>` packages ship a
BSD-3-Clause license file carrying Go's notice, where earlier releases declared MIT; `go.lib` and
`go.gen` stay MIT; and the converter itself is AGPL-3.0-only, with an exception that leaves the code it
generates outside that license
([LICENSING.md](https://github.com/ritchiecarroll/go2cs/blob/master/LICENSING.md)).
...
about 570 words against the archive's 228-458 for release entries; it is the shortest text that keeps
the upgrade note, the licensing change and the FIPS disclaimer in the archive itself. See OQ 2.)
...
This is the first release under the project's current licensing. The `go.<pkg>` packages ship a
BSD-3-Clause license file carrying Go's notice, where earlier releases declared MIT; `go.lib` and
`go.gen` stay MIT; and the converter itself (`src/go2cs`) is AGPL-3.0-only, with an additional
permission that leaves the code it generates outside that license.
[LICENSING.md](https://github.com/ritchiecarroll/go2cs/blob/master/LICENSING.md) has the full matrix.
...
**218 of the 230 testable standard-library packages pass their own Go 1.24.13 test suites in C#** —
56,974 matching verdicts against `go test -json`, compared verdict for verdict, with 283 divergences
disclosed by exact failure signature and nothing else waived. Six of those 230 cannot be validated
at all — no eligible tests on this platform, a suite whose whole subject is the raw memory layout a
managed runtime deliberately does not have, or a comparison that runs cleanly and validates nothing —
so the honest denominator is **224, putting the roster at 97.3%**. Each of the six is listed with its
class, mechanism and evidence in the [exclusion ledger](ValidatedTestPackages.md#excluded-packages),
and any one of them rejoins the count the day its evidence changes. On Linux, 187 of the 216
applicable rows validate at their own Linux counts. A package appears on the
...
This release re-derives every row from Go 1.24.13's own test sources. The validated count rises from
the Go 1.23.12 record's 204 while the testable set grows by fifteen, so the share of the implementable
set dips from 97.6%. Ten packages Go 1.24 moved to new import paths validate under the packages that
now hold their tests, and `unique` and Go 1.24's new `crypto/hkdf`, `crypto/mlkem`, `crypto/pbkdf2`,
`crypto/sha3` and `weak` join. One validated package leaves: `net/http`, whose 17 divergences all
trace to tests run under Go 1.24's new `internal/synctest`, which go2cs does not yet support. The
release publishes as **NuGet 1.24.13.1**, with its proof pages frozen at `validation/1.24.13.1` and the
exact shipped tree at the `nuget-1.24.13.1` tag — see
[the announcement](NEWS.md#september-24-2026--the-converted-standard-library-moves-to-go-12413-and-218-packages-validate-against-it).

**➡ All announcements can be found in the [go2cs News Archive](NEWS.md).**
````

Guard §2e, read against this block. The block is joined and its whitespace collapsed, and the first
match of each pattern wins:

| Pattern | First match in this block | Roster header @fa18863b94 |
|:--|:--|:--|
| `(\d+) of the (\d+) testable` | `218 of the 230 testable` | 218 / 230 |
| `([\d,]+) matching verdicts` | `56,974 matching verdicts` (across the wrap) | 56,974 |
| `([\d,]+) divergences disclosed` | `283 divergences disclosed` (across the wrap) | 283 |
| `denominator is \*{0,2}(\d+)` | `denominator is **224` | 224 |
| `roster at \*{0,2}([\d.]+)%` | `roster at 97.3%` | 97.3 |
| `(\d+) of the (\d+) applicable rows` | `187 of the 216 applicable rows` (across the wrap) | 187 / 216 |
...
hand in `6b472ad6fe`). Paragraph 2's `204`, `97.6%` and "grows by fifteen" match no pattern, and it
states only this release's own record, so a later bank cannot falsify it. None of the 1.23.12 anchor
figures is written in a guarded phrasing. The heading keeps `NEWS`, and the archive line stays last.
The block measures about 300 words, against 223-282 for the 1.23.12.1/.2/.3 blocks.
...
|:--|:--|:--|:--|
| 1 | go2cs converts Go 1.24.13's STANDARD LIBRARY (P1, P1b). The lead no longer claims Go 1.24 as a whole: no Behavioral `go.mod` declares `go 1.24` (670 `go 1.23`, 18 `go 1.23.1`, 3 `go 1.22`, 10 `go 1.21`), and no generic type alias is exercised | `src/version.props:23` GoStdLibVersion 1.24.13 @fa18863b94; `src/tests/Behavioral/*/go.mod` @fa18863b94 | V |
| 2 | 218 of the 230 testable packages validate (P1, P1b, P2, P3) | `docs/ValidatedTestPackages.md:135` @fa18863b94 | V, TIP |
| 3 | 56,974 matching verdicts; 283 disclosed (P1, P1b, P2, P3) | roster :137 @fa18863b94 | V, TIP |
| 4 | Implementable 224 (230 − 6); 97.3% (P1, P1b, P2, P3) | roster :154 @fa18863b94 | V, TIP |
| 5 | Linux 187 of 216 applicable rows (P1, P1b, P2); 53,048 matching (P1, P1b) | roster :167 @`836004dd20` (G's crypto/tls Linux rebank, ACCEPTED LEDGER 2026-09-24 00:31, joins at CP2): 187 / 216, 53,048, 162 disclosed. At fa18863b94: 49,629 / 163. NOT guarded in NEWS.md or the companion page | V (at 836004dd20), TIP |
| 6 | Every validated row is proved by a run at Go 1.24.13 (P1, P1b); every row re-derived from 1.24.13's test sources (P2, P3) | roster :901-903 @fa18863b94 ("every other banked row's first proof page reads Go 1.24.13"); LEDGER 2026-09-23 22:30 ("every first proof page reads 1.24.13"). Ten successor rows link a 1.23.12 anchor page SECOND, as provenance. The RN-14 full-roster sweep is owed before the cutover | V (text), TIP |
| 7 | 1.23.12 record: 204; 97.6% implementable; 94.9% testable (P1, P1b, P2) | roster :135, :141 @nuget-1.23.12.3; NEWS.md Sept 7 entry | V |
| 8 | The count rises by 14; the testable set grows by 15 (215 → 230) (P1, P1b, P2) | arithmetic on claims 2 and 7 | V |
| 9 | 94.9% → 94.8% and 97.6% → 97.3% (P1, P1b, P2) | 204/215 = 94.88; 218/230 = 94.78; 204/209 = 97.61; 218/224 = 97.32 | V |
| 10 | Go 1.24 moved ten validated packages; none is a loss; their tests validate UNDER THE PACKAGES THAT NOW HOLD THEM (P1, P1b, P2). Not "at their new paths": `crypto/internal/alias`'s one test routes to `crypto/internal/fips140test` (`fips140/alias` has only `alias.go` and no row), `crypto/internal/nistec`'s 2,200 verdicts route to `fips140test`, and two declarations are retired upstream | roster :449 ("None of them is a loss"), :515, :534, :539, :614-619 @fa18863b94; go1.24.13 `src/crypto/internal/fips140/alias/` | V |
...
| 12 | crypto/hkdf, crypto/mlkem, crypto/pbkdf2, crypto/sha3 and weak are validated rows, and all five are new Go 1.24 public packages (P1, P1b, P2) | roster :246, :263, :264, :270, :440 @fa18863b94; CENSUS-go124-package-delta.md | V |
| 13 | unique validates; one of the five the 1.23.12 record left open (P1, P1b, P2) | roster :439 @fa18863b94; NEWS.md Sept 7 entry | V |
| 14 | net/http matches 1,370 of 1,387; its 17 divergences are 11 `synctest.Run` leaves + their 6 aggregation parents (P1, P1b, P2, P3) | roster :931 @fa18863b94; LEDGER 2026-09-23 22:30. The 2026-09-22 17:33 ruling line says "7 parents"; the close's final 11 + 6 = 17 is used | V |
| 15 | The 17 trace to tests run under Go 1.24's new `internal/synctest`, the runtime support behind the experimental `testing/synctest` (P1, P1b, P2, P3). NOT `testing/synctest` itself | go1.24.13 `src/net/http/{async,clientserver,netconn,serve,transport}_test.go` import `"internal/synctest"`, none imports `testing/synctest`; `src/testing/synctest/synctest.go` is `//go:build goexperiment.synctest`; CENSUS §5 @fa18863b94 (the experiment gates the public surface only) | V |
| 16 | "which go2cs does not yet support" (P1, P1b, P2) | the synctest arc S1a..S4 is complete but unmerged (LEDGER 2026-09-23 11:33, `7984c46149`, not an ancestor of fa18863b94). The forward note ("lands after this release") is kept OUT of public text; see OQ 17 | V |
| 17 | net/http still ships as `go.net.http` (P1, P1b) | `src/go2cs-stdlib.slnx` @fa18863b94 lists net.http.csproj; `<AssemblyName>net.http`; not among the removed 14 | V |
...
| 25 | The converted stdlib carries os.Root, weak.Pointer, crypto/mlkem and strings.Lines "among others" (P1, P1b). "And the rest" was dropped: the hand-owned `testing` host's Go 1.24 `B.Chdir`/`F.Chdir` are empty compile-only stubs | src/core/os/root.cs:77; src/core/weak/pointer.cs; src/core/crypto/mlkem; src/core/strings/iter.cs:18; src/core/testing/testing.cs:471, :589 @fa18863b94 | V |
...
| 33 | First release under the current licensing: go.<pkg> ship a BSD-3-Clause license file carrying Go's notice (plus a go2cs Authors line) where earlier releases declared MIT; go.lib and go.gen stay MIT; the converter is AGPL-3.0-only with an output exception (P1, P1b) | bytes.csproj `PackageLicenseExpression MIT` @nuget-1.23.12.3 vs `PackageLicenseFile LICENSE` @fa18863b94; src/core/LICENSE opens "Copyright 2009 The Go Authors. / Copyright © 2026 The go2cs Authors."; golib.csproj and go2cs-gen.csproj MIT; LICENSING.md matrix @fa18863b94 ("AGPL-3.0-only, with the additional permission for emitted output"); nuget-1.23.12.3:LICENSE begins "MIT License"; 1800b04f86 is an ancestor of fa18863b94 and master, NOT of nuget-1.23.12.3; no NEWS entry names the AGPL change | V |
...
| 43 | Still targeting .NET 10 (P1, P1b) | src/Directory.Build.props:27 `net10.0` @fa18863b94 | V |
| 44 | Every proof page frozen at validation/1.24.13.1 (P1, P1b, P2) | CP0 RN-6 (LEDGER 2026-09-23 23:41); the census seat 869d8e4683 (Froze 232) | FWD |
| 45 | The exact shipped tree at the nuget-1.24.13.1 tag (P1, P1b, P2, P3) | ritual element 2 | FWD |
| 46 | The Go 1.23.12 record stays frozen at validation/1.23.12.3 (P1, P1b) | docs/validation/1.23.12.3/ @master: 205 files | V |
| 47 | Six exclusions: no eligible tests on this platform / the raw memory layout / a comparison that validates nothing (P2) | roster exclusion ledger @fa18863b94: 1 E1 (runtime/internal/wasitest), 1 E3 (internal/unsafeheader), 4 E4 | V |
| 48 | The exclusion ledger link `#excluded-packages` resolves (P2) | roster heading "## Excluded packages" @fa18863b94 | V |
| 49 | No snapshot LINK: `validation/1.24.13.1` is named in backticks only (P1, P1b, P2) | a relative link resolves against the visitor's tree and 404s until the snapshot commit, and forever at the pre-pack tag tree; the 1.23.12.2 block's `validation/1.23.12.2/index.md` link never resolved | V |
...
2. **Length and the companion page.** Piece 1 is now condensed, with the full text on a companion page
   (Piece 1b), the July 18 / July 26 precedent. The condensed body is about 570 words, still above the
   archive's longest release entry (Sept 7, 458), because it keeps the upgrade note, the licensing
   change and the FIPS disclaimer in the archive itself. Brief C5 names only `NEWS.md` and the README
...
5. **The licensing sentence.** It now names the converter's move to AGPL-3.0-only with the output
   exception, which no NEWS entry has announced (the README's License section already states it).
   Confirm the wording: "the converter itself is AGPL-3.0-only, with an exception that leaves the code
   it generates outside that license".
...
11. **Where the commit lands (RN-12/13).** Under the master-first ruling, the commit lands on the
    ...
    lacks master's public-handle admit `dd18e5e2ab`. The only owner handle in the text sits inside
    github.com URLs, which the admit covers on master.
...
16. **The front-page block drops the ended IDs.** Piece 2 no longer mentions the fourteen ended IDs,
    for length (the block is about 300 words against 223-282). Pieces 1 and 1b carry them. Keep it off
    the front page?
