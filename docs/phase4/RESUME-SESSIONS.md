...
STATE AT THIS REVISION (2026-09-14 18:45 -- new usage week; the save-state refresh runs after every landing/ruling and at every wake tick):
  BRANCH: claude/coord-handover 96b35639ba0920a5ca0555bc5736f3875a6417f8 yes landed -- the handover log (blocks 1-15) + this file
  BRANCH: claude/mailbox f6c60275e15cb040af39dac0011088361b6eb492 yes transport -- rotated 2026-09-13 02:36; COORD's read anchor is the tool's own
...
```
  ...
  BLOCKED-ON: none for windows-amd64 rows (ruling f6c60275: the pair identity is the CONTENT-NORMALIZED per-file hash; ARM 3 b0b825af proved normalized equality of halfA against f0f8826894 for everything the windows target writes) -- linux-amd64 / darwin-amd64 rows wait on the same normalized join against the per-GOOS committed folders; the raw-byte question waits on i9's ARM 2 manifests; row 20 LAST after C1's commit
