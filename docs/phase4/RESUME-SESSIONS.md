...
STATE AT THIS REVISION (2026-09-14 18:45 -- new usage week; the save-state refresh runs after every landing/ruling and at every wake tick):
  BRANCH: claude/coord-handover d0cafb397cad671d5c45cf86c5e748717dc2ca4a yes landed -- the handover log (blocks 1-15) + this file
  BRANCH: claude/mailbox a79e08a3b24d17b2d1be48787cca2d7f53b9b2de yes transport -- rotated 2026-09-13 02:36; COORD's read anchor is the tool's own
...
```
  ...
  NEXT: APPLY IN ORDER, then ONE rebuild (COORD a61895f7e0): (1) fast-forward claude/version-go1.24.13 onto claude/c1-h6-rows 5ace121f0 (row 46); (2) MERGE claude/g-red1-gotype-rooted 5be78d8473; announce-then-push each; prediction BEFORE the rebuild (godebug 0; go/types 2 x CS0411; hmac CS0311; skipped dependents counted; the fips140 eight produced); rebuild; the reading by project and code; row 46's acceptance = os/user's own build on the Windows arm + its tests at the gate tree, posted as its own line. Then RED 2's seat the same way; then RED 3's when assigned. 90-min cadence.
...
```
  ...
  NEXT: RED 3 SIZING (read-only, COORD a61895f7e0) and the EOL seat's THIRD commit (COORD's ruling after ee6ea5bb28: the three sibling annotation sites through normalizeNewlines at the annotation boundary, not getPrintedNode; a spanning printed-node fixture per site asserting the annotation is emitted first; each made to fail by its own regression; push-then-announce; CR-stripped 324/324 re-measured at the third tip) -- either order, stated. The RED 3 sizing: mechanism at the version tip (New<ж<sha256.Digest>> vs where H : fips140.Hash; why the nistec self-referential constraint proxy does not reach it); census over the 1.24.13 std + tests + behavioral of every generic call whose type argument is a pointer type against a method-set constraint satisfied only through [GoRecv] extensions, hmac cast.go:26 the control; candidate rules (a) proxy generalized, (b) generator-emitted nominal adapter, (c) other, each with footprint by class and run-time cost; NO CUT. TestSafePushSelfTest on a shallow clone = a train-49 guard nit, banked. 90-min cadence.
...
```
  ...
  NEXT: RED 2 as predicted at 4b4abe6a85 (typed nil for an inferred generic type-parameter slot: (T)(default!); census 1.24 std INFERRED 2 = infer.go :42/:61; footprint go/types/infer.cs 2 lines per target; CNR 0; go/types builds at the seat tip): post the reading element by element, then the seat push-then-announce (new ref, one signed commit, parent = the version tip read at origin or 5a03aac159 -- i9 merges either). Then HOLD until COORD assigns RED 3's cut on C2's sizing (fips140/hmac CS0311: pointer type argument vs method-set constraint; likely yours after RED 2 with the build proof). The CR write-back lesson (Git Bash text-mode grep never matches CRLF; test by byte count; the untouched sibling is the tell) is banked for the merge-hazards skill. Row 20 LAST after the gate is green. 90-min cadence.
