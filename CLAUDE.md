...
- **A new converter `.go` file must be registered in `src/go2cs/go2cs-src.projitems`** — the VS
  ...
  **Three more census/launch traps, each paid repeatedly (2026-08-17):** a DEFAULT ripgrep honors
  ...
  ⚠ **A BEHAVIORAL-TREE CENSUS KEYS ON THE GO PACKAGE NAME, NEVER THE DIRECTORY NAME** (2026-09-08, G):
  the emission keys on the package CLAUSE — a `sortlocal/` directory declaring `package sort` emits
  `sort_package` — so a directory-keyed census of root-package collisions read NONE while the
  package-keyed one read exactly one file, the lane's own guard. The Go-side twin of the alias rule
  above: a directory census answers a different question and reports a clean zero for the wrong reason,
  and the guard being its own counter-example is what exposed it before publication.
  ...
  flat-only control — four identical numbers from one query, and the only tell was that **a census whose
  arms CANNOT differ is announcing that it does not discriminate.** Fixed with the `:(glob)` magic
  pathspec and controlled 0 against 3. Two companions from the same record: a "displacing X leaves Y
  UNREACHED" sentence read as if the displacement achieved it for ALL of Y when it achieved it for four
  sites and 58 were dead by construction (two reasons in one clause — stated, not rewritten); and at
  go1.24.13 the `getcallerpc` family is ABSENT (0 sites), reborn as
...
- **⚠ An UNBANKED package's `-tests` assembly is in NO standing gate — route #7's shape, one
  ...
  **STRENGTHENED** the original hazard, because unreached is exactly why no gate sees them and exactly
  ...
  argued.
  ⚠ **A CORRECT GENERAL RULE APPLIED TO AN UNVERIFIED PARTICULAR IS STILL A GUESS** (2026-09-08, R, twice
  in one day): "dependents are skipped, not errored" correctly said ONE root behind five skipped
  GolibTests references — and the root was then PICKED, the failure already in mind, rather than DERIVED.
  One grep of the csproj `ProjectReference` graph, run only to audit an already-published claim, found
  the real root (`fmt` → `slices`, CS8761, 99 dependents) and moved that seat's acceptance from one
  blocker to three. **The rule names the SHAPE, the derivation names the MEMBER; run the derivation
  BEFORE the claim.**
  ...
  ⚠ **A MARKER-KEYED CENSUS CANNOT OBSERVE A FILE THAT CARRIES NO MARKER BY DESIGN** (2026-09-08, G): the
  frozen-metadata class — the four packages whose every production file is hand-owned, so the driver
  never re-emits their `package_info.cs`, csproj or README — was invisible to a hop alias census that
  walked MARKED files. Re-derived at master, 2 of the 4 are affected and both RELOCATE at the next
  release as renames, so an alias hand-edit would patch metadata for a package that will not exist. The
  sizing ruled the relocation COMPOSED with an un-freeze: un-freezing the metadata is a converter seat
  (its footprint at the current release is those four packages' metadata re-minted, the eight missing
  forced-init hooks arriving), and the relocation rides the hop's hand-own branch. Its corollary: **THE
  MARKER THAT PROTECTS A HAND-OWN FROM BEING CLOBBERED PROTECTS AN ORPHANED HAND-OWN FROM BEING CLEANED
  UP** — the deletion pass tests PROTECTED first, so orphan removal at a hop is explicit work.
  ⚠ **UN-FREEZING THE FROZEN METADATA CANNOT CLEAR A STRAGGLER IN A PACKAGE THE QUEUE NEVER VISITS**
  (2026-09-08, G): the `-stdlib` queue is `go list std` at the SOURCE release, so a package RENAMED at
  the target release never enters the target's queue and no emission re-mints its metadata — the
  RELOCATION is what clears the CS0426, while the un-freeze is what makes the relocation's destination
  self-minting rather than re-frozen at the NEXT hop. The coordinator's "relocation composed with
  un-freeze" ruling stood; its implied "the un-freeze levels the straggler" did not. **Read the queue's
  POPULATION before assigning a remedy a leg.**
...
- **FALSE-GREEN route #8 — a guard DISARMED by a LEGITIMATE change (found 2026-09-01, the
  ...
  ⚠ **A CENSUS KEYED ON `partial <type> X` COUNTS THE LEGAL CASE** (2026-09-08, R): 141 "collisions" were
  `partial class <pkg>_package`, which every file declares by design — a duplicate partial TYPE is legal
  and only a duplicate MEMBER is CS0102 — so the predicate keys on `[GoType]` and on member declarations
  per package instead. Withdrawn by its author before the number was posted; third predicate over-match
  in one day.
  ...
  ⚠ **WHERE A GUARD'S COMMENT NAMES A CLASS, ITS CONDITION IS KEYED ON THE CLASS, NEVER ON THE INSTANCE
  THAT PROMPTED IT — a comment cannot widen a condition** (2026-09-08, G/R): `convIdent:227`'s box-render
  escape hatch says in its own words that a REF RECEIVER has no box, that reaching it inside a
  lambda-conversion context (a `defer` or a `go` method group) renders a nonexistent name, and cites
  flate's init CS0103 ×11 — while its flag asks "is this a ref RECEIVER" where the render needs "does
  this ident have a box AT ALL". Ref-lowering later created the identical box-less binding for a
  ref-lowered PARAMETER (`paramIsRefLowered` already existed at `refLoweringEmissionOperations:450`) and
  the condition stayed receiver-only. TWO sites, the `defer` and the `go` twin at `visitGoStmt`, found by
  putting a `go` variant in the probe rather than assuming. R's hedge ("same shape, not the same code
  path") is scored as an UNDER-claim: right on the evidence held, weaker than the evidence would carry.
...
- **testhost lock gotcha:** a stray `testhost`/`vstest.console` from a prior run can lock
  ...
  **⚠ An MSTest verdict WORD is not a verdict — an ABORTED run prints one anyway** (measured 2026-09-02,
  ...
  ⚠ **`dotnet test --filter <expr>` THAT MATCHES NOTHING EXITS 0 WITH "No test matches"** (2026-09-08, i9
  via C1), so an rc-only gate reports 10 of 10 from a run that executed ZERO tests — the vacuous-green
  class through the tool everyone trusts to be loud. **A filtered MSTest gate asserts the EXECUTED count
  against the expected count and greps for `No test matches`, never rc alone.**
  ⚠ **GOLIBTESTS ON A LINUX HOST IS RED BY CONSTRUCTION WITHOUT `-p:GoTargetOS=linux`** (2026-09-08, C2):
  48 failed / 744 at BOTH configurations, every block a `DllNotFoundException` on kernel32 from the
  converted `syscall_package` static initializer; pinned, after a depth-UNLIMITED purge of 2,021 output
  directories, at 0 failed / 781 both, count-reconciled (788 `[TestMethod]` − 4 in a netapi32 file − 3
  filtered = 781). The csproj's own 2026-08-31 comment had ruled it. **A gate routed to lanes on two OS
  families carries the flavour pin in its instruction**; and a window-grep (`-A8`) over failure blocks
  INVENTED six non-kernel32 findings where block-parsing gave 48/48 — parse on real block boundaries.
  ⚠ **A GATE WHOSE NEW TESTS ARE ENVIRONMENT-GATED `Assert.Inconclusive` READS COUNT-MATCHED WHILE NEVER
  EXECUTING THEM** (2026-09-08, i9): 747 count-matched on four legs with eight census controls SKIPPED —
  skip delta +8 in BOTH configurations, exactly the new file. "GolibTests green" for such a branch is
  clean-because-untested; **the gate for an env-gated control set is the leg with the variable SET,
  posted with the controls' own lines.** Companion: two branch Debug runs against one base run establish
  "not a deterministic regression", never a RATE.
  ⚠ **A DERIVATION CAN READ THE RIGHT NUMBER BY LUCK** (2026-09-08, G): 752 was reached by subtracting
  the FIRST FOUR files of an EIGHT-file `Compile Remove` group, the inverse of a sibling lane's
  735-vs-739 slip. **A count is stated WITH its derivation, and MSBuild `-getItem:Compile` is the sound
  source**, never a number on its own. Three `FixtureLinkStagingTests` failures on the same run are the
  symbolic-link privilege, a HOST capability shared by G-LAPTOP and the i9 box, named from the error
  text.
  ⚠ **A HOST'S ADMISSIBLE COUNT IS DERIVED PER FLAVOUR BEFORE THE RUN AND CONTROLLED ON ANOTHER FLAVOUR**
  (2026-09-08, G; two readings of one rule): the linux compile set is NOT the windows 752 — the `!=
  'linux'` group that removes eight files on windows KEEPS them on linux, while the `!= '' and !=
  'windows'` group removes two — so GolibTests at the linux flavour read 784/0/1 = **785**, exactly what
  `-getItem:Compile` (129 items) had derived, with the same instrument's windows arm reading the i9 box's
  752 as the positive control. A reused 752 would have read a correct run as 33 short. **Derive per
  flavour with `-getItem:Compile`, never carry a count across.** Three companions from the same
  first-contact run: a skip whose name was observable only in an interim read (that host's temp directory
  did not survive between invocations) was re-derived from the TRX artifact rather than quoted; a first
  clone 645 commits stale — the Windows repo's LOCAL master, not `origin/master` — was caught before any
  reading; and the one deviation from the install scoping (two profile export lines, user state, required
  by the login-shell bar) was NAMED by the installer rather than found by the reader.
  ...
  ⚠ **A PROCESS CENSUS KEYED ON A WORKTREE PATH IS BLIND TO A `go test` BINARY** (2026-09-08,
  coordinator): `go test` builds `<pkg>.test.exe` into a TEMP build directory and runs it with the
  package directory as cwd, so neither its `ExecutablePath` nor its `CommandLine` carries the worktree
  string — a census reading ZERO `go.exe` under that worktree while the suite was mid-run said NOTHING.
  **Liveness of a `go test` is the harness task's own notification, or the test binary BY NAME**; and a
  zero-line log at a fresh mtime is the run's buffering, not its death.
- **⚠ CONCURRENT-SESSION KILLS — worktree isolation does NOT isolate `Get-Process <name> | Stop-Process`.**
  ...
  **⚠ "An instrument that can match ITSELF is measuring its own presence" (named by the lane that
  ...
  timed run.
  ⚠ **A KILL CENSUS THAT MUST INCLUDE THE HOST SHELL BY DESIGN MATCHES ITS OWN LAUNCHER** (2026-09-08,
  coordinator): a stray monitor was killed and re-armed in ONE background command, and the PowerShell
  census `Name -eq bash.exe -and CommandLine -like '*<script>*'` matched the task's OWN parent bash —
  whose command line carried the script name because it was about to launch it — killing four processes
  including the querying task, exit 255. The self-match trap through the LAUNCHER's door, where the usual
  host-shell exclusion cannot apply because the target IS a bash script. **Never kill and launch in one
  command; and where the host-shell exclusion is unavailable, exclude by ANCESTRY (own PID and parent
  chain) or by AGE — a stray is minutes old, the launcher seconds.**
  ...
  **⚠ Correction to the pipe rule, measured 2026-09-03: a pipe masks a command's exit status only
  ...
  ⚠ **`grep -q` UNDER `set -o pipefail` SILENTLY DROPS TRUE MATCHES** (2026-09-08, R): `grep -q` exits at
  the first match and closes the pipe, the producer dies of SIGPIPE, pipefail reports the PIPELINE
  failed, and `&& echo hit` never fires — measured on a 4,093-line hand-own whose marker sits at line 28
  (exit 255 with pipefail, 0 without) while 144 smaller files were unaffected, the one miss reading as a
  phantom ladder-only file. **BOTH halves of the banked pipe rule are true**: a pipe masks a status
  WITHOUT pipefail, and pipefail plus an EARLY-EXITING CONSUMER (`grep -q`, `head`, `grep -m1`) is the
  defect. The fix is `grep -c` and a test on the COUNT — it reads the whole input and cannot SIGPIPE its
  producer. Rooted only by varying ONE option after two plausible wrong mechanisms (stdin consumed by
  `git show`; process substitution against a pipe), both drawn from real banked traps, **which is what
  made them attractive.** Grep your own instruments for `pipefail` in the same file as an early-exit
  consumer.
  ⚠ **THE `grep -q`/pipefail HAZARD HAS NO SIZE THRESHOLD** (2026-09-08, C2): 400 fast lines exit 0, 500
  exit 141, and TWENTY lines with a 20 ms sleep between them exit 141 — the discriminator is whether the
  producer is STILL WRITING when the consumer exits, so **a site cannot be certified safe by inspection**
  (a three-line producer becomes unsafe the moment it streams) and a census by producer size cannot be
  written; the structural fix (`grep -c` plus a count test, safe by construction) is the only checkable
  one. 125 raw sites collapsed to FIVE copied patterns — the count was never the unit — and the two with
  streaming producers are both ABORT guards that fail toward FALSE GREEN: a dirty tree reading clean, a
  failed build reading passed. "None of my sites is exposed because every producer fits the pipe buffer"
  was written and then measured false in REASONING even where true in outcome.
  ⚠ **THE TWO EARLY-EXIT CONSUMERS ARE NOT EQUALLY DANGEROUS** (2026-09-08, i9 and R): with `head -N` the
  VALUE still arrives and only the exit status is poisoned (rc 141, which most sites never read), while
  with `grep -q` **THE EXIT STATUS IS THE ANSWER**, so it inverts silently — the audit therefore targets
  pipelines where an early-exiting consumer's status IS the result and the producer emits many lines (a
  rare-pattern grep over a 6 MB file into `grep -q` is safe, one line emitted; `cat` of the same file is
  not). "A large input with an early match" was a SUFFICIENT condition its author happened to hit,
  corrected by that author to the race it is: a small slow producer looks perfectly safe on the page, the
  same site can read right today and drop a match under load tomorrow, and it presents as an
  unreproducible phantom. One lane's security gate is immune because it sets NO pipefail — **luck,
  recorded at the site as "the tidy-up that adds it is the commit that arms this"** — and *"a correct
  rule applied to the wrong symptom is indistinguishable from insight until you vary one thing and
  measure"*, which a day of banked lessons makes MORE likely, not less.
  ⚠ **THE pipefail CONDITION IS "WRITES STILL PENDING WHEN THE CONSUMER EXITS"** (2026-09-08, R):
  measured on the discriminating arm — three lines in ONE `printf` write is the only safe shape (rc 0),
  while 400 `printf` lines, a 20-line bash loop and 300k `seq` lines all read 141. Volume and slowness
  are two ROUTES to that state. One disagreement between boxes was reported as **UNRECONCILED** — two
  boxes, two producer shapes, neither having varied the other's variable — rather than resolved by
  argument; and four earlier arms were CONFOUNDED (a multi-stage pipeline where an earlier stage
  SIGPIPEs; a bash loop as the "fast" arm) and would have refuted both lanes, withheld with *"a
  confounded arm that agrees with your prior is the easiest thing in the world to publish."* The
  practical rule is unchanged and now has its reason: **the only certifiable producer completes its whole
  output in a single write, which cannot be read off the script, so the census stays structural.**
  ⚠ **A DISAGREEMENT BETWEEN TWO HONEST MEASUREMENTS RESOLVES ON A THIRD BOX BY VARYING THE ONE VARIABLE
  NEITHER VARIED** (2026-09-08, i9): line count and byte count held fixed, PRODUCER SHAPE varied — 400
  lines from `printf` with 400 substitution arguments (it cycles its format and writes incrementally)
  reads rc 141 and DROPS the match, 400 lines from `printf` of a pre-built string (one completed write of
  the same ~800 bytes) reads rc 0, `seq | sed` at 400 is safe and bare `seq` at 300k is dropped. The
  discriminator is **WRITE GRANULARITY** — whether the producer's entire output landed in a COMPLETED
  write before the consumer exits — one lane's condition stated as a mechanism; "it fits the pipe buffer"
  is not the explanation, because the write has to have been ISSUED. Both disputing lanes were right
  about different producers. So: a producer cannot be certified by inspection (one `printf` form safe,
  another unsafe at identical output), `grep -q` is the UNSAFE consumer, `head -N` is TOLERABLE, and
  `grep -c` plus an integer test is the fix. *"Three lanes, three withheld conclusions, one day."*
  ⚠ **THE MARGINAL ARMS FLIP BETWEEN REPS, SO EVEN THE CORRECT MECHANISM CANNOT CERTIFY** (2026-09-08,
  C2): with `strace` counting writes to stdout, the same 1,498 bytes and 400 lines read 401 writes /
  4-of-5 dropped (`printf` with arguments) against 3 writes / 0-of-5 (`cat` of a file) — the one-axis
  pair with the mechanism OBSERVED — and a TWO-write producer still dropped 1 of 5, because the second
  process forks after the consumer has already left. **"Few writes, therefore safe" would have certified
  an arm that drops matches**, and a single-rep reading of a marginal arm lands either way, which is how
  two honest lanes measuring nearby shapes got opposite verdicts. The published sentence was refuted by
  its own author on its own box, and the instrument that caught it was the other lane **reporting a
  DISAGREEMENT rather than a refutation.**
  ...
  **⚠ WHY that slot is censused by the PARENT processes and NEVER by `go2cs.exe`** (measured
  ...
  ⚠ **A TREE KILL FROM THE HARNESS TASK'S SHELL DOES NOT REACH A SCRIPT WHOSE PARENT WAS A DIFFERENT
  HARNESS SHELL** (2026-09-08, coordinator): `taskkill /T` on the task wrapper took its child and
  reported success while the assembly script's OWN bash — its parent already gone — kept running a leg
  for five more minutes; the follow-up census read ZERO because it EXCLUDED bash by name and the leg's
  `dotnet` child had an unreadable `CommandLine`, and MSYS `ps` showed the survivor only as a bare shell
  with no arguments. **The lock file the script writes (pid plus script name) was the honest signal** — a
  relaunch REFUSED on it. Three rules: kill from the pid the LOCK names (mapped MSYS to Windows via `ps
  -l`'s WINPID), census bash INCLUDED by command line with the querying shells excluded by AGE, and read
  the lock before believing any "no survivors".
  ...
  **⚠ Three 2026-09-05 sharpenings of that stamp, and the first is the FALSE-RED twin of the
  ...
  across sessions.
  ⚠ **A WAIT KEYED ON A WORD FIRES ON THE INFORMATIONAL LINE THAT CARRIES IT** (2026-09-08, coordinator):
  an `until grep -qE '<words>'` waiter returned at minute ONE on the run's own notice that a pending row
  would "ABORT BY NAME" — the verdict-word family inside a waiter. **Key a wait on the exact VERDICT
  STAMP** (the light-gates-done, assemble-done and assertions-failed stamps), never on a word the log
  uses in prose. Companion (i9): killing a harness task does not stop `check-no-regression.ps1`'s
  PowerShell driver, which kept spawning converter workers for minutes — **the box was not free when the
  task read "stopped"**; terminate the driver by command line and sweep workers by worktree path.
  ...
  **⚠ Two more, 2026-09-04, and the first is the family's `go test` member.** A NON-VERBOSE
  ...
  a zero.
  ⚠ **A fourth member, 2026-09-08:** `grep -i -F` ABORTS on the coordinator box (rc 134, core dumped, no
  output), so **with stderr discarded it reads as a CLEAN ZERO** — a security census's own positive
  control caught it, and the census was re-run under a different tool with all twelve arms proven live.
  Companion from the same cut: a DISCLOSED proof-page row renders as a verdict followed by a
  parenthesised link and not as a bare verdict, so a bare-verdict cell test rejected exactly the rows a
  disclosure census needs (181 phantom not-on-page) — **normalise the cell and confine the scan to the
  verdicts section**, since the page carries a second same-shaped table; the implausible number is what
  caught it.
  **⚠ The false-empty family has a deeper member: instrumentation that never compiled in (measured
  ...
  ⚠ **A PROBE THAT `git show`s A NONEXISTENT PATH READS 0 EXACTLY LIKE A REAL ZERO** (2026-09-08, i9): a
  registration check named `TargetTests.cs` where the class file is `TargetComparisonTests.cs`, read 0
  added, and was right BY LUCK, the real count also being 0; the mint's per-class shape gate caught the
  same wrong name and REFUSED, which is what it was for. **A probe naming a file asserts the file EXISTS
  (`git cat-file -e <ref>:<path>`) before it counts, or its zero is not a reading** — the false-empty
  family through a path-typo door.
  ...
  **⚠ The general form, named after five instances in ONE day (2026-09-01): an instrument built out
  ...
  ⚠ **"ENV-GATED, FREE WHEN OFF" IS NOT "OBSERVATION-ONLY WHEN ON"** (2026-09-08, i9): a census's arm-4
  classifier performed an EXTRA `Resolve` on every conversion, and `Resolve` evicts dead weak entries and
  reassigns the count — so the instrument MUTATED the registry at moments the uninstrumented program
  would not, and a BANKED row (`os`, PASS 683) flipped to FAIL with the census ON, twice, both
  directions, one variable. **An instrument built out of the thing under test proves its NEUTRALITY on a
  banked row with the instrument ON before any of its numbers are tabled**; numbers taken before that
  proof are floors or void, never the record; and a remedy is never sized against a population the
  instrument may have manufactured or hidden.
  ⚠ **AN INSTRUMENT THAT WRITES TO THE PROGRAM'S OWN STDERR PERTURBS EVERY TEST THAT COMPARES A CHILD'S
  STREAM, AND AN ENV GATE INHERITED BY CHILDREN ARMS IT IN PROCESSES NOBODY INSTRUMENTED** (2026-09-08,
  C2/i9): the census printed "armed" from a `ModuleInitializer` at start and its whole block from a
  `ProcessExit` hook at exit, while `childEnvWithGo2CSPath` copies the ENTIRE parent environment and
  scrubs only `go2csPath` — so the gate variable reached every helper child and a child doing ZERO census
  work emitted 222 bytes onto the stream its parent compares. `os` spawns helpers (a row flipped);
  `go/types` and `encoding/json` do not (no flip), and **the mechanism that also explains the NEGATIVES
  outranks one that explains only the positive.** Two named-and-refuted candidates preceded it, the
  second an `Interlocked` counter placed ON the hot path by the neutrality fix itself — fixing a false
  pass introduces the next one. Rules: **an instrument's reporting channel is a FILE, never the program's
  stdout/stderr; a gate carried in the environment is inherited by children and must be scrubbed or
  scoped; and neutrality is proven on the banked row that SPAWNS children.**
  ⚠ **A SHARED PER-ROW INSTRUMENT FILE WITHOUT A PROCESS ID LETS A PARENT AND ITS HELPERS RACE, AND THE
  BLOCK THAT SURVIVES IS THE LEAST REPRESENTATIVE ONE** (2026-09-08, C2/i9): with `{pid}` the `os` row
  writes FIVE files (parent 260,132 conversions; children 94, 100, 96, 16); without it the surviving
  block was the smallest child, 16, and was tabled as the row. **Every census number taken through the
  shared path was VOID — not truncated, WRONG** — and the "floor" built on four million conversions went
  with it, since a destroyed block could have held the very hits the floor denied. A per-process
  instrument NAMES its process in its file, a row's table is the SUM over blocks with the block COUNT
  printed, and a reading with fewer blocks than helper processes is not tabled.
  ⚠ **A SHARED-FILE RACE READS THE MODAL BLOCK, SO A ZERO FROM IT IS A LIKELY ZERO, NOT A MEASURED ONE**
  (2026-09-08, i9): `crypto/tls` carries 1,236 arm-2a conversions across 507 of 2,241 bogo processes and
  ZERO in the other 1,734, so one surviving block drawn from that row reads zero with probability 77% —
  the old method would MOST LIKELY have reported "the corpus takes no arm-2 path" with four million
  conversions behind it. Three fixes were each NECESSARY to see it: instrument neutrality (the verdict is
  the row's), per-pid files (every block survives), per-row sums (the row is the sum). The population's
  existence changes nothing about the withdrawn remedy — the sites live in a banked row passing 3,643
  verdicts — and everything about the record. Companion: **a `ProcessExit`-reporting census reads NOTHING
  from a host that dies before exit, indistinguishable from no conversions**; report incrementally and
  mark partial.
  ⚠ **A PER-PROCESS INSTRUMENT THAT FLUSHES CUMULATIVE SNAPSHOTS IS FOLDED BY THE LAST BLOCK PER FILE,
  THEN SUMMED ACROSS FILES** (2026-09-08, i9): one pid writes PARTIAL at conversions=1, PARTIAL at
  250,135 and FINAL at 260,132 — monotone snapshots of one running total — so the naive fold (sum every
  block) read 510,580 against the true 260,440, a factor of 1.96, while "take the FINAL block" silently
  drops the processes that never wrote one, which are exactly the processes the flush exists for. **The
  instrument's READING RULE is stated WITH the instrument: increment or snapshot, and how to fold.** A
  reconciliation against the earlier measurement (260,440 against 260,438, two new one-conversion
  processes) is what made the fold trustworthy; a host that dies before its first conversion stays
  UNMEASURED, since the flush does not rescue it.
  ⚠ **A CENSUS WHOSE OUTPUT INVITES THE WRONG FOLD IS THE CENSUS'S DEFECT** (2026-09-08, C2 accepting
  i9's ask): the partial blocks were cumulative snapshots, the naive sum overcounted by 1.96x, and
  NOTHING in the output said so. The reader now emits the per-file LAST value and the row total so the
  naive sum is not the reachable reading, and a row whose main host writes nothing is recorded **NO
  USABLE CENSUS**, never a near-zero.
  ⚠ **"TAKE THE FINAL BLOCK" IS WRONG TOO** (2026-09-08, C2): a process killed before it finishes leaves
  only a PARTIAL block — the case the flush exists for, two of seven files — so the fold is the per-file
  LAST value with the partial LABELLED, and **the output STATES ITS OWN FOLD RULE** (a fold line carrying
  a control arm), because an output that invites the wrong fold is the instrument's defect. The reader is
  core cmdlets only, runs on both PowerShell editions, and REFUSES (exit 1) on a file carrying no census
  block.
  ⚠ **"NO FILE" IS AMBIGUOUS ACROSS THREE CANDIDATES** (2026-09-08, C2) — never armed; armed but died
  before its first conversion; armed with zero conversions and no exit hook. The partial flush moved the
  census's boundary from EXIT to the FIRST CONVERSION and left everything before it in the same hole, the
  falsifier one step earlier. The honest move was to DECLINE to pick the most plausible, take the FREE
  arm first (correlate the writing pid against the pipeline's own process record — it halves the space at
  no cost), and build a block written AT ARM TIME (conversions = 0, one write at module init, nothing on
  the per-conversion path) **so an absence becomes a positive reading**, controlled both directions.
  Keeping the hazard and the cause as TWO sentences from the start is what let one be withdrawn without
  defending the other.
  ...
  ⚠ **A SECOND NUMBER DERIVED FROM THE FIRST AND WRITTEN BESIDE IT AS "N DISTINCT NAMES" IS FALSE
  CORROBORATION** (2026-09-08, i9): the "nine" matched NEITHER definition — 10 excluding one disclosed
  row, 8 cases — and the charitable "definitional" reading was too charitable, as the author said.
  **Chase a number that moves nothing when it disagrees with another derivation.**
  ⚠ **PROPOSING A MECHANISM FOR SOMEONE ELSE'S DISCREPANCY IS NOT NEUTRAL** (2026-09-08, C2, the other
  side of that exchange): "it is definitional" handed the reader a story that made a miscount look
  SETTLED and stopped the checking — and the discrepancy matched NEITHER definition. **Report the
  discrepancy; let its owner explain it.** The correction sharpened the finding rather than softening it:
  8 of 8 unique cases, zero overlap.
  ⚠ **TWO INSTRUMENTS FROM OPPOSITE DIRECTIONS LANDING ON THE SAME TWO FILES CLOSES A CLASS**
  (2026-09-08, G): a corpus BUILD found the hop ladder's 16 roots in `runtime2.cs` and `mfinal.cs`, and a
  compiler-free GOROOT-plus-master alias census (145 marked files, 94 converted-package aliases: EXISTS
  92 / MOVED 2 / ABSENT 0) named the same two files and the same alias — so its answer to the rung order
  is "after runtime there is NOTHING ELSE in this class". Free datum from the same census:
  `runtime/internal/math` also moved to `internal/runtime/math`, aliased by no hand-own.
  ⚠ **THREE DERIVATIONS SHARING NO INSTRUMENT NAMING ONE SET — AND THE THING NONE OF THEM CAN SEE**
  (2026-09-08, G/C1/R): a corpus build, that compiler-free alias census, and a COMPLETED piece of
  maintenance sitting on an unmerged branch all named `runtime2.cs` + `mfinal.cs` — and **a file needing
  a retarget and a file whose retarget lives on an UNMERGED branch look IDENTICAL from GOROOT.** A wall
  read as "NOT DONE, next rung" was a re-base SUBTRACTION, and the ruling that sent a lane to redo it
  repeated the grep-the-implementing-half error — `git branch --contains` and the announced branches are
  that check. The census landed as a COMMITTED RECORD rather than as a post, because a 94-row table in a
  post is transport.
  ...
  to substitute for.
  ⚠ **TOTALS MUST CLOSE BEFORE A NUMBER IS POSTED** (2026-09-08, i9): the `runtime` row at master reads
  185 verdicts of 880 with 695 empties reconciled THREE independent ways (880 − 185; the `C#=""` entries;
  the name-set difference) — the first pass read 186/694 from LINE ARITHMETIC between section markers
  rather than from a parse, and the verdict tally read 178 until a filter was found dropping every
  HYPHENATED verdict, all seven infrastructure-error rows. The next door is named by test AND frame
  (`TestLockOSThreadNesting`: a goroutine logging after its test completed because the converted lock
  counts read 0,0 where Go wants 1,0, the teardown fatal ahead of it; the host produces nothing after
  it). And six of the 43 failures are managed-pointer-token refusals, **reported as a count and a shape
  and NOT as a claim about the refusals' correctness.**
  ...
  **the fourth COUNTED what it reads — 203 proof pages, 195 one platform, ONE the other — and found a
  ...
  ⚠ **READ THE ROWS, NOT THE TOTALS** (2026-09-08, G): four mapper defects found that way, none of them
  visible in a total. A `/MOVED/` verdict matched inside `REMOVED` — fixed by RENAMING the verdict to
  ABSENT so the tokens are substring-disjoint, not by a smarter grep; the corpus root namespace went
  unstripped (29 false ABSENT rows); a naive dot-to-slash rewrite broke a VENDORED DOMAIN, fixed by a
  resolver that re-joins adjacent pairs and takes the first existing path; and a MOVED resolver answering
  by first same-basename hit named a REAL package that was the wrong one, so it now ranks on shared
  components, excludes toolchain trees and reports AMBIGUOUS rather than picking, controlled on two known
  answers.
  ...
  **A NARROW census's zeros are believed only because the SAME detectors read NON-ZERO on the BROAD
  ...
  ⚠ **A CLASS THAT HIDES IN PACKAGES THAT DO NOT BUILD IS CENSUSED STATICALLY OVER THE PRISTINE TREE,
  NEVER BY A BUILD** (2026-09-08, R): `sort`'s forced-init hook collision — `initᴛᴛimportꓸslices`
  declared by BOTH the 1.24 emission's `package_info.cs` and the 1.23.12 hand-own `sort_impl_go121.cs` —
  survived a whole ladder because `sort` never built behind `slices`. The static detector read 1
  collision across 112 `_impl.cs` and was believed only because the SAME pattern read 2,695 hook
  declarations across 334 `package_info.cs` files (the broad population is the narrow census's positive
  control); it ran on the PRISTINE tree because the lane's own working tree had already dropped the hook
  and would have HIDDEN the member; and the fix — drop the hand-own's copy, identical `[GoInit]`
  semantics — was VERIFIED compiling before it was proposed, with the site's comment saying why a re-base
  does not clear it. Companion converter root from the same post: a REF-LOWERED pointer parameter meeting
  a BOX-FORM defer emission (`defer(Ꮡr.Value.root.decref, …)`) — the defer lowering does not consult the
  parameter's ref-lowering decision, the `[GoRecv]` ref-receiver box-form trap one construct over.
  ⚠ **A CENSUS OVER HAND-OWNS TAKES THE CLASS, NOT A FILE SUFFIX — measured twice in one day on two
  different instruments** (2026-09-08, R): "census every `_impl.cs`" scanned 112 companions and missed
  the 44 whole-file `[module: GoManualConversion]` rewrites — 156 hand-owns in all — one of which
  declares a forced-init hook (`crypto/internal/boring/bcache/cache.cs:50`); the ANSWER did not move and
  it gained a REASON, since bcache is one of the four hand-owned-by-consequence packages whose
  `package_info.cs` is never re-emitted, so it can never RECEIVE the relocation and the collision class
  is immune there BY CONSTRUCTION — the difference between "I found nothing" and stating what the
  population DOES. The hop ladder's PREFLIGHT was blind in the same way: a 16-root wall, identical on
  three flavours, sat in two `[module: GoManualConversion]` files carrying 1.23's package layout while
  the set diff read 115/115 against a MARKED set of 145. **A hand-own preflight widens to the
  line-anchored MARKER census and reports BOTH counts.** R's third narrowing of one census,
  self-reported; four lanes probed their own gates in sequence that hour, each prompted by the previous
  one's hole.
  ...
  **An EMPTY census is actionable only in its STRONG form — what the population DOES, not what it
  ...
  ⚠ **AN INSTRUMENT'S OUTPUT IS READ AFTER IT RUNS, NEVER DESCRIBED FROM A TRUNCATED EARLIER VIEW**
  (2026-09-08, C1, four misses in one day): a self-record control was claimed to have "fired naming
  `getg`" from an earlier `head -8` run in which `getg` was visibly ABSENT — the lane told itself it
  "would be there" and published while the post tool's own output showed it did not fire, the specificity
  band that killed 3,118 `runtime` hits having also excluded `getg` at 108. **`head -8` is a silent WHERE
  clause inside a control's description.** The three earlier misses were absence claims about the world
  ("I have minted nothing", "getg appears unowned", "14 of 34" against a committed 47), each one grep
  away and two made INSIDE the post correcting the first; the remedy is a script that puts the committed
  record's lines in front of the author — deliberately NOT a gate, because an honest absence claim is
  common and a refused honest post gets routed around.
...
- **⚠ Banked-row protection at MERGE time — two rules, both paid for by the crypto/tls regression
  ...
  **And the canary RULE is now split (R's proposal, ratified 2026-09-01):** a change to the reflect
  ...
  ⚠ **THE SAFE-PUSH SELF-TEST CANNOT PASS ON A SHALLOW CLONE** (2026-09-08, C2): `TestSafePushSelfTest`
  aborts on `shallow update not allowed` while seeding its hermetic origin — identical at the announced
  base, green on a full clone in 187 s — so **a cloud-container lane attributes that ONE converter-suite
  red to its clone before hunting: a host-qualification fact, not a tree fact.** Companions: `go test`
  from a worktree ROOT with no `go.mod` exits 1 and an empty grep of its output reads as an answer; and a
  background task's exit code is the WRAPPER's last pipe stage, not the suite's.
  ...
  **A battery's FIRST LEG and a SOLO re-run are not two arms of one experiment** — the deciding arm runs
  ...
  ⚠ **A RULING THAT CITES A BANKED VERDICT AS A PROPERTY OF THIS TREE, WITHOUT A RUN ON THIS TREE,
  CARRIES AN UNMEASURED PREMISE** (2026-09-08, coordinator): "inside a banked row at 3,643 verdicts PASS
  on that same tree" quoted the ROSTER — another day, another host, and on the i7 a standing "not
  measurable here" — while the lane's run on that tree read FAIL with the census ON and the OFF control
  unfinished. The lane caught it before the control landed and named the three outcomes; the CONCLUSION
  survived because it rested on other evidence, and the ruling was corrected in its REASON before the
  premise was tested rather than after. **A roster figure names a tree and a day; a sentence about "this
  tree" names a RUN.**
...
- **⚠ The three-run flake standard, and the A/B a re-converting SWEEP silently invalidates
  ...
  ⚠ **A ROW WHOSE FAILING MEMBER MOVES IS READ ACROSS THREE RECORDS, EACH WITH A PLANTED-DIFFERENCE
  CONTROL** (2026-09-08, i9, `crypto/tls`): three census-OFF runs (393/398/394 s) read Go=pass/C#=fail
  among bogo cases 0, 0 and 0 with the positive control on the same query reading 2, 1 and 3; the
  skip-set difference was strictly one-way three times of three; exactly ONE divergence was shared across
  records, an already-DISCLOSED row; and ZERO bogo subtests diverged in more than one run. So the row's
  own divergence set is that single name and everything else is per-run bogo nondeterminism — and a claim
  that the ORACLE is the flaky side was corrected by the same numbers. Each record passed three controls
  before being read (both sides 3,644 and equal, every name joined, a planted difference detected), and
  the records were preserved before the sweep's own cleanup.
  ⚠ **A CLAIM STATED THREE TIMES IS OVERTURNED BY THE FOURTH OBSERVATION** (2026-09-08, i9):
  "`crypto/tls` does not reach PASS on this host" became PASS at 3,643 verdicts with the census ON —
  which the author's OWN three-record finding (nine transient bogo divergences, zero overlap) had
  PREDICTED, since a row whose failing member MOVES will sometimes have none. "Does not reach PASS"
  should have read "did not in three of three, and the divergences are transient"; it was retracted with
  the caveat that the pass and the fails sit on two tips of one branch, which is CONSISTENT with
  transience and not proof. (`net/http` passing on the same box is the runner's own unconditional-tiering
  defect closing.) Two instrument rules from the same post: **a NEW instrument's first number is compared
  against one derived another way before it is published** — an `awk` fold whose key carried a trailing
  space silently excluded every PARTIAL-only process, exactly the ones the flush exists to rescue, caught
  against the runner's independent 260,440 — and **a per-run population moving with how much of the row
  executes** (1,018 against 1,236) **is sized as "four figures in one package", never to either number.**
...
- **⚠ 37 packages are in LAYOUT L3 and the ritual below is UNCHANGED because of it, not despite it.**
  ...
  **Two L3 gate lessons (measured 2026-08-15, the three-target leveling lane):** (1) a change whose
  ...
  fitting the answer.**
  ⚠ **THE FLAVOUR'S BUILD LOG IS THE INSTRUMENT; THE LEFTOVER `bin` TREE IS THE LAST FLAVOUR'S**
  (2026-09-08, R): a three-flavour ladder purges before each flavour and darwin runs LAST, so
  "`internal.syscall.windows.dll` PRESENT under `bin`" was true of darwin — where that package compiles
  nothing — and false of the windows question being asked. Corrected before it reached the post. **Read
  per-flavour presence from that flavour's own log (built=N errors=N), never from what survives on
  disk.**
...
- **⚠ Blast-radius measurement for a converter change: TWO seeded reconverts diffed against each
  ...
  ⚠ **A SEEDING CONTROL'S PREDICATE IS THE HAZARD, NOT A PROXY FOR IT — and a byte-for-byte `git
  archive`-against-checkout comparison is the WRONG LAYER on a mixed-ending tree** (2026-09-08, the
  coordinator and a sub-agent standalone; one finding stated twice, the second correcting the first's
  mechanism). A train's seeding leg refused itself on 1 of ~300 converter files: a `_test.go` whose blob
  is LF, from a Linux lane's commit, that autocrlf smudges to CRLF in the checkout, 263 of 264 reading
  `w/crlf` — the ARCHIVE applies the conversion, so the brief's inverted mechanism made the conclusion
  STRONGER, not weaker. A `_test.go` is not a `go build` input and Go source line endings do not reach
  the binary (raw string literals spanning lines being the one exception, to be measured), so the
  archive-built base converter WAS the base converter and the leg reported UNMEASURED for a CONTROL
  fault. The corrected control has THREE arms, each keyed on what can change a BINARY: non-test files
  CR-strip-identical (189, zero differences); `_test.go` EXCLUDED from the criterion while a CONTENT
  change there is still red (a planted defect was refused — the split is STATED, not used as the
  criterion); and the 15 `//go:embed` targets byte-identical. The first writing of arm 2 refused a benign
  EOL-only difference in a non-test file and was caught by its own control — the exact false-refusal
  class the exercise was correcting, one file over.
- **⚠ The bank unit for a converter change's corpus footprint is the two-seeded diff's HUNKS, never
  ...
  ⚠ **A PREDICTION CAN MISS ON A FACT ITS AUTHOR POSTED AN HOUR EARLIER** (2026-09-08, R): 5 → 1
  predicted, 5 → 2 measured, the extra being a fatal-report type the same lane had itself reported
  BLOCKED (zero files at landed master) before applying a file that depends on it — *"predicting 2 was
  available to me from my own writing."* Its assembly falsifier fired (869 → 862 assemblies) and was read
  as "sync is the gate and STILL blocked, now by the ruled dependency" rather than as "wrong about the
  gate". The lane prepared a self-consistent re-point to the ruled-AGAINST exception shape and did NOT
  apply it, **leaving the ladder at 2 with both reds named, owned and dated — a better state than a
  consistent tree carrying a shape the fleet ruled against** — applied as a DELTA, never the peer's whole
  file, and verified after: the peer's references and pair present, all four orphans gone, markers
  intact.
  ...
  **a control whose INPUT is wrong is thrown out and rebuilt, never read** (a two-hex-digit `printf`
  ...
  one it did not have.
  ⚠ **A LINE-LEVEL FOOTPRINT PREDICTION DERIVED FROM ONE FLAVOUR'S COMMITTED FILE IS FLAVOUR-BLIND, AND
  ITS "MISSED" ON ANOTHER FLAVOUR IS THE PREDICATE'S, NOT THE TRAIN'S** (2026-09-08, coordinator): a
  standalone leg read windows MET / darwin MET / linux MISSED on the one predicted file
  (`runtime/runtime2.cs.auto`, +1/−1), because the predicted line pair was a seat's `Δtrace → trace` on
  the `m` struct's `[GoValueClone(…)]` list AS WINDOWS EMITS IT, while linux's `m` carries `sigmask` at
  the head of that list — so the linux pair differed by the field list and the compare read NO.
  Reproduced from two rebuilt binaries with content KEPT: linux's delta is exactly the same mechanism on
  linux's own line. **A per-flavour prediction compares the MECHANISM — the token pair inside the line —
  or derives its expected line PER FLAVOUR; and a failed arm's DIFF CONTENT is preserved before any
  purge**, since that run purged its seeds and binaries and left only `Files differ`, so the miss had no
  evidence until it was re-run. Note the standing class it rode on: a FLAT `.cs.auto` review sibling of a
  hand-own whose emission VARIES per GOOS.
  ...
  behavioral census.**
  ⚠ **A FOOTPRINT PREDICTED ZERO BY ARGUMENT COVERS EVERY CONTEXT THE RULE FIRES IN, OR IT IS A
  PREDICTION ABOUT ONE CONTEXT** (2026-09-08, G/coordinator, with G's same-day amendment folded in):
  "such a site would be CS0266 today, so none can exist" is true of an ASSIGNMENT context and FALSE of an
  EXPLICIT-CAST context, where `(uintptr)<ulong literal>` compiles through `uintptr`'s explicit operator
  — so a native-width unsigned prefix reached a behavioral golden as an intended-but-unpredicted
  emission, found by the union CNR at CHANGED 1. The amendment sharpens the boundary: **"it compiles
  today" bounds a converter branch only for IMPLICIT-CONVERSION sites**, because the converter's own
  explicit-cast wrapping makes BOTH the bare `UL` and the `(nuint)`-prefixed form compile, so a 307/307
  compile-clean corpus says NOTHING about whether explicit-cast sites reach the signed-parse branch — a
  correct argument about one class published as an argument about the population. Two halves, adopted
  fleet-wide: **enumerate the CONTEXTS a converter predicate fires in (assignment, cast, argument,
  return, composite element) before arguing any is empty; and every footprint prediction names BOTH
  populations — corpus via the two-seeded diff, behavioral via CNR — or says which it did not measure.**
...
- **Metric:** measure **packages-compiling**, not raw error count. Fixing file-inclusion bugs (e.g. the
  ...
  ⚠ **A LADDER'S TOTAL IS NOT THE CUTTER'S PREDICTION; THE CUT'S OWN SITES ARE** (2026-09-08, R): a rung
  predicted 24 → 20 and measured 40/34/44 per flavour, because clearing one package let ~1,100 more
  assemblies compile and their errors were unreachable before — while the sub-prediction about the cut's
  OWN sites held exactly. Third instance of the count-versus-own-sites split: **predict the sites the cut
  REACHES, and score the total as a READING of the residue every other class contributes.**
  ⚠ **ON AN UNMASKING LADDER THE ERROR COUNT STOPS MEASURING PROGRESS; THE COMPILED-ASSEMBLY COUNT IS THE
  HONEST MEASURE** (2026-09-08, R): rung 5 read 24 errors at 750 assemblies and four exact cuts later the
  ladder read 34 errors at 2,752 — the error total UP 40 percent while the compiling corpus more than
  TRIPLED, every cut's own cleared set exact and every net swamped by what it unblocked. **Predict the
  CLEARED SET, the cut's own sites to zero, and report the net as a READING**; a net-total prediction on
  such a ladder is a bet on how much more corpus compiles. Companion: a compiler names only the
  implementers it actually COMPILES, so "one site reported twice" is never the population — a CS0535 fix
  reached a second implementer another lane's build never named.
...
- **Windows local time works (fixed 2026-08-01).** Binding the converted `time` exposed a pre-existing
  ...
  **A retention fix is keyed on the ARGUMENT's shape (a bridged same-frame box), never on the callee's
  ...
  ⚠ **A DOOR CAN BE RIGHT ON CLASS AND WRONG ON ITS OWN STATED REASON** (2026-09-08, C2): `runtime`'s six
  managed-pointer-token refusals are 6 of 6 the reference-bearing-pointee class, refused by name and none
  of them the pin-unheld hole — and 5 of the 6 refuse on a premise ("native code would read or write
  memory that is not the caller's") that is FALSE for them, since the five callback rows pass a funcval
  pointer at argument 3 as an OPAQUE PASS-THROUGH COOKIE that the OS carries back to Go's own `callback`,
  which reinterprets and calls through it IN GO; native code never dereferences it. Every call shape was
  re-derived from the pinned GOROOT source — the zero-based index 3 matching the door's own report is two
  derivations of one number — and the five rows are ONE funnel with SIX callers. **No remedy is owed from
  a results file**: what one would owe is the emission route MEASURED, the inbound half (can `callback`
  recover its box) measured on the same row, the sixth caller run, and a falsifier — a native callee that
  STORES and DEREFERENCES the cookie.
  ⚠ **A NAMED COMPLETENESS GAP CAN BE A DIFFERENT FACT ALREADY IN THE ARTIFACT** (2026-09-08, i9): the
  sixth caller of that funnel is absent from the refusal list because it NEVER REACHES the token door —
  Go=pass / C#=fail in 1 ms at its own line, on `runtime.LockOSThread didn't`, before any syscall wrapper
  — so the funnel reads FIVE of six with the sixth UNMEASURED with respect to the door, neither
  counter-example nor confirmation, and the hand-owned `LockOSThread` no-ops cost a SECOND test that its
  name had filed under the callback question. Read from the PRESERVED RECORD rather than held for the
  dispatched solo run (still owed: a solo run is a different experiment), it hands the other lane's root
  a falsifiable prediction on a test it was not derived from — **the fix should move the sixth caller
...
- **Phase 3 complete (2026-07-10 — commit `51ba5d9cf`, tag `stdlib-green-2026-07-10`):** all **302**
  ...
  **Compiling is the milestone, NOT operational** — operational validation is Phase 4 (running Go's own
  ...
  - **⚠ Phase-4 operational: two hand-owned patterns, and a WHOLE-FILE rewrite MUST carry the marker.** Making a
    ...
    **whole-file** hand rewrite *replaces* the converted `<name>.cs` and **must carry `[module:
    ...
    ⚠ **WHEN A HOP MOVES A FACT INTO THE CORPUS, ASK WHETHER ITS CONSUMER IS CONVERTED OR HAND-OWNED —
    the first updates itself, the second never does** (2026-09-08, R): the 1.24 runtime carries
    `waitReasonSyncWaitGroupWait = 24` with its string free with the corpus (`runtime2.cs:881/927`), but
    `mapWaitReason` lives in `runtime/stubs_impl.cs` under `[module: GoManualConversion]`, so without a
    hand line the member still falls to `waitReasonZero` and the runtime-DERIVED guard fails on the 1.24
    tree exactly as it does at 1.23.12. **A hop census lists, per moved fact, the hand-owns that CONSUME
    it.**
    ⚠ **A BODY THAT BECOMES WRONG SILENTLY WHEN A STUBBED PACKAGE IS LATER IMPLEMENTED CARRIES ITS
    RETIREMENT TRIGGER AT THE SITE** (2026-09-08, R): `runtimeNow`'s non-bubble body is a delegation to
    the hand-owned `now()` of identical signature, and the bubble refusal is real BY CONSTRUCTION —
    `synctest.Run` throws first, since `internal/synctest` is five throwing stubs — while no guard could
    be written, because `inBubble` is one of those five (unreachable code asking an unanswerable
    question). The site's comment therefore NAMES all five stubs as the reason and states that the moment
    synctest is implemented this delegation hands a bubbled goroutine the REAL wall clock without failing
    loudly, so whoever lands synctest owns revisiting it: **the obligation sits in the FILE, not in a
    board row.** Stub census 243/22 → 242/21 on windows, with `time` leaving the list.
    ...
    **`[module: GoManualConversion]` needs the `go.` qualifier when it precedes the namespace
    ...
    ⚠ **A `-tests` CONVERSION INTO A SCRATCH OUTPUT ROOT DROPS EVERY HAND-OWNED `*_impl.cs` THE PACKAGE
    DEPENDS ON** (2026-09-08, i9), so the build fails naming exactly the missing bodies (`abiSeq` without
    `regAssign`, which `abi_impl.cs` defines) and READS AS THE PACKAGE'S STATE. The sweep converts IN
    PLACE — `core/<pkg>` as the output directory — precisely because the hand-owns live there, so **a
    measurement of a row hand-rolls the sweep's own invocation line or runs the sweep.** A lane published
    "neither named row can complete" off this and retracted within the hour; the census finding beside it
    stood untouched because it had been run through the sweep.
  ...
  **That property is normally invisible because it is normally not tested.**
  ...
  ⚠ **THE ORPHAN CENSUS OVER THE WINDOWS RECORD READS 7 PASS/PASS OF 267 ENTRIES** (2026-09-08, increment
  1 of that check): six in `sync` (five alloc-profile rows and one codegen-liveness) and `crypto/tls`'s
  bogo row as a host-limit — plausibly its third environmental outcome, ruling nothing; 157
  Go-pass/C#-fail entries are LIVE, and 75 are UNEXAMINED for want of a proof page (`reflect` 62,
  `runtime` 6, `runtime/pprof` 6, `unique` 1 — all unbanked rows). **Increment 1 is REPORT-ONLY (record
  key plus stderr line) because the manifest is SHARED across platforms**; increments 2 (platform-scoped
  entries) and 3 (the refusal) are specified in the record, not built. The `unicode/utf8` positive
  ...
  **And a ROW IS NOT AN ASSERTION**: a "cheapest want-zero row in the package" claim was wrong by
  ...
  - **⚠ After an operational SWEEP, `git status` is dirty and it is (almost always) NOTHING — classify,
    ...
    what it says.
    ⚠ **A SWEEP'S DIRT IS CONTROLLED AT MASTER BEFORE IT IS CHARGED TO THE CHAIN THAT RAN IT**
    (2026-09-08, i9): a `sync` sweep after a fatal-path chain left 24 files dirty (+199/−250, `[GoInit]`
    hooks dropped from `cond.cs`, a new `package_info_internal_test.cs`) — and the SAME sweep at landed
    master left the same 24 files with the same numstat, the two drift diffs differing only by a blob
    index and the proof page's own converter stamp. So it is master's standing relocation debt, flagged
    for the regen and not fixed under a measurement. Beside it: the probe was run as the BUILT BINARY
    because its own README had measured that `go run` masks the exit code (1 against the binary's 2), and
    both pins were asserted on version AND GOROOT and ABORTED on, since printing a pin is not checking
    it.
    ⚠ **A BARE `git checkout <ref> -- .` OVER A SEAT'S OWN WORKTREE IS A REVERT WITH NO MARKER**
    (2026-09-08, G): it STAGES master content over every seat file, `git status` calls it modified
    exactly as it would real work, HEAD stays at the seat SHA, and any gate run on that tree measures
    LANDED MASTER while reporting as the seat — green because master is green, the purest false green.
    **Read the dirty COUNT against what you expect before any gate**; prefer ref reads (`git show`, `git
    grep <ref>`) when only content is needed, since they cannot touch the tree; restore with a checkout
    of HEAD over the directory (narrower than a hard reset) and verify content BY GREP, never by the SHA.
...
- **⚠ SECURITY — no real machine names or other internal-infrastructure identifiers on ANY pushed
  ...
  **A lane does not scrub a shared transport branch piecemeal** — the remedy is the coordinator's, as
  ...
  ⚠ **A LINE-ANCHORED SECURITY CENSUS CANNOT SEE A TOKEN WRAPPED ACROSS A LINE BREAK** (2026-09-08, i9):
  of six planted shapes five fired and the SPLIT token was missed — an account name broken mid-token
  across the wrap, with a sibling lane's SHA wrapped the same way. The first fix, joining on a bare
  newline, was PARTIAL and its own re-probe caught it: an INDENTED continuation and a TRAILING SPACE at
  the break, the two commonest real shapes, were still missed, and collapsing whitespace ADJACENT TO THE
  BREAK closes all three without fusing arbitrary words. Structural, not per-arm — the joined copy is
  appended to the buffer EVERY arm searches, so an arm added tomorrow is covered. **A red arm on ONE
  historical plant shape proves the census catches that shape and nothing else: probe with realistic
  variants before believing a gate.**
  ⚠ **THE JOIN-CLASS DISCRIMINATOR IS CONTINUATION, NOT LENGTH** (2026-09-08, C2): a leaked path
  CONTINUES past the account — the next byte is a separator — where fused prose ENDS at a space or EOL,
  and a minimum-segment-length rule fails in BOTH directions (fused 9/10/16-character vocabulary against
  4-character real accounts). 8 of 8 wrap positions kept, 6 prose shapes freed, one residual STATED (a
  break exactly at the separator AND a path ending at an unknown account). Third instance of "a mechanism
  does not transfer by resemblance" — a sibling's redundancy was real on its own gate and absent on this
  two-arm one. Its red control was INVALID the first time, run against a tree not yet carrying the prose
  arms (0 red where 4 were required), and was fixed by asserting that the variant DIFFERS in the intended
  way AND that the arms it must fail are PRESENT; a duplicate arm that fired two kinds was DELETED rather
  than the attributability test loosened.
  ...
  ⚠ **A BATTERY'S COARSE IDENTIFIER CENSUS READS THE SECURITY GUARD'S OWN FIXTURES AS HITS** (2026-09-08,
  coordinator): one run read residual = 11, ALL of them inside the guard's own census test file — the
  detector's quoted literals and its `Sprintf` fixtures, whose SEGMENT after the root is a placeholder —
  with the owner-account alternative at 0 and the authoritative repo guard green inside the same battery
  leg at the union. Ruled a TEMPLATE class keyed on the SEGMENT and **never on the path** (the guard's
  source is never exempted by name, so a fixture carrying a REAL segment still counts), applied to hard
  hits too, with the excluded lines PRINTED beside the residual and a six-arm control before any merge:
  four fixture shapes excluded, while a real segment and a placeholder-elsewhere line SURVIVE — the two
  arms that keep the class from laundering a name. Proved from the INSTALLED bytes on the union delta
  (raw 21 / template 11 / residual 0), after a first patch that collapsed its backslashes in transit and
  broke the parse, rebuilt from `chr(92)` per this file's own rule.
  ...
  **The pre-post census runs before EVERY push — diff-scoped and EXIT-GATED, or it is decoration**: a
  ...
  ⚠ **A HOME-PATH ARM IS PROVEN INDEPENDENT OF THE ACCOUNT NAME ONLY BY A FOREIGN-NAME PROBE**
  (2026-09-08, G): the real account name COINCIDED with the owner-token arm, so "all refused at 2 hits
  each" proved NOTHING about the path arm — per-arm isolation defeated by a probe whose token TWO arms
  can match. Re-probed with a FOREIGN name the arm stands alone (home path REFUSED, bare foreign name
  CLEAN, windows path REFUSED). **Every lane gaining a Linux host inherits the exposure**, so that arm is
  re-controlled there rather than carried from the Windows reading.
...
- **⚠ Two lanes solving the same problem produce a SILENT DUPLICATION, not a conflict.** Independently
  ...
  **a clean auto-merge of two cuts touching ONE method is read whole for two REPRESENTATIONS of one
  ...
  ⚠ **A PACKAGE SPLIT CAN DECLARE ONE GO PRIMITIVE TWICE, AND TWO SEATS CAN BODY THE TWO COPIES
  DIFFERENTLY WITH BOTH GREEN ALONE** (2026-09-08, C1): at the newer release `throw`/`fatal` are declared
  in `sync/runtime.go` AND in the new `internal/sync/runtime.go`; the five call sites split 2/3 and
  `sync.Mutex` DELEGATES to `internal/sync`, so every Mutex-misuse path runs through the SECOND pair —
  and a union with one seat re-pointing `sync`'s pair to the fatal report while another bodies
  `internal/sync`'s as the catchable exception ships ONE primitive in TWO behaviours with the hot path
  un-converted. Named by the lane whose own seat it NARROWS (its acceptance covers 2 of 5 sites, restated
  at measured scope rather than left to read as coverage) BEFORE either lands; ruled one primitive, one
  behaviour, the acceptance asserted in BOTH files with `TestMutexMisuse` at the new release as the
  measured gate. Its census note: a FILE-scoped count read seven declarations where the PACKAGE has eight
  — **census the package, with a control.**
- **⚠ An INSERT adjacent to a line the other side edited folds into ONE hunk, and BOTH single-side
  ...
  ⚠ **A DERIVE'S OWN COPY OF A TABLE-DERIVED NUMBER IS A LITERAL THAT DRIFTS, AND A SELF-CHECK THAT READS
  THE TABLE CANNOT SEE IT** (2026-09-08, coordinator): an assembly merged all six seats clean and then
  REFUSED on a hardcoded seat count written when the table had five rows, while the derive's own
  self-check had passed six rows because it reads the TABLE. **The assembly derives its expected count
  from the same source the self-check reads, or the self-check covers the assembly's literal.** Beside
  it: a launch wrapper ending in `tail` reported the task's exit as 0 over the script's 1 — **end a
  launch wrapper with `exit $rc`.**
- **⚠ Its MIRROR is the SILENT SUBTRACTION, and it is worse: one lane REMOVES a definition because
  ...
  **A WIDENING of a shared golib door keeps the PREVIOUS arity as a forwarding overload for at least
  ...
  ⚠ **"COMPILES AT BOTH RELEASES" IS A FACT ABOUT COMPILATION STATED AS A FACT ABOUT LANDABILITY**
  (2026-09-08, R) — compiling is not correctness, applied to a LANDING decision. A golib `WaitReason`
  member for a park reason the converted 1.23.12 runtime does not carry BUILT at 878 projects and FAILED
  the runtime-DERIVED guard (`TheWaitReasonMapIsGosOwnTableByDerivation`) at both configurations: a
  member added ahead of its runtime table is exactly the state that guard refuses, and it stays
  un-weakened. The coordinator's split ruling rested on the compile claim and was REVERSED on the
  measurement. Companion: the search that missed the second guard was `grep | head -8` — a head-limited
  search on the ONE query whose purpose was completeness, the guard-discovery member of that family.
  ⚠ **A NEW MEMBER ON A PUBLIC GOLIB INTERFACE IS A BREAKING CHANGE FOR EVERY IMPLEMENTER, TEST DOUBLES
  INCLUDED, AND THE CUT'S PREDICTION CARRIES THE IMPLEMENTER CENSUS** (2026-09-08, i9/coordinator):
  `ISlice.IsNil` added with one implementation (`slice<T>`) failed GolibTests at BOTH configurations on
  CS0535 at a test double (`ArrayRangeAllocationTests.NamedSliceLike`) — golib itself compiled, the
  CONSUMERS did not, the shape a producer-only compile cannot see, route #7's consumer gate firing as
  named. RULED: **the member goes on EVERY implementer explicitly, and a DEFAULT INTERFACE IMPLEMENTATION
  is never the remedy for a member whose answer depends on state the interface cannot see**
  (`ISliceBacking` is internal by design), because the default would answer `false` for exactly the
  implementers nobody checked — a silent wrong answer wearing a compile-clean face.
  ...
  a ruling's word.
  ⚠ **A PEER'S CUT IS COMPOSED INTO A LADDER TREE BY ITS FULL FOOTPRINT (`git show --name-only <sha>`,
  every path applied), NEVER BY THE FILES ONE REMEMBERS THE CUT FOR** (2026-09-08, R): a first
  composition attempt died CS0122 on a peer's `FatalReport.cs` and read like a defect in a cut its author
  could not compile — it was the COMPOSITION's, since that change widens `CrashReport.WriteToCrashOutput`
  private → internal in a SECOND file that had not been copied, and every assertion written checked
  PRESENCE while none checked CLOSURE. **Composition by memory is the stale-base illusion inside one
  worktree.**
  ⚠ **THE HAND-FIX WOULD HAVE BEEN A SECOND SILENT SUBTRACTION** (2026-09-08, R): a lane's own repair of
  `mfinal.cs` carried master's three finalizer-binding commits and DROPPED a peer's `AddCleanup` entirely
  — no conflict, no marker — while the peer's 3-way (base 747 / ours 822 / theirs 797 → 872, `AddCleanup`
  3, `fingStatus` 20 = 10 + 8 + 2) reproduced to the digit and was applied instead. Three instrument
  notes: a POSIX temp path handed to NATIVE git under `MSYS_NO_PATHCONV` read rc 255 with 0 merged lines
  while a bash-side `wc -l` on the same paths worked — the namespace split, third time in a day — so a
  **NON-EMPTY-INPUTS assertion is now the cheapest thing in both lanes' merge instruments**; a widened
  preflight read 144 of 145 with a phantom ladder-only file, caught only because an independent 145
  disagreed; and **a CLASS description can stand while its ATTRIBUTION is withdrawn** ("a hop class
  arriving early" became "my own re-base's subtraction").
  ⚠ **A FILE CAN DISAGREE WITH ITSELF, AND THE GUARD IS A PER-MEMBER STAMP CENSUS** (2026-09-08, R and
  C1): applying a peer's re-derives took the hop ladder's windows leg 120 → 1 distinct errors, the last
  being a `GoValueClone` attribute naming a field the same blob declares under a DIFFERENT spelling —
  five of six members match a field, one matches none — because a re-derive's 3-way (base `.cs.auto` /
  ours hand file / theirs fresh emission) took the ATTRIBUTE line from one side and the FIELD from the
  other with NO conflict: the `mfinal.cs` subtraction one file over. **The census is "does every stamped
  name resolve to a field of the declaration it stamps"** — master 34 stamped declarations / 39 names / 0
  unresolved (the negative arm), the defective blob 39 / 57 / 1, exactly that one name (the positive
  arm), and 0 after the fix — **so the finding is ONE instance and not a class, and nobody goes
  looking.** The applier's original check compared the DECLARATION line, which both sides shared, and
  never the names inside; its one false positive was a C# VERBATIM identifier, which `\w` does not match.
  The fix went ON TOP, never a rewrite, and is RED at the current release by construction until the hop.
  ⚠ **A PRE-FIX EMISSION IS NOT A MERGE ARTIFACT, AND ONE `--is-ancestor` SEPARATES THEM** (2026-09-08,
  G/C1/R): the blob carrying that stamp PREDATES the train commit whose own guard cites the case
  verbatim, so it is a STALE CONVERTER's own output with no cross-side mixing needed — **the remedy for a
  stale emission is RE-EMISSION under the fixed converter, where a 3-way artifact would want the merge
  re-run.** A mechanism AND a lane were named on a hedge ("likely, not measured") and both halves were
  wrong, while the two MEASURED facts (a name with no field; the review sibling spelling it correctly)
  were sufficient alone. Beside it, one more two-dot reading: a branch census taken as a two-point diff
  against master reported a 587-line deletion in this very file where the three-dot form from the merge
  base reports one.
...
- **A branch behind master shows master's newer files as DELETIONS.** This is the stale-base illusion,
  ...
  **merge base** (`git merge-base A B`), never from a moving tip.
  ...
  ⚠ **A SCRATCH TREE PINNED AT A FIXED BASE GOES STALE INVISIBLY FOR EXACTLY THE FILES THAT MATTER MOST —
  THE HAND-OWNS** (2026-09-08, R): a hand-own landing on master AFTER a ladder's base is absent from the
  ladder, and because a hand-own is what BODIES a displaced or bodyless declaration its absence presents
  EITHER as a phantom compile blocker indistinguishable from a real wall (`rtlGetVersion` CS0103 from a
  missing `zsyscall_windows_version_impl.cs`; restored, windows 34 → 32, prediction held) OR as NOTHING
  AT ALL (`time/sleep_impl.cs` bodies `runtimeNow`; without it the tree carries one generated throwing
  stub that compiles clean and throws at run time — blind to every error-count instrument). **Preflight
  before ANY rung is read: `git ls-tree -r master -- src/core` against a `find` over the tree, every
  missing file NAMED, and the ladder re-based on the landed master at each train landing.** That seat's
  blocker list moved a third time in one day, each movement the finding.
  ⚠ **A RE-BASE OWES THE STALE-FILE DELETION PASS** (2026-09-08, R): the seed's
  `exp_aliastypeparams_off.cs` sitting beside the emitted `_on.cs` killed the first build at 186 dlls in
  80 s; 86 stale files were deleted by WRITE-EVIDENCE, zero conflicts, the three flavours' sets
  identical. Ruled with it: **1.24 hand-own edits live on ONE branch off master that never seats on a
  train** — they cannot compile against the 1.23 corpus — **and land WITH the H5 regen**, with the
  following stage reviewing them file by file.
  ...
  **on a commit that also changed two non-test converter sources `go build` compiles in.** A skipped gate
  ...
  ⚠ **A TEMPLATE STAMP THAT RESTATES A NARROW CENSUS AS A CLASS-WIDE ZERO IS A FALSE READING WITH A TRUE
  NUMBER INSIDE IT** (2026-09-08, coordinator): an assembly's FINDING line said "no seat in this train
  touches `src/go2cs` or `src/gen`, and G11(b) asserts that zero" while THREE seats touched `src/go2cs` —
  G11(b)'s zero is over the lift/dedup OWNER SET only. **A stamp that names a census as its evidence
  prints THAT census's population and scope beside the number, and a class-wide claim is computed by its
  own one-command derivation (per-seat converter-file counts) rather than borrowed from a neighbour's.**
...
- **Re-fetch immediately before any merge in a live campaign.** Refs move under you; arithmetic against
  ...
  **reachability is the one property invisible from inside the worktree that holds the work.** The lane
  ...
  ⚠ **ANNOUNCE-THEN-PUSH BUYS THE RECONSIDERATION** (2026-09-08, C2): a FINAL tip drafted with two more
  commits — one touching golib — would have invalidated FOUR of the i9 box's five running legs (the
  solution build, the full suite and GolibTests twice; CNR unaffected) for an instrument-ergonomics fix
  that blocks nobody, and it was re-weighed only because the post was drafted BEFORE the push. **The SEAT
  is the scarce resource, not the increment**: the two commits went to a separate branch and FINAL stayed
  at the tip already under gate.
...
- **⚠ SEATING MECHANICS, five rules measured 2026-09-03.** **A SEATED branch takes NO commits at
  ...
  **A measurement tree must CONTAIN every seat the cut depends on** — `git merge-base --is-ancestor
  ...
  order, never a cut.**
  ⚠ **A MEASUREMENT TREE CARRIES A SEAT'S SHAPE ONLY WHEN THE SEAT'S TYPE IS REACHABLE** (2026-09-08, R):
  a ruled re-pointing could not be done in a hop ladder because the type exists on a peer's UNLANDED
  branches and nowhere in master or the ladder (CS0246), so the ladder keeps the pre-seat exception pair
  BY NECESSITY **and says so**; and the lane REFUSED to patch the contested seat into a measurement tree
  to get it, because resolving that contest inside a measurement tree would be choosing an order that is
  its owner's to choose. The same tree's `sync/runtime_impl.cs` 3-way (base 320 / ours 239 / theirs 344 →
  263, rc 0) kept the lane's own nine references where a verbatim take would have dropped them silently —
  the subtraction class one file over — and the prediction was restated WITH its condition rather than
  carried.
  ...
  ⚠ **A RULING THAT ASKS FOR AN ARTIFACT CHECKS WHETHER IT EXISTS FIRST — one `git show
  origin/master:<path>`** (2026-09-08, coordinator/C1): the coordinator ruled a lane to mint a host-fatal
  manifest entry on the strength of that lane's "I have minted nothing"; the entry had been at master
  SIXTEEN HOURS. Two halves: **a lane's sentence about its OWN pushed surface is a claim like any
  other**, and **a coordinator asking for a deliverable derives its ABSENCE from the tree, never from a
  post** — the dispatch-preflight rule above, applied to rulings. Companion from the same post:
  **citations for a corpus row are read AT THE CORPUS PIN** — line numbers taken at 1.24.13 (the
  converter pin, because the ambient `go` switches UP inside `src/go2cs` under `GOTOOLCHAIN=auto`) cited
  `exec.go:225-231`, which is PAST EOF at 1.23.12's 222 lines, and a line past EOF is the cheap tell.
  ⚠ **A RULING THAT NAMES WORK GREPS THE IMPLEMENTING HALF FIRST — and "I wrote that rule an hour ago" is
  not the check** (2026-09-08, coordinator/C1, twice in one hour): one ruling named a mint that already
  existed; the next scheduled managed `getg`, which had landed with train 27, on a lane's "bodyless on
  all three flavours" — a claim produced by two instruments structurally INCAPABLE of finding a partial's
  implementing half, namely reading the DECLARATION file (`stubs.cs`) and a filename pattern keyed on the
  SYMBOL, while the body lives in a file named for its CONCERN (`stubs_impl.cs:102`). **An instrument's
  evidence base is a property of its KEY.** The narrow check is the lane's own is-implemented script —
  implementing half (`partial … symbol(…) {`) against declaration (`;`) against a registry entry, with
  raw mentions labelled as neither — and the coordinator runs that question BEFORE any ruling that names
  work.
  ...
  ⚠ **AN IDLE LANE CHECKS ROUTING BEFORE SPENDING A RUN ON AN UNCLAIMED-LOOKING GAP** (2026-09-08, R): a
  lane ran a peer guard's Compile+Output (196.8 s) that another lane had been routed twenty minutes
  earlier. Two hosts agreeing on a .NET gate its author cannot run is CORROBORATION, not a second
  finding, and is recorded as such. **"My lane is idle" is not "this gap is unclaimed": read anchor..tip
  for the routing first.**
  ...
  ⚠ **A SEATED RECORD'S ERRATUM IS POSTED, NOT AMENDED** (2026-09-08, C1): three defects in a seated
  record — a non-greedy extractor taking the modifier `static` as the function name on TUPLE-returning
  declarations (five per flavour collapsed into one phantom function; fixed by enumerating every `name(`
  and discarding keywords, controlled on seven shapes both directions); two of 109 "sites" being the
  DECLARATIONS (call-site population 107, 109 = token occurrences); and a falsifier that used the fatal
  path's own site count as its "reachable" baseline and so **could not fire** (measured reach is 47/46/50
  sites per flavour, with MUST-BE-ZERO and MUST-BE-NONZERO controls run before any bucket prints). The
  headline population and the hop reading stand; **the erratum lands as one dated block AFTER the train,
  and until then the POST is the record's erratum.**
...
- **⚠ REBASE AND RE-LANDING, two shapes with explicit acceptances (2026-09-03).** Two of three
  ...
  **State an acceptance as a relation between two DELTAS, never between two trees, whenever the base
  ...
  merge.
  ⚠ **A RE-BASE'S ACCEPTANCE IS NOT "0 CONFLICTS"** (2026-09-08, C1): a hop re-base touching five files
  collided inside ONE LANE'S OWN TWO CUTS — a seat severs two fatal primitives onto a report type at the
  current release, while the hop cut REMOVES them because the newer Go moved them to another file — and
  both are right for their release while the union is NEITHER (theirs' LOCATION with ours' BODIES
  re-pointed). Taking the obviously-correct side WHOLE silently restores the pre-seat catchable
  exception, with no marker and no gate that builds the newer corpus. **The acceptance was stated as 0
  conflicts on the runtime files, ONE named conflict resolved to theirs, AND the report type present
  twice afterwards — an assertion that FAILS on the silent subtraction — and it was written BEFORE acting
  on the ruling, not after.**
...
- **⚠ THE UNION IS THE GATE OF RECORD, and a landing HOLDS on a broken banked set (2026-09-03).** A
  ...
  ⚠ **A CNR RED ON A BRANCH WHOSE BASE PREDATES A LANDED EMISSION CHANGE IS THE BASE'S, NOT THE SEAT'S**
  (2026-09-08, i9): `CHANGED = 8` at the oracle pin on a seat's branch was exactly the drop set the
  previous train's landing post had PRE-STATED as "seat 1 did not reach its sites", proven without the
  base arm by two facts — the branch changes ZERO non-test converter files, so the binary cannot move
  emission, and landed master reads 0 at the same pin on the same box — and posted inside the minute,
  because a ruled "red stops the battery" must not be applied to a BASE artifact. Beside it: counts
  running a constant 2 lower than master across three readings are the two projects master gained since
  the base — consistent, explained, not a shortfall. The one unmeasured arm (CNR on the bare merge-base)
  was NAMED by its author and declined by the coordinator as not owed.
  ⚠ **A BYTE-IDENTICAL CONVERTER BINARY AT BASE AND SEAT TIP PROVES THE SEAT CANNOT MOVE EMISSION**
  (2026-09-08, C2): `go build ./src/go2cs` at the base and at the seat tip under the pinned toolchain
  gave the SAME sha256 and size, so that same `CHANGED = 8` is the base's BY MEASUREMENT — CNR's verdict
  is a function of the binary and the Go sources it reads, and the seat changes neither. **Reusable in
  about two minutes for any seat whose diff touches no non-test converter source**, retiring the "which
  file kinds matter" argument; the merge-base CNR stays the better arm for the POSITIVE half, that the 8
  are present on the bare base.
  ⚠ **A BRANCH-ALONE GREEN AND A BRANCH-ALONE RED ARE EQUALLY SILENT ABOUT THE UNION** (2026-09-08, C2
  and i9): one lane's oracle pin read 8 on a pre-seat base that reads 0 at the union, while the other's
  split arms COMPILED on the branch and not at the union, where a train's six-parameter widening reaches
  them — measured from opposite sides in the same hour, neither instrument able to see the other's
  finding, both run correctly. **A rehearsal that merges without BUILDING is blind to exactly the seam
  that caught the second**, and the first is why seat gates are "the seat's own lines" while the union
  battery is the gate of record. Method note from the same exchange: the hazard (a real clobber, 19
  blocks destroyed) and the cause of the open row were kept as TWO claims, and the finding survived
  because its author asked a question whose answer it could not control.
- **⚠ TRAIN-ASSEMBLY MECHANICS, four traps in one assembly (2026-09-03).** A seat cut off an OLDER
  ...
  ⚠ **A `merge-file` CHAIN THAT USES THE WRONG MERGE BASE TURNS THE OTHER SIDE'S ADDITIONS INTO DELETIONS
  AT rc 0** (2026-09-08, coordinator): a union check over the four MSTest classes read +1 Check line per
  class with THREE new names and zero duplicates — two master lines had VANISHED — because one seat was
  chained against the landed master where its true `git merge-base` was an earlier commit: the seat's
  unchanged copy then LACKED the previous train's two Check lines, and `merge-file` honoured that as a
  removal. Re-run with each seat's own merge base, the chain read 693 → 696, one line per step, nothing
  removed. **A count that disagrees with a SET (three names, +1 lines) is the instrument, not the
  union**, and the real rehearsal (`git merge`, true bases) is the reading of record — the hand chain is
  a cross-check only.
  ...
  **a derive that FIXES an instrument fault at the source carries NO acceptance path for it** (two
  ...
  ⚠ **AN A-ASSERTION CARRIED FROM THE PREVIOUS TRAIN CARRIES ITS PREMISE, AND THE ARM THAT REPLACES IT IS
  BLOB IDENTITY** (2026-09-08, coordinator with G): one train's "no seat may move a committed behavioral
  golden" was TRUE of that train and FALSE of the next, whose seat modifies a LANDED guard by an
  ANNOUNCED and ACCEPTED golden change — the assembly merged six seats clean and stopped at that
  assertion before any leg, which is the right shape. **The remedy is a STRONGER arm, never an
  exemption**: the assertion admits exactly the ruled file set AND requires the union's change on each
  admitted file to EQUAL that seat's own change. **A GOLDEN IS ITS EMISSION, so the arm is BLOB
  IDENTITY** — `main.cs` and its `.cs.target` are the SAME blob, so the union's blob for each must equal
  the seat's, which beats a numstat (a different fifteen lines can match a count, not a sha) and beats a
  line-set hash; the seat owner supplied the reference values WITH the enumeration ("fifteen paths, no
  others"), so a sixteenth file or a different blob is provably a rider. **Every carried assertion is
  re-read against the new seat set at derive time; a derive that keeps them verbatim has re-asserted the
  previous train.**
...
- **A liveness/health probe must be able to OBSERVE the thing it asks about** (2026-08-29, the iter
  ...
  **name the process you are actually waiting on.**
  ...
  ⚠ **A POST IS DISCHARGED PER ITEM, NEVER PER POST** (2026-09-08, G): a post headed "BOTH ITEMS FROM
  `<sha>`" carried item 1 (the poster's own fix, done) and item 2 ("that is yours to root"); the reader
  read item 1, saw the post was the poster's own work, and reported the WHOLE post handled — the
  heading-as-summary trap from the other side, not acting on a heading but treating a post as discharged
  once its first item was. **And a ruling naming a seat as a row's ride is re-read AT THE SEAT'S CUT**:
  that lane's branch shipped without the twin row the ruling had put on it.
