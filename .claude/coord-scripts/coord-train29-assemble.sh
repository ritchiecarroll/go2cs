...
log="$SP/coord-train29-merge-$ts.log"
...
stamp "LEG syscall-linux $(tail -1 "$SP/coord-train29-syscall-linux-$ts.log") :: $(grep -aE 'Error\(s\)|error (CS|MSB|NETSDK)[0-9]+' "$SP/coord-train29-syscall-linux-$ts.log" | tail -2 | tr '
' ' ')"
...
if grep -aq "DISK PREFLIGHT" "$SP/coord-train29-sweeps-$ts.log"; then stamp "LEG sweeps UNMEASURED: the sweep refused on its disk preflight ($(grep -aoE "[0-9.]+ GB free" "$SP/coord-train29-sweeps-$ts.log" | head -1)) -- battery STOPPED before nistec/reflect; free space, then relaunch the three tail legs"; exit 3; fi
