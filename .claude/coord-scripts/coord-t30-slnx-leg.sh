...
git -c user.name=coord -c user.email=c@x cherry-pick "$FIX" >/dev/null 2>&1 || { echo "ABORT -- cherry-pick failed (conflict)"; git cherry-pick --abort 2>/dev/null; git checkout -q "$ORIG"; exit 7; }
echo "  tree = $(git log -1 --format=%h) : assembly head + refusal"
