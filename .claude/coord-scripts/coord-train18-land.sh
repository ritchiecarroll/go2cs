...
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND18 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/sub-ref-receiver-displacement claude/sub-testing-row claude/sub-stale-converter-refusal claude/sub-alias-overlap-token claude/c1-goroutine-death claude/c1-symbolizer-design claude/c1-symbolizer claude/g-net-linux-bank claude/c2-darwin-classb claude/c2-darwin-classc-read claude/c2-darwin-runlayer-design claude/c2-darwin-nanotime claude/c2-darwin-runlayer-2 claude/c2-darwin-keystone-probe; do
