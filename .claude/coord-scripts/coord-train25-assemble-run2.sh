#!/usr/bin/env bash
# coord-train25-assemble.sh -- merge train 25 on top of the train-24 landed master (set CONTROL_SHA to it), then run its battery. Seats (remote tips; an unset SHA skips):
...
# Every leg stamps `LEG <name> exit=<code> :: <verdict>` into THIS log (the Monitor tails it); per-leg logs keep the detail. Pre-resolved conflicts apply from coord-t25-resolutions/<branch>/<path>.
set -uo pipefail
...
SUBQ32_SHA="${SUBQ32_SHA:-fccc2c59e}"; SUBQ32_BRANCH="${SUBQ32_BRANCH:-claude/sub-q32}"; RINC2_SHA="${RINC2_SHA:-216cc5f5c}"; RINC2_BRANCH="${RINC2_BRANCH:-claude/reflect-cargo-inc-2}"; GB_SHA="${GB_SHA:-a238b1855}"; GB_BRANCH="${GB_BRANCH:-claude/g-b-defer-finally}"
C1Q34_SHA="${C1Q34_SHA:-3c6f1616a}"   # C1 Q34 docs: the init-hook relocation regen-debt census (board block + PLAN-rebank-wave line); set when C1 announces
C2CENSUS_SHA="${C2CENSUS_SHA:-eaae0d998}"   # C2: the train-23 darwin census board block (docs); set C2CENSUS_BRANCH + SHA when C2 announces
C1Q31_SHA="${C1Q31_SHA:-d6af08bf7}"   # C1 Q31: the fourth absorption arm (host-conditional-disclosure) -- shared .ps1 change, 5.1 parse owed at the rehearsal; set when C1 announces
SUBQ36_SHA="${SUBQ36_SHA:-e4a286866}"   # SUB-Q36: net/http re-bank 1343 -> 1345; RE-POINT to the pin-deletion commit; set when final
SUBQ27_SHA="${SUBQ27_SHA:-d1e1300a4}"   # SUB-Q27: the goroutine profile (golib API change -> slnx leg owed); set when the row runs are posted
C1RT1_SHA="${C1RT1_SHA:-44b5089b2}"   # C1: runtime Linux increment 1 -- the rtsigprocmask hand-own body + linux GolibTests guard (bodyless partial displaced by writing it); set when C1 announces
GQ35_SHA="${GQ35_SHA:-}"   # G Q35: I1 + I3 linux/darwin footprint (per-GOOS hunks byte-identical to the emission, remainder routed) + board block + PLAN line; set when G announces
C2INC5_SHA="${C2INC5_SHA:-9074e18ce}"   # C2: darwin run-layer increment 5 -- runtime.sigprocmask over pthread_sigmask via the registry (converter + hand-own + linux contract tests + 2-file footprint); set when verified
SUBQ39_SHA="${SUBQ39_SHA:-}"   # SUB-Q39: the external-variant lift-dedup fix (productionLiftReuseReachable reads the test-project MODEL); set when verified
SUBQ29_SHA="${SUBQ29_SHA:-0a504b2f3}"   # SUB-Q29: the test.v tri-state retirement via golib AdapterBinder; testing row 35+17 -> 37+15 (HEADER COLLIDES with SUBQ36: both write 27,774/167, the union is 27,776/165 -- the roster guard recomposes); set when verified
C2CRASH_SHA="${C2CRASH_SHA:-}"   # C2 Q41 instrument: darwin crash-report collection in os-matrix.yml (workflow only); train 26; set when seated
ts=$(date +%Y%m%d-%H%M%S)
...
log="$SP/coord-train25-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 25 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
merge_one() { # ref msg label
  if git merge --no-ff -S -q -F "$2" "$1"; then stamp "merged $3 -> $(git rev-parse --short HEAD)"; else
    ...
    RES="$SP/coord-t25-resolutions/$(echo "$1" | sed 's#^origin/##' | tr '/' '_')"
...
stamp "TRAIN 25 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train25-roster-guard-$ts.log" 2>&1; rg=$?
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/syscall-keepalive-census.ps1 > "$SP/coord-train25-keepalive-guard-$ts.log" 2>&1; kg=$?
stamp "KEEPALIVE GUARD exit=$kg :: $(tail -c 300 "$SP/coord-train25-keepalive-guard-$ts.log" | tr -d '' | tail -2 | tr '
' ' ')"
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train25-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train25-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train25-suite-cnr-$ts.log"
stamp "LEG suite+cnr $(tail -1 "$SP/coord-train25-suite-cnr-$ts.log") :: $(grep -aE '^ok|LEG1 END|LEG2 END' "$SP/coord-train25-suite-cnr-$ts.log" | tr '\n' ' ') :: CHANGED: $(sed -n '/CHANGED converter output/,/^\[/p' "$SP/coord-train25-suite-cnr-$ts.log" | grep -aE '^ +[AM?] ' | tr '\n' ' ')"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train25-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train25-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train25-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train25-slnx-$ts.log"
stamp "LEG slnx $(tail -1 "$SP/coord-train25-slnx-$ts.log") :: $(grep -aE 'SLNX BUILD END' "$SP/coord-train25-slnx-$ts.log" | tail -1)"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train25-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train25-golibtests-$ts.log"
stamp "LEG golibtests $(tail -1 "$SP/coord-train25-golibtests-$ts.log") :: $(grep -aE 'count-matched|Aborted|Failed:' "$SP/coord-train25-golibtests-$ts.log" | tail -2 | tr '\n' ' ')"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train25-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train25-alone-$ts.log"
stamp "LEG alone $(tail -1 "$SP/coord-train25-alone-$ts.log") :: $(grep -a 'EACH-CLASS-ALONE END' "$SP/coord-train25-alone-$ts.log" | tail -1)"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train25-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train25-reflect-$ts.log"
stamp "LEG reflect $(tail -1 "$SP/coord-train25-reflect-$ts.log") :: $(grep -aE 'exit=' "$SP/coord-train25-reflect-$ts.log" | tr '\n' ' ')"
...
(cd src/tests/Behavioral && powershell -NoProfile -ExecutionPolicy Bypass -Command "\$ErrorActionPreference='Continue'; & './run-behavioral.ps1' --build-timeout 10800 --build-one-timeout 900"; echo "runner exit=$?") > "$SP/coord-train25-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
other_red=$(tr -d "\000" < "$SP/coord-train25-runner-$ts.log" | grep -aE "^\s{2}[A-Za-z0-9_]+ \[" | grep -av "${KNOWN_RED:-__none__}" | wc -l); stamp "FULL BEHAVIORAL: failing projects other than the known red ${KNOWN_RED:-none} = $other_red (must be 0)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Label "$TRAIN_LABEL" -Sweeps net/http,errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache,internal/cpu,time,internal/abi,sync,crypto/internal/alias,slices -SweepTimeout 30m > "$SP/coord-train25-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train25-sweeps-$ts.log"
stamp "SWEEP EVIDENCE: $(grep -a 'RECORD PRESERVED' "$SP/coord-train25-sweeps-$ts.log" | wc -l) record(s) preserved :: $(grep -a 'kept: ' "$SP/coord-train25-sweeps-$ts.log" | sed 's/.*kept: //' | tr '\n' ' ')"
stamp "LEG sweeps :: $(grep -aE 'PASS|FAIL|NOT MEASURED' "$SP/coord-train25-sweeps-$ts.log" | grep -av 'RECORD PRESERVED' | cut -c1-100 | tail -24 | tr '
' ' ; ')"
stamp "BATTERY: nistec COST PAIR vs the train-24 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train25-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train25-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train25-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label "$TRAIN_LABEL" > "$SP/coord-train25-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train25-reflectrun-$ts.log"
stamp "LEG reflectrun $(tail -1 "$SP/coord-train25-reflectrun-$ts.log") :: $(grep -aiE 'moved|PASS|FAIL|matched' "$SP/coord-train25-reflectrun-$ts.log" | tail -3 | cut -c1-120 | tr '
' ' ; ')"
stamp "TRAIN 25 CHAIN DONE"
