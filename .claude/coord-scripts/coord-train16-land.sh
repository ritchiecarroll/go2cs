#!/bin/bash
# Train 16 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
cd /c/Projects/go2cs/.claude/worktrees/musing-moser-d4552c || exit 1
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND16 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/c2-formatter-delegation claude/c1-poll-bank claude/sub-bcache-bank claude/sub-runtime-stoptrace claude/sub-module-namespace-asymmetry claude/sub-typespec-comment-drain claude/c2-multicast-mreq claude/c1-keystone-linux1 claude/sub-windows-pin-holders claude/c2-pin-lifetime; do
