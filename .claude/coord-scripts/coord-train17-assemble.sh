#!/usr/bin/env bash
# coord-train17-assemble.sh -- merge train 12 after train 11 is pushed, then run its battery. Seats (remote tips; an unset SHA skips):
...
RVMN_SHA="${RVMN_SHA:-}"; RCARGO_SHA="${RCARGO_SHA:-}"; C1LBL_SHA="${C1LBL_SHA:-}"; C1PCD_SHA="${C1PCD_SHA:-}"; C1PCR_SHA="${C1PCR_SHA:-}"; C1PCRB_SHA="${C1PCRB_SHA:-5fed14a4e}"; TIMEOUT_SHA="${TIMEOUT_SHA:-}"; D3_SHA="${D3_SHA:-}"; D4_SHA="${D4_SHA:-}"; FUNNEL_SHA="${FUNNEL_SHA:-}"; AV_SHA="${AV_SHA:-}"; GS1_SHA="${GS1_SHA:-}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train17-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 17 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/reflect-tail-r-vmn "$RVMN_SHA" "$SP/coord-merge-r-vmn.txt" "R valueMethodName walk retired via CallerMemberName"
seat claude/reflect-tail-r-cargo-record "$RCARGO_SHA" "$SP/coord-merge-r-cargo-record.txt" "R descriptor-cargo design record (docs; stacked on the vmn seat)"
...
seat claude/sub-rawsyscall-keepalive "$FUNNEL_SHA" "$SP/coord-merge-sub-funnel.txt" "sub-agent RawSyscall keep-alive funnel widening (R1+R3)"
seat claude/sub-pprof-builder-av "$AV_SHA" "$SP/coord-merge-sub-av.txt" "sub-agent pprof profile-builder access violation"
seat claude/sub-convert-timeout-flag "$TIMEOUT_SHA" "$SP/coord-merge-sub-timeout.txt" "sub-agent -convert-timeout flag"
seat claude/g-bprime-s1 "$GS1_SHA" "$SP/coord-merge-g-bprime-s1.txt" "G zh-box B-prime S1"
seat claude/c1-pprof-labels "$C1LBL_SHA" "$SP/coord-merge-c1-pprof-labels.txt" "C1 pprof goroutine labels via AsyncLocal (bucket-B root 1)"
seat claude/c1-pc-registry-design "$C1PCD_SHA" "$SP/coord-merge-c1-pc-design.txt" "C1 synthetic-PC registry design record (docs)"
seat claude/c1-pc-registry "$C1PCR_SHA" "$SP/coord-merge-c1-pc-registry.txt" "C1 synthetic-PC registry increment 1 (stacked on the design seat)"
seat claude/c1-pc-readback "$C1PCRB_SHA" "$SP/coord-merge-c1-pc-readback.txt" "C1 synthetic-PC read-back consumer 1 (stacked on the registry seat)"
...
stamp "TRAIN 17 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train17-roster-guard-$ts.log" 2>&1; rg=$?
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/syscall-keepalive-census.ps1 > "$SP/coord-train17-keepalive-guard-$ts.log" 2>&1; kg=$?
stamp "KEEPALIVE GUARD exit=$kg :: $(tail -c 300 "$SP/coord-train17-keepalive-guard-$ts.log" | tr -d '' | tail -2 | tr '
' ' ')"
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train17-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train17-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train17-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train17-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train17-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train17-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train17-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train17-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train17-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train17-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train17-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train17-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train17-reflect-$ts.log"
stamp "BATTERY: filtered behavioral run (UdpWriteMsgAddrPort, SendtoSeam, TypedNilPtrArrayDims)"
(cd src/tests/Behavioral && for f in TypedNilPtrArrayDims TypedNilPtrArrayPositions; do powershell -NoProfile -ExecutionPolicy Bypass -File ./run-behavioral.ps1 --filter $f; echo "runner $f exit=$?"; done) > "$SP/coord-train17-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache,internal/cpu,time,internal/abi -SweepTimeout 30m > "$SP/coord-train17-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train17-sweeps-$ts.log"
stamp "BATTERY: nistec COST PAIR vs the train-11 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train17-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train17-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train17-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train17-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train17-reflectrun-$ts.log"
stamp "TRAIN 17 CHAIN DONE"
