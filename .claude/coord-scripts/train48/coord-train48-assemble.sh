...
Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>" >> "$MERGELOG" 2>&1 < /dev/null
  ...
  if [ "$mrc" != 0 ]; then
    ...
    while IFS= read -r up; do
      ...
      if [ -f "$src" ]; then
        mkdir -p "$(dirname "$up")"
        # ⚠ (F1b) THE RESOLVED PATH IS **RE-MATERIALISED THROUGH GIT** AFTER THE ADD, and that is a
...
for c in $MERGED_CLASSES; do
  # ⚠ (F2) `golibtests` LEFT THIS LOOP ON 2026-09-13 and is derived from the DELTA below.  A class
...
# ⚠ (F2) `golibtests` IS DERIVED FROM THE **MERGED DELTA**, and it is measured HERE so the stamped
...
while IFS= read -r f <&3; do
  ...
  # ⚠ (F1a) AN `eol=lf` PIN IS AN EXEMPTION, AND IT IS NOT THE `-text` ONE.  Train 48 run 2 refused
