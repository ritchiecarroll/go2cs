#!/usr/bin/env bash
# coord-train10b-assemble.sh -- merge train 10 into the coordinator worktree after train 9 is pushed, then run its battery.
...
RECVMSG_SHA="${RECVMSG_SHA:-}"; CHANDIR_SHA="${CHANDIR_SHA:-}"; CHANDIR_BRANCH="${CHANDIR_BRANCH:-}"; CPUID_SHA="${CPUID_SHA:-acc79ab48}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train10b-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 10b ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
stamp "TRAIN 10b ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"

stamp "BATTERY: converter suite + full CNR"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train10b-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train10b-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train10b-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train10b-syscall-linux-$ts.log"
stamp "BATTERY: slnx"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train10b-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train10b-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train10b-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train10b-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train10b-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train10b-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train10b-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train10b-reflect-$ts.log"
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps crypto/internal/nistec,encoding/json,encoding/xml,crypto/x509,go/types,crypto/aes,crypto/cipher,unicode/utf8,sort,strings,os/exec -SweepTimeout 10m > "$SP/coord-train10b-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train10b-sweeps-$ts.log"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train10 > "$SP/coord-train10b-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train10b-reflectrun-$ts.log"
stamp "TRAIN 10b CHAIN DONE"
