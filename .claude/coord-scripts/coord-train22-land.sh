...
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND22 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/reflect-cargo-r1-1 claude/reflect-cargo-r1 claude/reflect-cargo-inc-b claude/c2-darwin-stderr claude/g-c0-contract claude/g-linux-annotation-refresh claude/g-cgo-class-admission ${C2PIN_BRANCH:-} claude/c2-darwin-netpoll claude/c2-route-endian claude/c2-xpkg-box-bind; do
