...
n=0; miss=0; local_only=0; landed=0; declared_local=0
    ...
    BRANCH:*)
      ...
      if ! git fetch -q "$remote" "refs/heads/$name" 2>/dev/null; then
        ...
        if [ -n "${MASTER_FETCHED:-}" ] && git merge-base --is-ancestor "$sha" "$MASTER_FETCHED" 2>/dev/null; then
          landed=$((landed+1)); echo "  OK-LANDED $name -- ref pruned; ${sha:0:9} reachable from $remote/master"
        else
          echo "  NO REF    $name -- not on $remote and ${sha:0:9} NOT reachable from $remote/master (block claimed on-origin=$claimed)"; miss=$((miss+1))
        fi
        continue
      fi
...
echo "branches=$n missing=$miss landed-and-pruned=$landed declared-local=$declared_local local-only=$local_only"
