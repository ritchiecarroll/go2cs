#!/usr/bin/env bash
# coord-train18-assemble.sh -- merge train 12 after train 11 is pushed, then run its battery. Seats (remote tips; an unset SHA skips):
...
C1SYMD_SHA="${C1SYMD_SHA:-}"; GNET_SHA="${GNET_SHA:-}"; STALE_SHA="${STALE_SHA:-02c9926c4}"; C2DARWIN_SHA="${C2DARWIN_SHA:-9757a64a6}"; C2CLASSC_SHA="${C2CLASSC_SHA:-1c4f856c3}"; C1DEATH_SHA="${C1DEATH_SHA:-}"; C2RLD_SHA="${C2RLD_SHA:-31dbb4768}"; REFRECV_SHA="${REFRECV_SHA:-}"; PARK_SHA="${PARK_SHA:-}"; D4_SHA="${D4_SHA:-}"; TESTROW_SHA="${TESTROW_SHA:-}"; GS1_SHA="${GS1_SHA:-b72fc5b29}"; RMODEL_SHA="${RMODEL_SHA:-}"; C1SYM_SHA="${C1SYM_SHA:-7a71eb403}"; ALIAS_SHA="${ALIAS_SHA:-9e634a854}"; C2NANO_SHA="${C2NANO_SHA:-c4616e951}"; C2RL2_SHA="${C2RL2_SHA:-7a2ba691a}"; C2PROBE_SHA="${C2PROBE_SHA:-f40c69860}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train18-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 18 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/g-bprime-s1 "$GS1_SHA" "$SP/coord-merge-g-bprime-s1.txt" "G zh-box B-prime S1"
...
stamp "TRAIN 18 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train18-roster-guard-$ts.log" 2>&1; rg=$?
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/syscall-keepalive-census.ps1 > "$SP/coord-train18-keepalive-guard-$ts.log" 2>&1; kg=$?
stamp "KEEPALIVE GUARD exit=$kg :: $(tail -c 300 "$SP/coord-train18-keepalive-guard-$ts.log" | tr -d '' | tail -2 | tr '
' ' ')"
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train18-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train18-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train18-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train18-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train18-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train18-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train18-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train18-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train18-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train18-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train18-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train18-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train18-reflect-$ts.log"
stamp "BATTERY: filtered behavioral run (UdpWriteMsgAddrPort, SendtoSeam, TypedNilPtrArrayDims)"
(cd src/tests/Behavioral && for f in TypedNilPtrArrayDims TypedNilPtrArrayPositions; do powershell -NoProfile -ExecutionPolicy Bypass -File ./run-behavioral.ps1 --filter $f; echo "runner $f exit=$?"; done) > "$SP/coord-train18-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache,internal/cpu,time,internal/abi,sync,crypto/internal/alias,slices -SweepTimeout 30m > "$SP/coord-train18-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train18-sweeps-$ts.log"
stamp "BATTERY: nistec COST PAIR vs the train-11 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train18-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train18-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train18-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train18-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train18-reflectrun-$ts.log"
stamp "TRAIN 18 CHAIN DONE"
