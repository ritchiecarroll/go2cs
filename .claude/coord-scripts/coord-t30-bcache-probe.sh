...
P=crypto/internal/boring/bcache
GP="$GOROOT/src/$P"
CP="$W/src/core/$P"
...
./src/go2cs/bin/go2cs.exe -tests -test-action all -test-timeout 30m -test-config Release \
  -go2cspath "$W/src" "$GP" "$CP" > $HOME/AppData/Local/Temp/claude/coord-bcache-pipe.log 2>&1
echo "  pipeline exit: $?"
tail -25 $HOME/AppData/Local/Temp/claude/coord-bcache-pipe.log
REC="$CP/go2cs_test_comparison.json"
echo "=== comparison record: $([ -f "$REC" ] && echo present || echo ABSENT)"
[ -f "$REC" ] && python -c "
import json,io,sys
d=json.load(io.open(r'$REC',encoding='utf-8'))
