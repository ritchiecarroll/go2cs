...
sec=$(git diff origin/master HEAD -- . ':(exclude)src/go2cs/fleetIdentifierCensus_test.go' | grep -a '^+' | grep -aicE 'users\\[a-z]|/home/[a-z]|\\\\[a-z0-9-]+\\' ); stamp "coarse security census over the diff (guard file excluded): $sec hits (must be 0)"
...
for b in ${SUBQ32_SHA:+claude/sub-q32} ${C1Q34_SHA:+${C1Q34_BRANCH:-claude/c1-q34-census}} ${C2CENSUS_SHA:+${C2CENSUS_BRANCH:-claude/c2-darwin-board-t24}} ${C1Q31_SHA:+${C1Q31_BRANCH:-claude/c1-q31-host-condition}} ${SUBQ36_SHA:+claude/sub-q36} ${SUBQ27_SHA:+claude/sub-q27} ${C1RT1_SHA:+${C1RT1_BRANCH:-claude/c1-runtime-inc1-sigprocmask}} ${GQ35_SHA:+${GQ35_BRANCH:-claude/g-q35-i1-l3-footprint}} ${C2INC5_SHA:+${C2INC5_BRANCH:-claude/c2-darwin-sigprocmask}} ${SUBQ39_SHA:+claude/sub-q39} ${C1RT2_SHA:+${C1RT2_BRANCH:-claude/c1-runtime-inc2-hash}} ${SUBQ29_SHA:+claude/sub-q29} ${C2CRASH_SHA:+${C2CRASH_BRANCH:-claude/c2-darwin-crashreport}} ${SUBQ42_SHA:+claude/sub-q42} ${SUBQ40_SHA:+claude/sub-q40} ${C1BILL_SHA:+${C1BILL_BRANCH:-claude/c1-runtime-bill-docs}} ${C2Q44D_SHA:+${C2Q44D_BRANCH:-claude/c2-q44-design}} ${C2MIR_SHA:+${C2MIR_BRANCH:-claude/c2-q44-mirror-population}} ${RINC2_SHA:+claude/reflect-cargo-inc-2} ${GB_SHA:+claude/g-b-defer-finally}; do
