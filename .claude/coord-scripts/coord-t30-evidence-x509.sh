#!/usr/bin/env bash
# coord-t30-evidence-x509.sh -- the CHEAP decisive check after the rtlGetVersion fix merges: crypto/x509 was 491 PASS at
...
ts=$(date +%Y%m%d-%H%M%S); L="$S/coord-t30-evidence-x509-$ts.log"
echo "head $(git rev-parse --short=9 HEAD) -- sweeping crypto/x509 (master read 341 PASS, union lost TestHybridPool alone)"
powershell -NoProfile -ExecutionPolicy Bypass -File src/run-validated-sweep.ps1 -Filter crypto/x509 -Exact -TestTimeout 30m > "$L" 2>&1; rc=$?
