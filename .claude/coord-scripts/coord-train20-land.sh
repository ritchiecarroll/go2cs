#!/bin/bash
# Train 20 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
# GSYNC_BRANCH: set to G's sync-rebank branch at launch (e.g. GSYNC_BRANCH=claude/g-sync-rebank); unset = not pruned.
cd /c/Projects/go2cs/.claude/worktrees/musing-moser-d4552c || exit 1
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND20 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/g-park-rebase claude/c2-alias-overlap-race claude/reflect-cargo-inc-b ${GSYNC_BRANCH:-}; do
