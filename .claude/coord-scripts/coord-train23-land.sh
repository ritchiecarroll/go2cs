#!/bin/bash
# Train 22 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
...
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND23 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/g-record-i1-retired ${C2PIN_BRANCH:-} ${RINCC_BRANCH:-} ${GI3_BRANCH:-} ${GSEMA_BRANCH:-} ${SUBQ14_SHA:+claude/sub-q14} ${SUBQ10_SHA:+claude/sub-q10} ${SUBDOC8_SHA:+claude/sub-doc8} ${SUBQ11_SHA:+claude/sub-q11} ${SUBQ1_SHA:+claude/sub-q1} ${SUBQ2_SHA:+claude/sub-q2} ${SUBQ9_SHA:+claude/sub-q9} ${C1ZR_SHA:+claude/c1-zero-readers} ${SUBQ20_SHA:+claude/sub-q20} ${SUBSEC_SHA:+claude/sub-sec} ${SUBQ17_SHA:+claude/sub-q17} ${SUBQ18_SHA:+claude/sub-q18} ${SUBSECG_SHA:+claude/sub-sec-guard} ${SUBQ26_SHA:+claude/sub-q26}; do
