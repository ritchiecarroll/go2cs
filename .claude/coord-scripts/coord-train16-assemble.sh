#!/usr/bin/env bash
# coord-train16-assemble.sh -- merge train 12 after train 11 is pushed, then run its battery. Seats (remote tips; an unset SHA skips):
...
C2FMT_SHA="${C2FMT_SHA:-}"; RCALLER_SHA="${RCALLER_SHA:-}"; C1POLL_SHA="${C1POLL_SHA:-}"; BCACHE_SHA="${BCACHE_SHA:-}"; STOPTRACE_SHA="${STOPTRACE_SHA:-}"; D3_SHA="${D3_SHA:-}"; D4_SHA="${D4_SHA:-}"; D6_SHA="${D6_SHA:-bfa50ffe8}"; TYPESPEC_SHA="${TYPESPEC_SHA:-}"; GS1_SHA="${GS1_SHA:-}"; C2MREQ_SHA="${C2MREQ_SHA:-}"; C1KEY_SHA="${C1KEY_SHA:-}"; WINPIN_SHA="${WINPIN_SHA:-8a4844996}"; C2PIN_SHA="${C2PIN_SHA:-7f66c422b}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train16-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 16 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/c2-formatter-delegation "$C2FMT_SHA" "$SP/coord-merge-c2-fmt.txt" "C2 test host formatter delegates to converted fmt"
seat claude/reflect-tail-r-callermember "$RCALLER_SHA" "$SP/coord-merge-r-callermember.txt" "R valueMethodName walk retired via CallerMemberName"
seat claude/c1-poll-bank "$C1POLL_SHA" "$SP/coord-merge-c1-poll-bank.txt" "C1 internal/poll Linux bank at 13 (TestSplicePipePool disclosed)"
seat claude/sub-bcache-bank "$BCACHE_SHA" "$SP/coord-merge-sub-bcache-bank.txt" "sub-agent bcache bank 1/1 at Release (row 202)"
seat claude/sub-runtime-stoptrace "$STOPTRACE_SHA" "$SP/coord-merge-sub-stoptrace.txt" "sub-agent StopTrace companion (faithful no-op)"
seat claude/sub-bytes-identity-cast "$D3_SHA" "$SP/coord-merge-sub-d3.txt" "sub-agent bytes identity cast (D3)"
seat claude/sub-array-range-enumerator "$D4_SHA" "$SP/coord-merge-sub-d4.txt" "sub-agent array range enumerator (D4)"
seat claude/sub-module-namespace-asymmetry "$D6_SHA" "$SP/coord-merge-sub-d6.txt" "sub-agent multi-segment module namespace (D6)"
seat claude/sub-typespec-comment-drain "$TYPESPEC_SHA" "$SP/coord-merge-sub-typespec.txt" "sub-agent displaced TYPE comment drain (stacked on sub-orphaned)"
seat claude/c2-multicast-mreq "$C2MREQ_SHA" "$SP/coord-merge-c2-mreq.txt" "C2 IPMreq mirror at the setsockopt boundary (net multicast)"
seat claude/c1-keystone-linux1 "$C1KEY_SHA" "$SP/coord-merge-c1-keystone.txt" "C1 cgocaller keystone increment 1 (credential family via libc shims)"
seat claude/sub-windows-pin-holders "$WINPIN_SHA" "$SP/coord-merge-sub-winpin.txt" "sub-agent Windows hand-own pin holders (R2 Windows half)"
seat claude/c2-pin-lifetime "$C2PIN_SHA" "$SP/coord-merge-c2-pin.txt" "C2 Linux hand-own pin holders (R2 Linux half)"
...
stamp "TRAIN 16 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train16-roster-guard-$ts.log" 2>&1; rg=$?
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train16-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train16-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train16-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train16-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train16-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train16-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train16-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train16-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train16-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train16-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train16-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train16-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train16-reflect-$ts.log"
stamp "BATTERY: filtered behavioral run (UdpWriteMsgAddrPort, SendtoSeam, TypedNilPtrArrayDims)"
(cd src/tests/Behavioral && for f in TypedNilPtrArrayDims TypedNilPtrArrayPositions; do powershell -NoProfile -ExecutionPolicy Bypass -File ./run-behavioral.ps1 --filter $f; echo "runner $f exit=$?"; done) > "$SP/coord-train16-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache -SweepTimeout 30m > "$SP/coord-train16-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train16-sweeps-$ts.log"
stamp "BATTERY: nistec COST PAIR vs the train-11 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train16-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train16-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train16-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train16-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train16-reflectrun-$ts.log"
stamp "TRAIN 16 CHAIN DONE"
