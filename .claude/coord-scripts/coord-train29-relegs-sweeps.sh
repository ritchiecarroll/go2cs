...
stamp "TRAIN 29 RELEGS START head=$(git rev-parse --short=9 HEAD) clean=$(git status --porcelain | wc -l) free=$(df -h /c | tail -1 | awk '{print $4}')"
...
' ' ; ')"
stamp "TRAIN 29 RELEGS SWEEPS DONE -- PAIR the sub-agent, then run coord-train29-relegs-tail.sh"
