#!/bin/bash
# Train 19 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
cd /c/Projects/go2cs/.claude/worktrees/musing-moser-d4552c || exit 1
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND19 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/g-bprime-s1 claude/g-phase-c-record claude/reflect-cargo-inc1-m18 claude/c2-darwin-keystone; do
