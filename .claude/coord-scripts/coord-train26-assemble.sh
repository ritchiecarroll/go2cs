...
log="$SP/coord-train26-merge-$ts.log"
...
stamp "LEG syscall-linux $(tail -1 "$SP/coord-train26-syscall-linux-$ts.log") :: $(grep -aE 'Error\(s\)|error (CS|MSB|NETSDK)[0-9]+' "$SP/coord-train26-syscall-linux-$ts.log" | tail -2 | tr '
' ' ')"
