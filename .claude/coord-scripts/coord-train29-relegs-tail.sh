#!/usr/bin/env bash
# coord-train29-relegs.sh -- re-run the three train-28 battery legs the disk preflight refused (sweeps, nistec pair, reflect run) on the assembled head; stamps into the same assembly log.
set -u
...
stamp "TRAIN 29 RELEGS TAIL START head=$(git rev-parse --short=9 HEAD) clean=$(git status --porcelain | wc -l) free=$(df -h /c | tail -1 | awk '{print $4}')"
stamp "BATTERY: nistec COST PAIR vs the train-28 head (9c44a6d6a) (control worktree must be at that head)"
