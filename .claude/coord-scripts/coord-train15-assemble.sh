#!/usr/bin/env bash
# coord-train15-assemble.sh -- merge train 12 after train 11 is pushed, then run its battery. Seats (remote tips; an unset SHA skips):
...
C2UNIX_SHA="${C2UNIX_SHA:-4d25915565}"; ORPHAN_SHA="${ORPHAN_SHA:-610aef4ae}"; OSROW_SHA="${OSROW_SHA:-3fc315106}"; C1IOVEC_SHA="${C1IOVEC_SHA:-4a7061298}"; C2FMT_SHA="${C2FMT_SHA:-}"; GS1_SHA="${GS1_SHA:-}"; ROSTER210_SHA="${ROSTER210_SHA:-ecc7a61a7}"; NETHTTP_SHA="${NETHTTP_SHA:-c5180f55a}"; C1POLL_SHA="${C1POLL_SHA:-}"; RCALLER_SHA="${RCALLER_SHA:-}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train15-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 15 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/c2-unix-abstract-addr "$C2UNIX_SHA" "$SP/coord-merge-c2-unix-abstract.txt" "C2 abstract unix address read-back + linux guard"
seat claude/sub-roster-denominator-210 "$ROSTER210_SHA" "$SP/coord-merge-sub-roster210.txt" "sub-agent roster denominator 210 (owner ruling; docs; board append)"
seat claude/sub-nethttp-timeout-floor "$NETHTTP_SHA" "$SP/coord-merge-sub-nethttp-floor.txt" "sub-agent net/http longTimeouts floor 60m"
...
seat claude/c1-iovec-mirror "$C1IOVEC_SHA" "$SP/coord-merge-c1-iovec.txt" "C1 Iovec blittable mirror at the writev/readv seam"
...
seat claude/c2-testhost-fmt "$C2FMT_SHA" "$SP/coord-merge-c2-fmt.txt" "C2 test host formatter delegates to converted fmt"
...
stamp "TRAIN 15 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"

stamp "BATTERY: converter suite + full CNR"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train15-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train15-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train15-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train15-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train15-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train15-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train15-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train15-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train15-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train15-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train15-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train15-reflect-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train15-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train15-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train15-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train15-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train15-reflectrun-$ts.log"
stamp "TRAIN 15 CHAIN DONE"
