#!/usr/bin/env bash
# coord-train19-assemble.sh -- merge train 19 on top of master fd2e618b9 (train 18 landed), then run its battery. Seats (remote tips; an unset SHA skips):
...
C1SYMD_SHA="${C1SYMD_SHA:-}"; GNET_SHA="${GNET_SHA:-}"; STALE_SHA="${STALE_SHA:-}"; C2DARWIN_SHA="${C2DARWIN_SHA:-}"; C2CLASSC_SHA="${C2CLASSC_SHA:-}"; C1DEATH_SHA="${C1DEATH_SHA:-}"; C2RLD_SHA="${C2RLD_SHA:-}"; REFRECV_SHA="${REFRECV_SHA:-}"; PARK_SHA="${PARK_SHA:-}"; D4_SHA="${D4_SHA:-}"; TESTROW_SHA="${TESTROW_SHA:-}"; GS1_SHA="${GS1_SHA:-ae444cc48}"; C2KEY_SHA="${C2KEY_SHA:-88f01638c}"; RMODEL_SHA="${RMODEL_SHA:-6dcbd7211}"; C1SYM_SHA="${C1SYM_SHA:-}"; ALIAS_SHA="${ALIAS_SHA:-}"; C2NANO_SHA="${C2NANO_SHA:-}"; C2RL2_SHA="${C2RL2_SHA:-}"; C2PROBE_SHA="${C2PROBE_SHA:-}"; GPHC_SHA="${GPHC_SHA:-204062c5a}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train19-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 19 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/c1-symbolizer-design "$C1SYMD_SHA" "$SP/coord-merge-c1-symbolizer-design.txt" "C1 PC read-back design note (docs)"
seat claude/sub-ref-receiver-displacement "$REFRECV_SHA" "$SP/coord-merge-sub-refrecv.txt" "sub-agent ref-receiver call-site seam"
seat claude/sub-goroutine-park-reason "$PARK_SHA" "$SP/coord-merge-sub-park.txt" "sub-agent goroutine park accounting (Stage A)"
seat claude/sub-testing-row "$TESTROW_SHA" "$SP/coord-merge-sub-testing-row.txt" "sub-agent testing row (host markers, refusal, Setenv fix)"
seat claude/c1-goroutine-death "$C1DEATH_SHA" "$SP/coord-merge-c1-goroutine-death.txt" "C1 stated death event for an escaping goroutine exception (after the testing-row seat, both touch the host)"
seat claude/g-net-linux-bank "$GNET_SHA" "$SP/coord-merge-g-net-linux-bank.txt" "G net Linux bank 577 + 2"
seat claude/g-bprime-s1 "$GS1_SHA" "$SP/coord-merge-g-eligibility.txt" "G B-prime eligibility fix (arc close)"
seat claude/g-phase-c-record "$GPHC_SHA" "$SP/coord-merge-g-phase-c.txt" "G Phase-C element-aliasing design record (docs)"
seat claude/sub-alias-overlap-token "$ALIAS_SHA" "$SP/coord-merge-sub-alias.txt" "sub-agent alias-overlap contract guard (both candidate mechanisms falsified; GolibTests only)"
seat claude/sub-array-range-enumerator "$D4_SHA" "$SP/coord-merge-sub-d4.txt" "sub-agent array-range copy (D4, returning with its fix; G finishes the gates)"
seat claude/reflect-cargo-inc1-m18 "$RMODEL_SHA" "$SP/coord-merge-r-cargo-model.txt" "R descriptor-cargo model increment"
seat claude/c1-symbolizer "$C1SYM_SHA" "$SP/coord-merge-c1-symbolizer.txt" "C1 PC read-back consumer 2 (stacked on the read-back seat)"
seat claude/sub-stale-converter-refusal "$STALE_SHA" "$SP/coord-merge-sub-stale.txt" "sub-agent stale-converter refusal (route #1 closure)"
seat claude/c2-darwin-classb "$C2DARWIN_SHA" "$SP/coord-merge-c2-darwin-classb.txt" "C2 darwin class-B FuncPCABI0 resolver"
seat claude/c2-darwin-classc-read "$C2CLASSC_SHA" "$SP/coord-merge-c2-classc-read.txt" "C2 darwin class-C reachability read (docs)"
seat claude/c2-darwin-runlayer-design "$C2RLD_SHA" "$SP/coord-merge-c2-runlayer-design.txt" "C2 darwin run-layer increment 1 design (docs)"
seat claude/c2-darwin-nanotime "$C2NANO_SHA" "$SP/coord-merge-c2-nanotime.txt" "C2 darwin run-layer increment 1: nanotime1 displaced onto MonotonicClock (HOLD lifted, fully gated)"
seat claude/c2-darwin-keystone "$C2KEY_SHA" "$SP/coord-merge-c2-keystone.txt" "C2 darwin run-layer keystone (libcCall displaced; golib GoLibcCall)"
seat claude/c2-darwin-runlayer-2 "$C2RL2_SHA" "$SP/coord-merge-c2-runlayer-2.txt" "C2 darwin run-layer increment 2 sizing: the keystone is one increment (docs)"
seat claude/c2-darwin-keystone-probe "$C2PROBE_SHA" "$SP/coord-merge-c2-keystone-probe.txt" "C2 darwin keystone args-recovery probe (GolibTests, 5 of 5, host-neutral)"
...
' ' ') -- ABORT"; exit 1; }
  if [ "$changed" != "0" ]; then git add src/tests/Behavioral/BehavioralTests/*Tests.cs && git commit -S -q -m "tests: re-derive the four behavioral test classes (ordinal UpdateTestTargets) after the train-19 seats
...
stamp "TRAIN 19 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train19-roster-guard-$ts.log" 2>&1; rg=$?
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/syscall-keepalive-census.ps1 > "$SP/coord-train19-keepalive-guard-$ts.log" 2>&1; kg=$?
stamp "KEEPALIVE GUARD exit=$kg :: $(tail -c 300 "$SP/coord-train19-keepalive-guard-$ts.log" | tr -d '' | tail -2 | tr '
' ' ')"
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train19-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train19-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train19-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train19-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train19-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train19-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train19-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train19-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train19-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train19-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train19-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train19-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train19-reflect-$ts.log"
stamp "BATTERY: filtered behavioral run (UdpWriteMsgAddrPort, SendtoSeam, TypedNilPtrArrayDims)"
(cd src/tests/Behavioral && for f in TypedNilPtrArrayDims TypedNilPtrArrayPositions; do powershell -NoProfile -ExecutionPolicy Bypass -File ./run-behavioral.ps1 --filter $f; echo "runner $f exit=$?"; done) > "$SP/coord-train19-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache,internal/cpu,time,internal/abi,sync,crypto/internal/alias,slices -SweepTimeout 30m > "$SP/coord-train19-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train19-sweeps-$ts.log"
stamp "BATTERY: nistec COST PAIR vs the train-11 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train19-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train19-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train19-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train19-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train19-reflectrun-$ts.log"
stamp "TRAIN 19 CHAIN DONE"
