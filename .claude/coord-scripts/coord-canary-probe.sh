...
./src/go2cs/bin/go2cs.exe -tests -test-action all -test-timeout 40m -test-config Release \
  -go2cspath "$W/src" "$GOROOT/src/${PKG:-reflect}" "$W/src/core/${PKG:-reflect}" \
  > $HOME/AppData/Local/Temp/claude/coord-drop-reflect.log 2>&1
echo "  pipeline exit: $?"
R="$W/src/core/${PKG:-reflect}/go2cs_test_comparison.json"
...
PY
cp "$W/src/core/${PKG:-reflect}/go2cs_test_results.json" "$HOME/AppData/Local/Temp/claude/coord-preserved-results-$BISECT.json" 2>/dev/null
cp "$W/src/core/${PKG:-reflect}/go2cs_test_comparison.json" "$HOME/AppData/Local/Temp/claude/coord-preserved-comparison-$BISECT.json" 2>/dev/null
echo "  records PRESERVED to distinct paths for $BISECT"
echo "=== results tail (deadline/crash statement):"
tail -c 900 "$W/src/core/${PKG:-reflect}/go2cs_test_results.json" 2>/dev/null | tr ',' '\n' | grep -iE 'timeout|action|exit status' | tail -4
