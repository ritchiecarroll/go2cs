#!/bin/bash
# Train 24 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
...
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND24 START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in ${GREC9_SHA:+claude/g-record-i1-retired} ${C2SIG_SHA:+claude/c2-darwin-signote} ${SUBQ18_SHA:+claude/sub-q18} ${SUBQ23_SHA:+claude/sub-q23} ${C1Q12_SHA:+claude/c1-q12-main-identity} ${RTRACE_SHA:+claude/r-trace-recon} ${C1Q15_SHA:+claude/c1-q15-syscall-tty} ${GI1_SHA:+claude/g-i1-samepkg-primary} ${RINC2_SHA:+${RINC2_BRANCH:-claude/reflect-cargo-inc-2}} ${SUBDOC9_SHA:+claude/sub-doc9}; do
