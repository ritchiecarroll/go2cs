#!/usr/bin/env bash
# coord-train27-relegs.sh -- re-run the three train-27 battery legs the disk preflight refused (sweeps, nistec pair, reflect run) on the assembled head; stamps into the same assembly log.
set -u
...
stamp "TRAIN 27 RELEGS START head=$(git rev-parse --short=9 HEAD) clean=$(git status --porcelain | wc -l) free=$(df -h /c | tail -1 | awk '{print $4}')"
