#!/bin/bash
# Train 15 landing: marker scan over the merge range, push HEAD:master fast-forward, verify, prune seated branches, reset the nistec control.
set -u
cd /c/Projects/go2cs/.claude/worktrees/musing-moser-d4552c || exit 1
stamp(){ echo "[$(date '+%F %T')] $*"; }
head=$(git rev-parse --short HEAD); stamp "LAND START head=$head dirty=$(git status --porcelain | wc -l)"
...
for b in claude/c2-unix-abstract-addr claude/sub-roster-denominator-210 claude/sub-nethttp-timeout-floor claude/c1-iovec-mirror claude/sub-orphaned-comments claude/sub-os-row; do
