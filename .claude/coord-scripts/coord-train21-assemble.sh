#!/usr/bin/env bash
# coord-train21-assemble.sh -- merge train 21 on top of the train-20 landed master (set CONTROL_SHA to it), then run its battery. Seats (remote tips; an unset SHA skips):
...
GRELAND_SHA="${GRELAND_SHA:-dc24a21c3}"; C2DARWINDOC_SHA="${C2DARWINDOC_SHA:-c7bd9f9048}"; GRECORD_SHA="${GRECORD_SHA:-06c241047}"; RR1_SHA="${RR1_SHA:-}"; C2VEND_SHA="${C2VEND_SHA:-aba54e39f2}"
ts=$(date +%Y%m%d-%H%M%S)
log="$SP/coord-train21-merge-$ts.log"
stamp() { echo "[$(date '+%F %T')] $*" | tee -a "$log"; }
stamp "TRAIN 21 ASSEMBLE START head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
...
seat claude/g-array-range-reland "$GRELAND_SHA" "$SP/coord-merge-g-array-range-reland.txt" "G array-range re-landing (revert-the-revert + cherry-pick of 3067aeff5; patch-equivalence proof verified before seating)"
seat claude/c2-darwin-first-run "$C2DARWINDOC_SHA" "$SP/coord-merge-c2-darwin-finding.txt" "C2 FINDING-darwin-run-layer sections 7 + 8: the run layer first light and the full census (docs)"
seat claude/reflect-cargo-r1 "$RR1_SHA" "$SP/coord-merge-r-cargo-r1.txt" "R descriptor-cargo R1: abiSeq.regAssign displaced for its two unseeable arms (seat CONDITION: BROKEN {} on reflect vs train 20)"
seat claude/c2-vendored-alias-overlap "$C2VEND_SHA" "$SP/coord-merge-c2-vendored-alias.txt" "C2 vendored x/crypto alias AnyOverlap twin + the GOROOT-vendored registry lookup fix (aba54e39f2)"
seat claude/g-three-capability-record "$GRECORD_SHA" "$SP/coord-merge-g-zh-box-record.txt" "G three-capability zh-box/defer-frame/cross-package design record (docs, 06c241047)"
...
' ' ') -- ABORT"; exit 1; }
  if [ "$changed" != "0" ]; then git add src/tests/Behavioral/BehavioralTests/*Tests.cs && git commit -S -q -m "tests: re-derive the four behavioral test classes (ordinal UpdateTestTargets) after the train-21 seats
...
stamp "TRAIN 21 ASSEMBLED head=$(git rev-parse --short HEAD) clean=$(git status --porcelain | wc -l)"
else
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/check-roster-format.ps1 > "$SP/coord-train21-roster-guard-$ts.log" 2>&1; rg=$?
...
powershell -NoProfile -ExecutionPolicy Bypass -File src/syscall-keepalive-census.ps1 > "$SP/coord-train21-keepalive-guard-$ts.log" 2>&1; kg=$?
stamp "KEEPALIVE GUARD exit=$kg :: $(tail -c 300 "$SP/coord-train21-keepalive-guard-$ts.log" | tr -d '' | tail -2 | tr '
' ' ')"
stamp "ROSTER GUARD exit=$rg ($(grep -aciE 'FAIL|violation' "$SP/coord-train21-roster-guard-$ts.log") fail lines)"
...
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" > "$SP/coord-train21-suite-cnr-$ts.log" 2>&1; echo "suite+cnr exit=$?" >> "$SP/coord-train21-suite-cnr-$ts.log"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "\$env:DOTNET_ROOT='$HOME\dotnet10'; \$env:PATH=\"\$env:DOTNET_ROOT;\$env:PATH\"; \$env:MSBUILDDISABLENODEREUSE='1'; Set-Location '$(pwd -W)/src/core/syscall'; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue; dotnet build syscall.csproj -c Debug -p:GoTargetOS=linux -p:UseSharedCompilation=false -clp:ErrorsOnly 2>&1 | Select-String -Pattern 'error (CS|MSB|NETSDK)[0-9]+|Build succeeded|error\(s\)' | Select-Object -First 10; Remove-Item -Recurse -Force bin,obj -ErrorAction SilentlyContinue" > "$SP/coord-train21-syscall-linux-$ts.log" 2>&1; echo "syscall-linux exit=$?" >> "$SP/coord-train21-syscall-linux-$ts.log"
stamp "BATTERY: slnx (the behavioral COMPILE gate)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-slnx-build.ps1" > "$SP/coord-train21-slnx-$ts.log" 2>&1; echo "slnx exit=$?" >> "$SP/coord-train21-slnx-$ts.log"
stamp "BATTERY: GolibTests --no-build + each-class-alone"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests.ps1" -NoBuild > "$SP/coord-train21-golibtests-$ts.log" 2>&1; echo "golibtests exit=$?" >> "$SP/coord-train21-golibtests-$ts.log"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-golibtests-alone.ps1" > "$SP/coord-train21-alone-$ts.log" 2>&1; echo "alone exit=$?" >> "$SP/coord-train21-alone-$ts.log"
stamp "BATTERY: reflect -tests build"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-tests-build.ps1" > "$SP/coord-train21-reflect-$ts.log" 2>&1; echo "reflect exit=$?" >> "$SP/coord-train21-reflect-$ts.log"
stamp "BATTERY: filtered behavioral run (UdpWriteMsgAddrPort, SendtoSeam, TypedNilPtrArrayDims)"
(cd src/tests/Behavioral && for f in TypedNilPtrArrayDims TypedNilPtrArrayPositions; do powershell -NoProfile -ExecutionPolicy Bypass -File ./run-behavioral.ps1 --filter $f; echo "runner $f exit=$?"; done) > "$SP/coord-train21-runner-$ts.log" 2>&1; git checkout HEAD -- src/tests/Behavioral 2>/dev/null
stamp "BATTERY: sweeps"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-union-battery.ps1" -SkipSuite -SkipCNR -Sweeps net/http,errors,math/bits,math/big,crypto/rsa,crypto/x509,crypto/tls,encoding/json,encoding/xml,go/types,unicode/utf8,sort,strings,os/exec,crypto/md5,crypto/internal/boring/bcache,internal/cpu,time,internal/abi,sync,crypto/internal/alias,slices -SweepTimeout 30m > "$SP/coord-train21-sweeps-$ts.log" 2>&1; echo "sweeps exit=$?" >> "$SP/coord-train21-sweeps-$ts.log"
stamp "BATTERY: nistec COST PAIR vs the train-20 head (control worktree must be at that head)"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-nistec-pair.ps1" -Main "$(pwd -W)" -ControlSha "${CONTROL_SHA:-origin/master}" -SkipWarmup -Rounds 2 > "$SP/coord-train21-nistec-pair-$ts.log" 2>&1; echo "pair exit=$?" >> "$SP/coord-train21-nistec-pair-$ts.log"; stamp "PAIR: $(grep -E 'mean' "$SP/coord-train21-nistec-pair-$ts.log" | cut -c23-120 | tr '
' ';')"
stamp "BATTERY: reflect RUN"
powershell -NoProfile -ExecutionPolicy Bypass -File "$SP/coord-reflect-run.ps1" -Package reflect -Timeout 30m -Label coord-train11 > "$SP/coord-train21-reflectrun-$ts.log" 2>&1; echo "reflectrun exit=$?" >> "$SP/coord-train21-reflectrun-$ts.log"
stamp "TRAIN 21 CHAIN DONE"
