...
if [ "$G11CONV" -ge 1 ]; then
  ...
  if [ "$G11GEN" -ge 1 ]; then
    ...
    if [ "${OWED_GEN:-0}" = "0" ]; then
      stamp "G11(b) src/gen JUSTIFICATION :: ZERO files under src/gen in this train's delta, and the OWED VECTOR reads gen=$OWED_GEN (no merged row's class owes src/gen) -- the EXPECTED reading for THIS train, stated as a reading.  (An earlier derive's 'seat 2 is a src/gen seat' premise was train 46's; per (L7) every justification derives from the OWED vector, never from a seat number.)"
    else
