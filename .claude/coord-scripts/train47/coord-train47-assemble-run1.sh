...
#     converter-corpus-metadata  DERIVED 2026-09-13 for row 8: converter source + the corpus metadata a
...
#     placeholder.  This train's table is FILLED -- SIXTEEN rows named, ZERO awaiting an announced SHA --
#     and with a filled table the cheap mistake changes direction: the dangerous run is no longer "the
#     template refused", it is "a launch quietly assembled a subset of the sixteen rows and every count in the
...
if [ "${TRAIN47_REQUIRE_ALL+x}" = "x" ]; then
  ...
  case "$TRAIN47_REQUIRE_ALL" in
    ...
    0) REQUIRE_ALL=0
       stamp "TRAIN47_REQUIRE_ALL=0 :: stated explicitly.  ⚠ THIS IS NOT A PERMISSION TO SKIP: a PENDING row still ABORTS unless TRAIN47_SKIP_PENDING=1 is ALSO given, because with a FILLED table the dangerous run is the one that quietly assembles a subset of the sixteen rows" ;;
...
# ⚠ RE-FILLED 2026-09-13 (SIXTEEN ROWS).  THE ROW ORDER **IS** THE MERGE ORDER, and this train's
#   ruled order is  1 2 3 4 9 10 7 6 8 5 11 12 13 14 15 16  -- those are the COORDINATOR'S seat
#   numbers and the TABLE is written in merge order, so row 5 reads `9|...` and so on.  Numbering
#   1..N contiguous is what the structural check asserts, so the coordinator's seat numbers are
#   carried in the NOTE column of each row's comment below and never in the first field.
...
#      8   seat 6   claude/g-unfreeze-handown-recut          converter-corpus-metadata
#      9   seat 8   claude/g-generic-alias-recut             converter-guard-rebaseline
#     10   seat 5   claude/c2-census-reader                  golib-converter-docs
#     11   seat 11  claude/laneR-h5-s15-rungs                docs
#     12   seat 12  claude/coord-glossary-kickoff            docs
#     13   seat 13  claude/g-census-2026-09-13               docs
#     14   seat 14  claude/c1-crashwhiletracing-marking       manifest
#     15   seat 15  claude/c1-getcallerpc-erratum             docs
#     16   seat 16  claude/c1-lockosthread-body               golib-corpus-handown
#   ⚠ ROW 8 IS **FILLED BY G'S RE-CUT (2026-09-13): claude/g-unfreeze-handown-recut @ce2d9d082**, and
#     that is a REPLACEMENT of the row's former occupant rather than its return.  The row held
...
#     coord-train47-resolutions/seat8-stale-7078dbada/, which merge_seat's lookup
...
#   ⚠ ROW 9 FILLS THE FORMER PENDING ROW with claude/g-generic-alias-recut @449ecce7a, and its class
...
#   ⚠ ROWS 12 and 13 are docs-only (docs/Glossary.md; docs/phase4/CENSUS-g-laptop-2026-09-13.md),
#     measured, and meet the `docs` shape.
#   ⚠ ROWS 14, 15 AND 16 FILL THE LAST THREE PENDING SLOTS (2026-09-13) with C1's three cherry-picks,
...
#       * row 14  claude/c1-crashwhiletracing-marking @d781b0251 -- `manifest`.  Measured diff is
...
#       * row 15  claude/c1-getcallerpc-erratum @3ca63093d -- `docs`.  Measured diff is EXACTLY ONE
...
#       * row 16  claude/c1-lockosthread-body @dc34e4b4a -- `golib-corpus-handown`.  Measured diff is
...
#     and none of the three diffs touches src/tests at all (row 14's and row 15's are single-path; row
#     16's three paths are two src/core/runtime and one docs/phase4).  A7's STRICT form applies to all
#     three, which is the correct and tighter gate.
#   ⚠ ROW 11 IS FILLED BY **claude/laneR-h5-s15-rungs @ff40eee3a** (2026-09-13), ONE commit on
...
#   ⚠ ROWS 9, 12 AND 13 CARRY THEIR SEAT-CONTENT ARMS (written 2026-09-13; A-row9, A-row12,
#     A-row13 in the fenced block below), AND ROWS 14, 15 AND 16 NOW CARRY THEIRS TOO (A-row14,
#     A-row15, A-row16, written the same day and controlled the same way).  ⚠ **AND ROWS 8 AND 11 NOW
#     CARRY THEIRS (2026-09-13, at seat fill): A-row8 was REWRITTEN from the ground up against the
#     RE-CUT's real diff -- its predecessor asserted the OLD pin's content and would have read as a
#     green over a seat it was never written for -- and A-row11 is new.  The block therefore holds
#     SIXTEEN arms for SIXTEEN filled rows, and there is no row left whose arm is owed.**
...
#     half-merge controls, row 9's emission-blob against the base's GOLDEN blob (PAIR -> 0) and row
#     12's kickoff block under a reverted count sentence (SIX -> 0, FIVE -> 1).  A gate never made to
#     fail proves nothing, and these three have each been made to fail.
#   ⚠ AND SO WERE A-row14/15/16, BY THE SAME METHOD AND WITH THE SAME LITERALS (2026-09-13).  Every
...
#       * row 14: AMENDMENT-marker 1 -> 0, the 1-6/7-8 marking sentence 1 -> 0, the sibling door
...
#       * row 15: the `## 8. ERRATUM` header 1 -> 0 and all three defect sentences 1 -> 0.  ⚠ THIS ROW
...
#       * row 16: every rewritten body reads 0 at both bases (lockedExt++ 1 -> 0, lockedExt-- 2 -> 0,
...
#   ⚠ AND SO WERE A-row8 AND A-row11 (2026-09-13), EVALUATED AT THREE REFS -- the seat's parent
#     654343a5e, the seat's TIP, and LANDED MASTER a02ac3df3 (this run's base) -- so a predicate that
#     would pass on a tree its row was subtracted from is caught by the base column rather than assumed
#     away.  The readings, parent / tip / master:
#       * row 8: the new test file's AGPL SPDX line 0/1/0 and its section-7 output-exception notice
...
#       * row 11: the `## 2026-09-13 — §15.` heading 0/1/0; the lineage-A ladder `10 → 8 → 6 → 24`
...
#         PREDICATE IS INVERTED IN THE SAME WAY ROW 15's IS: §14's own distinctive closing sentence
...
#       * seat 6 (`converter`) carries TWO `src/core/**/README.md`, which `converter`'s shape refuses
...
SEAT_TABLE="1|claude/coord-orphan-disclosure-check|36cbef240|converter-test+docs|tip
2|claude/coord-stamp-guard|ec1fe2745|converter-test+docs|tip
3|claude/laneR-armc-guard|bbd0afe43|converter-test+docs|tip
...
8|claude/g-unfreeze-handown-recut|ce2d9d082|converter-corpus-metadata|tip
9|claude/g-generic-alias-recut|449ecce7a|converter-guard-rebaseline|tip|allowed=^src/tests/Behavioral/CollidingPackageNames/(duprenamed/)?[a-z_]+\.[cg][so](\.target)?$
10|claude/c2-census-reader|44ab61dad|golib-converter-docs|tip
11|claude/laneR-h5-s15-rungs|ff40eee3a|docs|tip
12|claude/coord-glossary-kickoff|ff9d0fb47|docs|tip
13|claude/g-census-2026-09-13|748beefbb|docs|tip
14|claude/c1-crashwhiletracing-marking|d781b0251|manifest|tip
15|claude/c1-getcallerpc-erratum|3ca63093d|docs|tip
16|claude/c1-lockosthread-body|dc34e4b4a|golib-corpus-handown|tip"
...
# ⚠ SIXTEEN ROWS IN THIS TRAIN, and the variables are named by ROW NUMBER (== merge position), which
...
merge_seat(){ # $1 branch, $2 required sha (prefix ok), $3 seat number, $4 class, $5 tipmode
  ...
  case "$cls" in
    ...
    # guard (row 9: one converter source plus seven MODIFIED files under one existing project, zero
    ...
    # row 9 as well, which is the shape doctrine forbids, so the admission is a SEPARATE arm: this one
...
# --- row 8 :: claude/g-unfreeze-handown-recut -- ⚠ **THE SILENT-SUBTRACTION ASSERTION, REWRITTEN
#     2026-09-13 FOR THE RE-CUT.**  ⚠ ITS PREDECESSOR ASSERTED THE OLD PIN'S CONTENT (7078dbada's
#     BaseOutputPath deferral, its README artwork attribution) AND WAS DISCARDED RATHER THAN AMENDED:
#     an arm carried over from a pin that never boards reads as a green about a seat it was never
#     written for, which is the same class of mistake as a saved resolution applied over a new base.
#     Every predicate below is re-measured against `git show` of the RE-CUT's own six commits.
#     THE HALF-MERGE THIS ARM EXISTS TO CATCH.  master carries a LICENSING change over these same
#     metadata files, and this row re-mints four .csproj again.  In internal.godebug.csproj the two
#     changes land in ADJACENT ItemGroups at the tail of one file: master's `<None Include="../../LICENSE"`
#     / `PackageLicenseFile` pair, and this row's `<ItemGroup Label="GoHandOwnReferences">` holding the
#     runtime.csproj reference the derivation cannot produce.  Git merges adjacent inserts
#     independently and reports a clean merge either way; a union that kept only one half COMPILES
#     (or packs) and is wrong.  So BOTH halves are read, by CONTENT, at the union -- the seat's half
#     asserted PRESENT, master's asserted STILL PRESENT -- and neither is inferred from the other.
#     ⚠ THE RUNTIME REFERENCE IS ASSERTED TWICE ON PURPOSE: once INSIDE the labelled block and once as
#     the file's WHOLE-FILE count.  In-block 1 with whole-file 1 is the reference MOVED; in-block 1
#     with whole-file 2 is the derived copy surviving beside the declared one, which is a duplicate
#     ProjectReference and exactly what a three-way merge of this hunk produces when it takes both.
if [ "$SEAT8_SHA" != "PENDING" ]; then
  S8TF='src/go2cs/handOwnReferences_test.go'; S8PW='src/go2cs/projectFileWriter.go'
  S8PJ='src/go2cs/go2cs-src.projitems'; S8GD='src/core/internal/godebug/internal.godebug.csproj'
  S8PIS='src/core/crypto/internal/boring/bcache/package_info.cs src/core/internal/concurrent/package_info.cs src/core/internal/godebug/package_info.cs src/core/internal/weak/package_info.cs'
  S8OK=1
  # shellcheck disable=SC2086
  for f in "$S8TF" "$S8PW" "$S8PJ" "$S8GD" $S8PIS; do
    [ -f "$f" ] || { stamp "  ^ A-row8 REFUSED: $f is not in the assembled tree"; S8OK=0; }
  done
  # (i) THE NEW GUARD FILE, AND ITS HEADER READ THE WAY THE LICENSING GUARD READS IT.
  #     src/go2cs/licensing_test.go requires BOTH lines inside the first 600 bytes of every converter
  #     source, so this arm reads the same window rather than the whole file -- a notice further down
  #     satisfies a grep and fails the guard.  The MIT predicate is the INVERTED half: the seat's own
  #     intermediate commit 4e0173359 carried the retired `MIT-style license` header and ce2d9d082
  #     replaced it, so a merge resolved to the seat's earlier side is refused here and nowhere else.
  S8HDR=$(head -c 600 "$S8TF" 2>/dev/null || true)
  S8AGPL=$(printf '%s' "$S8HDR" | grep -acF -- '// SPDX-License-Identifier: AGPL-3.0-only' || true)
  S8EXC=$(printf '%s' "$S8HDR" | grep -acF -- '// Additional permission for emitted output: see LICENSE-EXCEPTION (AGPL section 7).' || true)
  S8MIT=$(grep -acF -- 'MIT-style license' "$S8TF" 2>/dev/null || true)
  S8REG=$(grep -acF -- 'handOwnReferences_test.go' "$S8PJ" 2>/dev/null || true)
  # (ii) THE CONVERTER MECHANISM: the function AND the call site.  A preserved-block helper that
  #      nothing calls is a csproj re-mint that still drops the block, and it compiles and tests green.
  S8FNDEF=$(grep -acE '^func preserveHandOwnReferences\(' "$S8PW" 2>/dev/null || true)
  S8FNCALL=$(grep -acF -- 'newContents = preserveHandOwnReferences(projectFileName, newContents)' "$S8PW" 2>/dev/null || true)
  # (iii) THE TWO ADJACENT ITEMGROUPS -- THIS ROW's block and MASTER's licence items -- AT THE UNION.
  S8GHOR=$(grep -acF -- 'Label="GoHandOwnReferences"' "$S8GD" 2>/dev/null || true)
  S8GHORRT=$(awk '/Label="GoHandOwnReferences"/{f=1;next} f&&/<\/ItemGroup>/{f=0} f&&/core\/runtime\/runtime\.csproj/{c++} END{print c+0}' "$S8GD" 2>/dev/null || true)
  S8RTALL=$(grep -acF -- 'core/runtime/runtime.csproj' "$S8GD" 2>/dev/null || true)
  S8LICF=$(grep -acF -- 'PackageLicenseFile' "$S8GD" 2>/dev/null || true)
  S8LICI=$(grep -acF -- '<None Include="../../LICENSE"' "$S8GD" 2>/dev/null || true)
  # (iv) THE FOUR package_info.cs METADATA EDITS, read by what the UN-FREEZE produces rather than by
  #      the file being modified: a re-minted metadata file carries the forced-import block, and the
  #      ELEVEN forced initializers across the four are the un-freeze's whole observable output here.
  S8II=0; S8INIT=0
  for f in $S8PIS; do
    [ "$(grep -acF -- '<ImportInitializers>' "$f" 2>/dev/null || true)" -ge 1 ] && S8II=$(( S8II + 1 ))
    S8INIT=$(( S8INIT + $(grep -acF -- 'builtin.initPackage(typeof(' "$f" 2>/dev/null || true) ))
  done
  # (v) THE FOOTPRINT, ASSERTED FROM THE SEAT'S OWN TIP so it can detect ADDITION as well as
  #     subtraction (a union-side pathspec read can only ever miss files, never see extra ones).
  S8SEATFILES=$(git diff --name-only "$(git merge-base "$BASE" "$SEAT8_SHA")" "$SEAT8_SHA" | grep -ac . || true)
  S8META=$(git diff --name-only "$BASE" HEAD -- src/core | grep -ac . || true)
  S8CONV=$(git diff --name-only "$BASE" HEAD -- src/go2cs | grep -ac . || true)
  stamp "A-row8 hand-own metadata unfreeze, BOTH HALVES AT THE UNION :: (the RE-CUT ce2d9d082) the eight files present (failures=$S8OK, 1 means clean) :: THE GUARD FILE -- AGPL SPDX in the first 600 bytes x$S8AGPL (must be 1) section-7 output-exception notice x$S8EXC (must be 1) retired 'MIT-style license' x$S8MIT (must be 0 -- ⚠ INVERTED: the seat's own 4e0173359 reads 1, so a header hunk resolved to the seat's earlier side is refused here) projitems registration x$S8REG (must be 1) :: THE CONVERTER MECHANISM -- preserveHandOwnReferences defined x$S8FNDEF (must be 1) AND CALLED from the re-mint path x$S8FNCALL (must be 1; a helper nothing calls drops the block and tests green) :: THIS ROW's ItemGroup -- Label=\"GoHandOwnReferences\" x$S8GHOR (must be 1) with runtime.csproj INSIDE it x$S8GHORRT (must be 1) and whole-file x$S8RTALL (must be 1 -- 2 is the derived copy surviving beside the declared one) :: MASTER's ItemGroup STILL THERE -- PackageLicenseFile x$S8LICF (must be 1) <None Include=\"../../LICENSE\"> x$S8LICI (must be 1) :: THE FOUR package_info.cs -- carrying <ImportInitializers>=$S8II of 4, forced initPackage calls=$S8INIT (must be 11) :: SEAT-OWN footprint=$S8SEATFILES file(s) (the prediction is 17) :: corpus files in the union delta=$S8META (the prediction is 10: 4 package_info.cs + 4 .csproj + 2 README.md) :: converter files in the union delta=$S8CONV (the prediction is 7)"
  { [ "$S8OK" = "1" ] && [ "${S8AGPL:-0}" = "1" ] && [ "${S8EXC:-0}" = "1" ] && [ "${S8MIT:-1}" = "0" ] \
    && [ "${S8REG:-0}" = "1" ] && [ "${S8FNDEF:-0}" = "1" ] && [ "${S8FNCALL:-0}" = "1" ] \
    && [ "${S8GHOR:-0}" = "1" ] && [ "${S8GHORRT:-0}" = "1" ] && [ "${S8RTALL:-0}" = "1" ] \
    && [ "${S8LICF:-0}" = "1" ] && [ "${S8LICI:-0}" = "1" ] && [ "$S8II" = "4" ] && [ "$S8INIT" = "11" ]; } || {
    stamp "  ^ A-row8 REFUSED: one of the two halves is not at the union.  ⚠ READ WHICH: a missing GoHandOwnReferences block, an in-block runtime reference of 0, a preserveHandOwnReferences definition or CALL SITE of 0, an ImportInitializers count below four or a forced-init total below eleven means THIS ROW's half was subtracted by the merge; a PackageLicenseFile or LICENSE <None> of 0 means MASTER's was.  The two live in ADJACENT ItemGroups at the tail of one file, so git reports a clean merge either way and no ancestry check can see it.  A whole-file runtime.csproj count of 2 is the OPPOSITE resolution -- both sides taken -- and is a duplicate ProjectReference.  An AGPL/section-7 count of 0 with 'MIT-style license' at 1 means the merge kept the seat's OWN earlier header and src/go2cs's licensing guard will red on it."
    fail_gate A-row8
  }
  [ "${S8SEATFILES:-0}" = "17" ] || stamp "      ⚠ A-row8 SEAT-OWN footprint is $S8SEATFILES file(s) where the measured prediction is 17.  DERIVED FROM THE SEAT'S OWN TIP, so it sees a file ADDED to the re-cut as well as one removed -- a stamped MISS to reconcile against the branch, not a refusal."
  [ "${S8META:-0}" = "10" ] || stamp "      ⚠ A-row8 corpus footprint is $S8META where THIS ROW's own diff predicts 10.  ⚠ NOT A REFUSAL AND NOT A DEFECT ON ITS OWN: rows 4, 10, 14 and 16 also land under src/core, so the union legitimately reads more than this row's own ten (a full sixteen-seat rehearsal at base a02ac3df3 read 15).  It is stamped so the number is reconciled against the rows' diffs rather than assumed -- the ADDITION-DETECTING read for THIS row is the SEAT-OWN footprint above."
  [ "${S8CONV:-0}" = "7" ] || stamp "      ⚠ A-row8 converter footprint is $S8CONV where THIS ROW's own diff predicts 7.  ⚠ NOT A REFUSAL AND NOT A DEFECT ON ITS OWN: rows 1, 2, 3, 9 and 10 also land under src/go2cs, so the union legitimately reads more than this row's own seven (a full sixteen-seat rehearsal at base a02ac3df3 read 17).  It is stamped so the number is reconciled against the rows' diffs rather than assumed."
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 9 :: claude/g-generic-alias-recut -- ⚠ **A CONVERTER FIX AND ITS RE-BASELINED GOLDEN.**
...
if [ "$SEAT9_SHA" != "PENDING" ]; then
  S9CPN='src/tests/Behavioral/CollidingPackageNames'
  S9OK=1
  for f in src/go2cs/typeNameResolution.go "$S9CPN/main.go" "$S9CPN/main.cs" "$S9CPN/main.cs.target" "$S9CPN/package_info.cs" "$S9CPN/duprenamed/lib.go" "$S9CPN/duprenamed/lib.cs" "$S9CPN/duprenamed/package_info.cs"; do
  ...
  S9LOOK=$(grep -acF -- 'v.importPathAliases[pkg.Path()]' src/go2cs/typeNameResolution.go || true)
  S9QUAL=$(grep -acF -- 'fmt.Sprintf("%s.%s[%s]", aliasQualifier' src/go2cs/typeNameResolution.go || true)
  S9CONV=$(git diff --numstat "$BASE" HEAD -- src/go2cs/typeNameResolution.go | cut -f1)
  case "$S9CONV" in ''|*[!0-9]*) S9CONV=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  ...
  S9GEN=$(grep -acE '^type Box\[T any\] struct' "$S9CPN/duprenamed/lib.go" 2>/dev/null || true)
  S9REF=$(grep -acE 'var b dupmeta\.Box\[string\]' "$S9CPN/main.go" 2>/dev/null || true)   # the DECLARATION, not the prose: the file's own comment names the spelling three times
  S9EMIT=$(grep -acF -- 'dupmeta.Box<@string>' "$S9CPN/main.cs" 2>/dev/null || true)
  S9WRONG=$(grep -acF -- 'dup.Box<@string>' "$S9CPN/main.cs" 2>/dev/null || true)
  S9ACC=$(grep -acF -- 'public partial struct Box<T> {}' "$S9CPN/duprenamed/package_info.cs" 2>/dev/null || true)
  ...
  S9BE=$(git rev-parse "HEAD:$S9CPN/main.cs" 2>/dev/null || true)
  S9BG=$(git rev-parse "HEAD:$S9CPN/main.cs.target" 2>/dev/null || true)
  S9PAIR=0; { [ -n "$S9BE" ] && [ "$S9BE" = "$S9BG" ]; } && S9PAIR=1
  S9BEH=$(git diff --name-only "$BASE" HEAD -- "$S9CPN" | grep -ac . || true)
  stamp "A-row9 generic cross-package alias qualifier, BOTH HALVES AT THE UNION :: CONVERTER half -- the path-keyed alias lookup x$S9LOOK (must be 2: the pre-existing NON-generic arm and THIS ROW's generic one; 1 is master alone and is this row subtracted), the generic arm rendering THROUGH the resolved qualifier x$S9QUAL (must be 1; the base reads 0), typeNameResolution.go additions=$S9CONV (must be > 0) :: BEHAVIORAL half -- the generic type declared in the ALIASED package x$S9GEN (must be 1), the Go DECLARATION that references it x$S9REF (must be 1), the emission qualified by the file's ALIAS x$S9EMIT (must be 1) and by the declared package NAME x$S9WRONG (must be 0 -- that spelling IS the CS0426) :: the TypeAccessibility registration x$S9ACC (must be 1) :: GOLDEN/EMISSION BLOBS EQUAL=$S9PAIR (must be 1; emission=$S9BE golden=$S9BG) :: behavioral files in the delta=$S9BEH (the prediction is 7)"
  { [ "$S9OK" = "1" ] && [ "${S9LOOK:-0}" = "2" ] && [ "${S9QUAL:-0}" = "1" ] && [ "$S9CONV" -ge 1 ] \
    && [ "${S9GEN:-0}" = "1" ] && [ "${S9REF:-0}" = "1" ] && [ "${S9EMIT:-0}" = "1" ] && [ "${S9WRONG:-1}" = "0" ] \
    && [ "${S9ACC:-0}" = "1" ] && [ "$S9PAIR" = "1" ]; } || {
    stamp "  ^ A-row9 REFUSED: one of the two halves is not at the union.  ⚠ READ WHICH: a missing alias lookup means the CONVERTER half was subtracted and the re-baselined golden now records an emission this converter cannot produce; a missing Box declaration, reference or registration means the BEHAVIORAL half was; EMISSION/GOLDEN BLOBS UNEQUAL means the merge kept one side of the re-baseline and dropped the other, which compiles, runs, and re-baselines nothing."
    fail_gate A-row9
  }
  [ "${S9BEH:-0}" = "7" ] || stamp "      ⚠ A-row9 behavioral footprint is $S9BEH where the ruled prediction is 7.  DERIVED, so this is a stamped MISS to reconcile against the row's own diff -- not a refusal, and not an expectation to adjust after the fact."
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 10 :: claude/c2-census-reader -- golib + GolibTests + the probe tree, all three halves.
if [ "$SEAT10_SHA" != "PENDING" ]; then
  ...
  for f in src/core/golib/Q44RegistryCensus.cs src/tests/GolibTests/Q44RegistryCensusControlTests.cs docs/phase4/probes/c2-census-read/Read-Q44Census.ps1; do
    [ -f "$f" ] || { stamp "  ^ A-row10 REFUSED: $f is not in the assembled tree"; S10OK=0; }
  done
  S10G=$(git diff --numstat "$BASE" HEAD -- src/core/golib/Q44RegistryCensus.cs | cut -f1)
  S10T=$(git diff --numstat "$BASE" HEAD -- src/tests/GolibTests/Q44RegistryCensusControlTests.cs | cut -f1)
  case "$S10G" in ''|*[!0-9]*) S10G=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  case "$S10T" in ''|*[!0-9]*) S10T=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  S10P=$(git diff --name-only --diff-filter=A "$BASE" HEAD -- docs/phase4/probes | grep -ac . || true)
  stamp "A-row10 census reader :: the three files present (failures=$S10OK, 1 means clean) :: golib census additions=$S10G (must be > 0) :: its GolibTests control additions=$S10T (must be > 0 -- a census whose control did not move is a reader nobody proved) :: probe-tree files ADDED=$S10P (must be >= 1)"
  { [ "$S10OK" = "1" ] && [ "$S10G" -ge 1 ] && [ "$S10T" -ge 1 ] && [ "${S10P:-0}" -ge 1 ]; } || fail_gate A-row10
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 11 :: claude/laneR-h5-s15-rungs -- ⚠ **A PURE APPEND TO A RECORD, SO THE ARM ASSERTS THE
...
#     it cannot reconcile.
#     ⚠ AND THE APPEND-NESS IS ITSELF ASSERTED, INVERTED, exactly as row 15's is: this record's
...
if [ "$SEAT11_SHA" != "PENDING" ]; then
  S11P='docs/phase4/REHEARSAL-h5-go124.md'
  S11OK=1; [ -f "$S11P" ] || { stamp "  ^ A-row11 REFUSED: $S11P is not in the assembled tree"; S11OK=0; }
  S11SEC=$(grep -acF -- '## 2026-09-13 — §15.' "$S11P" 2>/dev/null || true)
  S11LA=$(grep -acF -- '10 → 8 → 6 → 24' "$S11P" 2>/dev/null || true)
  S11LAB=$(grep -acF -- '240 → 4 → 126 → 68 → 10 → 8 → 6 → 24' "$S11P" 2>/dev/null || true)
  S11LB=$(grep -acF -- '120 → 7 → 5 → 2' "$S11P" 2>/dev/null || true)
  S11RUNG=$(grep -acF -- '8a1b7e71c` — 12 / 12 / 12' "$S11P" 2>/dev/null || true)
  S11NM=$(grep -acF -- 'NOT MEASURED' "$S11P" 2>/dev/null || true)
  ...
  S11S14=$(grep -acF -- 'Scratch `h5b` now carries C1' "$S11P" 2>/dev/null || true)
  S11NS=$(git diff --numstat "$BASE" HEAD -- "$S11P"); S11ADD=$(printf '%s' "$S11NS" | cut -f1); S11DEL=$(printf '%s' "$S11NS" | cut -f2)
  case "$S11ADD" in ''|*[!0-9]*) S11ADD=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  case "$S11DEL" in ''|*[!0-9]*) S11DEL=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  # THE SEAT'S OWN DELTA, from its TIP, so an EXTRA file on the branch is seen as well as a lost one.
  S11SEATNS=$(git diff --numstat "$(git merge-base "$BASE" "$SEAT11_SHA")" "$SEAT11_SHA")
  S11SEATFILES=$(printf '%s\n' "$S11SEATNS" | grep -ac . || true)
  S11SEATADD=$(printf '%s' "$S11SEATNS" | head -1 | cut -f1); S11SEATDEL=$(printf '%s' "$S11SEATNS" | head -1 | cut -f2)
  case "$S11SEATADD" in ''|*[!0-9]*) S11SEATADD=0 ;; esac
  case "$S11SEATDEL" in ''|*[!0-9]*) S11SEATDEL=0 ;; esac
  S11DELTA=$(git diff --name-only "$BASE" HEAD -- "$S11P" | grep -ac . || true)
  stamp "A-row11 REHEARSAL-h5-go124 §15, THE NEW BLOCK AND THE OLD SECTIONS TOGETHER :: file present=$S11OK :: the §15 heading x$S11SEC (must be 1) :: LINEAGE A's ladder 10 → 8 → 6 → 24 x$S11LA (must be >= 1) and its full label 240 → 4 → 126 → 68 → 10 → 8 → 6 → 24 x$S11LAB (must be 1) :: LINEAGE B's ladder 120 → 7 → 5 → 2 x$S11LB (must be >= 1) :: the landed-master rung, SHA and figures ON ONE LINE (8a1b7e71c — 12 / 12 / 12) x$S11RUNG (must be 1) :: NOT MEASURED markers x$S11NM (must be >= 2 -- a record that reconciled what the posts do not is a different document) :: §14 UNCHANGED -- its own closing sentence x$S11S14 (must be exactly 1; ⚠ INVERTED, and a heading regex reads 2 here because §15's heading names §14) :: union numstat +${S11ADD}/-${S11DEL} (deletions must be 0: a PURE APPEND) :: in this train's delta=$S11DELTA (must be 1) :: SEAT-OWN delta=$S11SEATFILES file(s) at +${S11SEATADD}/-${S11SEATDEL} (the measured prediction is 1 file at +250/-0)"
  { [ "$S11OK" = "1" ] && [ "${S11SEC:-0}" = "1" ] && [ "${S11LA:-0}" -ge 1 ] && [ "${S11LAB:-0}" = "1" ] \
    && [ "${S11LB:-0}" -ge 1 ] && [ "${S11RUNG:-0}" = "1" ] && [ "${S11NM:-0}" -ge 2 ] \
    && [ "${S11S14:-0}" = "1" ] && [ "$S11DEL" = "0" ] && [ "$S11ADD" -ge 1 ] && [ "${S11DELTA:-0}" = "1" ] \
    && [ "$S11SEATFILES" = "1" ] && [ "$S11SEATADD" = "250" ] && [ "$S11SEATDEL" = "0" ]; } || {
    stamp "  ^ A-row11 REFUSED.  ⚠ READ WHICH: a missing §15 heading means the row was subtracted; a heading present with a missing LADDER FIGURE or a NOT MEASURED count below two means a merge kept the block's skeleton and dropped what it banks, which renders as prose and records nothing; §14's sentence at 0 -- or a DELETION column above 0 -- means the append REWROTE what stands above it, the one thing this record's masthead forbids and the thing no line count or ancestry check can see; §14's sentence above 1 is the mirror defect, a duplicated section; a SEAT-OWN delta that is not one file at +250/-0 means the branch is not the one this row was measured and ruled to be."
    fail_gate A-row11
  }
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 12 :: claude/coord-glossary-kickoff -- ⚠ **A MODIFIED DOCUMENT, SO THE ARM IS A PAIR.**
...
if [ "$SEAT12_SHA" != "PENDING" ]; then
  S12P='docs/Glossary.md'
  S12OK=1; [ -f "$S12P" ] || { stamp "  ^ A-row12 REFUSED: $S12P is not in the assembled tree"; S12OK=0; }
  S12ANCH=$(grep -acF -- '<a id="kickoff"></a>**Kickoff.**' "$S12P" 2>/dev/null || true)
  S12SIX=$(grep -acF -- 'durable documents fall into six kinds' "$S12P" 2>/dev/null || true)
  S12FIVE=$(grep -acF -- 'durable documents fall into five kinds' "$S12P" 2>/dev/null || true)
  S12LADDER=$(grep -acF -- 'the kickoff is the rewritable restart' "$S12P" 2>/dev/null || true)
  S12DELTA=$(git diff --name-only "$BASE" HEAD -- "$S12P" | grep -ac . || true)
  S12NS=$(git diff --numstat "$BASE" HEAD -- "$S12P"); S12ADD=$(printf '%s' "$S12NS" | cut -f1)
  case "$S12ADD" in ''|*[!0-9]*) S12ADD=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  stamp "A-row12 Glossary kickoff kind, DEFINITION AND DECLARATION TOGETHER :: file present=$S12OK :: the Kickoff anchor x$S12ANCH (must be 1) :: 'six kinds' x$S12SIX (must be 1) :: the SUPERSEDED 'five kinds' x$S12FIVE (must be 0 -- a six-kind definition under a five-kind sentence is the half-merge this arm exists to catch) :: the one-line ladder naming the kickoff x$S12LADDER (must be 1) :: in this train's delta=$S12DELTA (must be 1) :: additions=$S12ADD (must be > 0; this is a MODIFICATION, never an addition, so no diff-filter=A arm applies)"
  { [ "$S12OK" = "1" ] && [ "${S12ANCH:-0}" = "1" ] && [ "${S12SIX:-0}" = "1" ] && [ "${S12FIVE:-1}" = "0" ] \
    && [ "${S12LADDER:-0}" = "1" ] && [ "${S12DELTA:-0}" = "1" ] && [ "$S12ADD" -ge 1 ]; } || fail_gate A-row12
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 13 :: claude/g-census-2026-09-13 -- one ADDED record, and the arm reads that it IS a record.
...
if [ "$SEAT13_SHA" != "PENDING" ]; then
  S13P='docs/phase4/CENSUS-g-laptop-2026-09-13.md'
  S13OK=1; [ -f "$S13P" ] || { stamp "  ^ A-row13 REFUSED: $S13P is not in the assembled tree"; S13OK=0; }
  S13N=$(grep -ac . "$S13P" 2>/dev/null || true)
  S13NEW=$(git diff --name-only --diff-filter=A "$BASE" HEAD -- "$S13P" | grep -ac . || true)
  S13REC=$(grep -acF -- 'Record, not a runbook' "$S13P" 2>/dev/null || true)
  S13AT=$(grep -acE 'Measured at .*origin/master' "$S13P" 2>/dev/null || true)
  S13ONLY=$(git diff --name-only "$BASE" HEAD -- docs/phase4/CENSUS-g-laptop-2026-09-13.md | grep -ac . || true)
  stamp "A-row13 G-LAPTOP preservation census :: file present=$S13OK :: non-empty lines=$S13N (must be >= 1) :: ADDED by this train=$S13NEW (must be 1 -- a record already at the base would mean this row landed nothing) :: it declares itself a RECORD x$S13REC (must be 1) :: it states the ref it was MEASURED AT x$S13AT (must be >= 1 -- counts nobody can re-derive are not a census) :: in this train's delta=$S13ONLY (must be 1)"
  { [ "$S13OK" = "1" ] && [ "${S13N:-0}" -ge 1 ] && [ "${S13NEW:-0}" = "1" ] && [ "${S13REC:-0}" = "1" ] \
    && [ "${S13AT:-0}" -ge 1 ] && [ "${S13ONLY:-0}" = "1" ]; } || fail_gate A-row13
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 14 :: claude/c1-crashwhiletracing-marking -- ⚠ **ONE JSON LINE, AND THE ARM IS A PAIR.**
...
if [ "$SEAT14_SHA" != "PENDING" ]; then
  S14P='src/core/runtime/go2cs_test_disclosures.json'
  S14OK=1; [ -f "$S14P" ] || { stamp "  ^ A-row14 REFUSED: $S14P is not in the assembled tree"; S14OK=0; }
  S14AM=$(grep -acF -- 'AMENDMENT 2026-09-13 (C1) -- THE 1-6/7-8 MARKING' "$S14P" 2>/dev/null || true)
  S14MARK=$(grep -acF -- '1-6 READ; 7-8 READ AND MECHANISM-CONFIRMED AT A SIBLING DOOR' "$S14P" 2>/dev/null || true)
  S14DOOR=$(grep -acF -- 'TestLockOSThreadNesting' "$S14P" 2>/dev/null || true)
  S14PAIR=$(grep -aF -- 'host-fatal CRASH member' "$S14P" 2>/dev/null | grep -acF -- 'AMENDMENT 2026-09-13 (C1)' || true)
  S14OLD=$(grep -acF -- 'terminal verdict."' "$S14P" 2>/dev/null || true)
  S14RET=$(grep -acF -- 'RETIRES when the test re-admitted produces a terminal verdict' "$S14P" 2>/dev/null || true)
  S14N=$(grep -acF -- '"name":' "$S14P" 2>/dev/null || true)
  S14HF=$(grep -acF -- '"class": "host-fatal"' "$S14P" 2>/dev/null || true)
  ...
  S14DELTA=$(git diff --name-only "$BASE" HEAD -- src/core/runtime | grep -ac . || true)
  stamp "A-row14 TestCrashWhileTracing 1-6/7-8 marking, THE AMENDMENT AND ITS HOST ENTRY TOGETHER :: file present=$S14OK :: the dated amendment marker x$S14AM (must be 1) :: the marking's CONCLUSION sentence x$S14MARK (must be 1 -- the ruling was for a verdict, not a note) :: the sibling door TestLockOSThreadNesting named here x$S14DOOR (must be 1; both bases read 0) :: SAME-LINE with that entry's own reason opener x$S14PAIR (must be 1 -- this is what says the amendment landed in THIS entry and not another of the six) :: the SUPERSEDED reason ENDING x$S14OLD (must be 0 -- INVERTED: both bases read 1, so a stale copy of this file cannot satisfy the arm) :: the RETIRES trigger still x$S14RET (must be 1 -- the commit states nothing above the amendment moves) :: manifest SHAPE entries=$S14N (must be 6) host-fatal=$S14HF (must be 4) :: numstat +${S14ADD}/-${S14DEL} (must be 1/1: the reason is ONE JSON line and this is a REPLACEMENT, so a +2 or a -0 is a different edit)"
  { [ "$S14OK" = "1" ] && [ "${S14AM:-0}" = "1" ] && [ "${S14MARK:-0}" = "1" ] && [ "${S14DOOR:-0}" = "1" ] \
    && [ "${S14PAIR:-0}" = "1" ] && [ "${S14OLD:-1}" = "0" ] && [ "${S14RET:-0}" = "1" ] \
    && [ "${S14N:-0}" = "6" ] && [ "${S14HF:-0}" = "4" ] && [ "$S14ADD" = "1" ] && [ "$S14DEL" = "1" ]; } || {
    stamp "  ^ A-row14 REFUSED.  ⚠ READ WHICH: a missing amendment marker or conclusion means the row was subtracted; a PAIRING of 0 with the marker at 1 means the amendment is in the file but NOT in TestCrashWhileTracing's reason, which is a manifest that reads amended and is not; a SUPERSEDED ENDING of 1 means the entry's OLD reason line survived beside the new one (the JSON would then carry the entry twice, or the merge kept the base side); a RETIRES of 0, an entry count off 6 or a host-fatal count off 4 means the edit moved something the commit states it does not touch."
    fail_gate A-row14
  }
  [ "${S14DELTA:-0}" = "1" ] || stamp "      ⚠ A-row14 src/core/runtime footprint is $S14DELTA file(s) where THIS row predicts 1.  ⚠ NOT A REFUSAL AND NOT A DEFECT ON ITS OWN: row 16 also lands under src/core/runtime, so the union legitimately reads 3 when both rows board and 1 when only this one does.  It is stamped so the number is reconciled against the two rows' diffs rather than assumed."
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 15 :: claude/c1-getcallerpc-erratum -- ⚠ **A PURE APPEND, SO THE 'SUPERSEDED = 0' HALF IS INVERTED.**
...
if [ "$SEAT15_SHA" != "PENDING" ]; then
  S15P='docs/phase4/DESIGN-getcallerpc.md'
  S15OK=1; [ -f "$S15P" ] || { stamp "  ^ A-row15 REFUSED: $S15P is not in the assembled tree"; S15OK=0; }
  S15SEC=$(grep -acF -- '## 8. ERRATUM' "$S15P" 2>/dev/null || true)
  S15D1=$(grep -acF -- '### 8.1 Defect (i)' "$S15P" 2>/dev/null || true)
  S15D2=$(grep -acF -- '### 8.2 Defect (ii)' "$S15P" 2>/dev/null || true)
  S15D3=$(grep -acF -- '### 8.3 Defect (iii)' "$S15P" 2>/dev/null || true)
  ...
  S15SITE=$(grep -acF -- '109 is token occurrences and 107 is call sites' "$S15P" 2>/dev/null || true)
  S15FALS=$(grep -acF -- "P2's falsifier should read: a reach census at the hop reading at most 47, 46 and 50 per flavour" "$S15P" 2>/dev/null || true)
  # THE INVERSE HALF: the superseded-IN-MEANING P2 text must SURVIVE, exactly once, un-rewritten.
  S15ORIG=$(grep -acF -- 'the 1.23.12 figure of 4' "$S15P" 2>/dev/null || true)
  S15NS=$(git diff --numstat "$BASE" HEAD -- "$S15P"); S15ADD=$(printf '%s' "$S15NS" | cut -f1); S15DEL=$(printf '%s' "$S15NS" | cut -f2)
  case "$S15ADD" in ''|*[!0-9]*) S15ADD=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  case "$S15DEL" in ''|*[!0-9]*) S15DEL=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  S15DELTA=$(git diff --name-only "$BASE" HEAD -- "$S15P" | grep -ac . || true)
  stamp "A-row15 getcallerpc erratum, THE BLOCK AND THE TEXT IT CORRECTS TOGETHER :: file present=$S15OK :: the section-8 erratum header x$S15SEC (must be 1) :: the three defect subsections x$S15D1/x$S15D2/x$S15D3 (each must be 1) :: defect (ii)'s corrected reading -- 109 token occurrences vs 107 call sites -- x$S15SITE (must be 1) :: defect (iii)'s REPLACEMENT falsifier, the one that makes P2 able to be wrong, x$S15FALS (must be 1) :: the SUPERSEDED-IN-MEANING P2 text still present x$S15ORIG (must be 1 -- ⚠ INVERTED ON PURPOSE: this row is a PURE APPEND and the masthead forbids rewriting, so the correct assertion is that the corrected text SURVIVED, not that it vanished) :: numstat +${S15ADD}/-${S15DEL} (additions > 0 and deletions must be 0 -- a deletion here IS the rewrite the record forbids) :: in this train's delta=$S15DELTA (must be 1)"
  { [ "$S15OK" = "1" ] && [ "${S15SEC:-0}" = "1" ] && [ "${S15D1:-0}" = "1" ] && [ "${S15D2:-0}" = "1" ] \
    && [ "${S15D3:-0}" = "1" ] && [ "${S15SITE:-0}" = "1" ] && [ "${S15FALS:-0}" = "1" ] \
    && [ "${S15ORIG:-0}" = "1" ] && [ "$S15ADD" -ge 1 ] && [ "$S15DEL" = "0" ] && [ "${S15DELTA:-0}" = "1" ]; } || {
    stamp "  ^ A-row15 REFUSED.  ⚠ READ WHICH: a missing header or defect subsection means the block was subtracted; a present heading with a missing corrected READING (S15SITE / S15FALS) means a merge kept the block's skeleton and dropped what it says, which renders as prose and records nothing; S15ORIG at 0 or a DELETION column above 0 means the erratum REWROTE the text above it instead of appending below it -- the one thing this record's masthead forbids, and the thing no ancestry or line-count check can see."
    fail_gate A-row15
  }
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi

# --- row 16 :: claude/c1-lockosthread-body -- ⚠ **FOUR REWRITTEN BODIES, A REVERSED HEADER BULLET,
...
if [ "$SEAT16_SHA" != "PENDING" ]; then
  S16M='src/core/runtime/managed_impl.cs'; S16S='src/core/runtime/stubs_impl.cs'; S16D='docs/phase4/DESIGN-managed-getg.md'
  S16OK=1
  for f in "$S16M" "$S16S" "$S16D"; do
  ...
  S16EXTI=$(grep -acF -- 'gp.Value.m.Value.lockedExt++' "$S16M" 2>/dev/null || true)
  S16EXTD=$(grep -acF -- 'gp.Value.m.Value.lockedExt--' "$S16M" 2>/dev/null || true)
  S16OVF=$(grep -acF -- 'throw panic("LockOSThread nesting overflow")' "$S16M" 2>/dev/null || true)
  S16INTI=$(grep -acF -- 'getg().Value.m.Value.lockedInt++' "$S16M" 2>/dev/null || true)
  S16INTD=$(grep -acF -- 'gp.Value.m.Value.lockedInt--' "$S16M" 2>/dev/null || true)
  ...
  S16DOL=$(grep -acF -- 'dolockOSThread();' "$S16M" 2>/dev/null || true)
  S16DOU=$(grep -acF -- 'dounlockOSThread();' "$S16M" 2>/dev/null || true)
  S16BAD=$(grep -acF -- 'systemstack(badunlockosthread)' "$S16M" 2>/dev/null || true)
  ...
  for d in 'public static void LockOSThread()' 'public static void UnlockOSThread()' 'internal static void lockOSThread()' 'internal static void unlockOSThread()'; do
    [ "$(grep -acF -- "$d" "$S16M" 2>/dev/null || true)" = "1" ] && S16DECL=$(( S16DECL + 1 ))
  done
  # (iv) THE REVERSED BULLET -- INVERTED, and the only predicate here that a kept-base-side hunk fails.
  S16OLDB=$(grep -acF -- 'are no-ops BY CONSTRUCTION, not by omission' "$S16M" 2>/dev/null || true)
  S16NEWB=$(grep -acF -- 'carry GO' "$S16M" 2>/dev/null || true)
  ...
  S16ERR=$(grep -acF -- 'that sentence was FALSE when written' "$S16S" 2>/dev/null || true)
  S16SEC=$(grep -acF -- '## 13. AMENDMENT 2026-09-13 (C1)' "$S16D" 2>/dev/null || true)
  ...
  S16FILES=$(git diff --name-only "$BASE" HEAD -- "$S16M" "$S16S" "$S16D" | sort | tr '\n' ' ')
  S16WANT="$S16D $S16M $S16S "
  S16FOOT=0; [ "$S16FILES" = "$S16WANT" ] && S16FOOT=1
  ...
  S16SEATFILES=$(git diff --name-only "$(git merge-base "$BASE" "$SEAT16_SHA")" "$SEAT16_SHA" | sort | tr '\n' ' ')
  S16SEATFOOT=0; [ "$S16SEATFILES" = "$S16WANT" ] && S16SEATFOOT=1
  S16NS=$(git diff --numstat "$BASE" HEAD -- "$S16M"); S16MADD=$(printf '%s' "$S16NS" | cut -f1); S16MDEL=$(printf '%s' "$S16NS" | cut -f2)
  case "$S16MADD" in ''|*[!0-9]*) S16MADD=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  case "$S16MDEL" in ''|*[!0-9]*) S16MDEL=0 ;; esac   # the RAW value: ${..:-} substitutes before the '' arm can be reached
  stamp "A-row16 LockOSThread carries Go's body, BODIES AND BULLET AND FOOTPRINT AS ONE :: the three files present (failures=$S16OK, 1 means clean) :: lockedExt++ x$S16EXTI (must be 1) lockedExt-- x$S16EXTD (must be 2 -- the overflow ROLLBACK and UnlockOSThread's decrement; 1 is one of the two bodies subtracted) nesting-overflow panic x$S16OVF (must be 1) lockedInt++ x$S16INTI (must be 1) lockedInt-- x$S16INTD (must be 1) :: the CHAIN to the auto-converted bodies -- dolockOSThread(); x$S16DOL (must be 2) dounlockOSThread(); x$S16DOU (must be 2) systemstack(badunlockosthread) x$S16BAD (must be 1) :: the four declarations each present exactly once=$S16DECL (must be 4) :: the SUPERSEDED header bullet x$S16OLDB (must be 0 -- ⚠ INVERTED: both bases read 1, and a tree carrying the new bodies UNDER the old bullet compiles, runs and documents itself as a no-op it no longer is) and the replacement bullet x$S16NEWB (must be >= 1) :: stubs_impl.cs's 'honest by persistence' erratum x$S16ERR (must be 1 -- the false sentence is repaired where it was written, not only where the code changed) :: DESIGN-managed-getg section 13 x$S16SEC (must be 1) :: managed_impl numstat +${S16MADD}/-${S16MDEL} (the -4 IS the bullet; a -0 means the bullet was never replaced) :: UNION-SIDE footprint, all three paths moved=$S16FOOT (must be 1; saw [$S16FILES] want [$S16WANT] -- ⚠ this read is PATHSPEC-RESTRICTED to the three and detects SUBTRACTION ONLY) :: SEAT-OWN footprint, the EXACT set from the seat's tip and the only read that sees a FOURTH file=$S16SEATFOOT (must be 1; saw [$S16SEATFILES] want [$S16WANT])"
  { [ "$S16OK" = "1" ] && [ "${S16EXTI:-0}" = "1" ] && [ "${S16EXTD:-0}" = "2" ] && [ "${S16OVF:-0}" = "1" ] \
    && [ "${S16INTI:-0}" = "1" ] && [ "${S16INTD:-0}" = "1" ] && [ "${S16DOL:-0}" = "2" ] && [ "${S16DOU:-0}" = "2" ] \
    && [ "${S16BAD:-0}" = "1" ] && [ "$S16DECL" = "4" ] && [ "${S16OLDB:-1}" = "0" ] && [ "${S16NEWB:-0}" -ge 1 ] \
    && [ "${S16ERR:-0}" = "1" ] && [ "${S16SEC:-0}" = "1" ] && [ "$S16MDEL" -ge 1 ] && [ "$S16FOOT" = "1" ] \
    && [ "$S16SEATFOOT" = "1" ]; } || {
    stamp "  ^ A-row16 REFUSED.  ⚠ READ WHICH: a body count short of its exact figure means one of the four was merged back to its empty form -- the tree then compiles and LockOSCounts still answers 0,0, which is the reading this row exists to move; the SUPERSEDED BULLET at 1 means the header hunk was resolved to the base side while the bodies landed, so the file now documents as a no-op code that keeps counters (or, with the bodies missing too, the whole row was subtracted); a missing stubs_impl erratum or section 13 means a file of the three was dropped; a UNION-SIDE FOOTPRINT of 0 means one of the three paths did not move at the union; a SEAT-OWN FOOTPRINT of 0 means the BRANCH is not the three-path change it was measured and ruled to be -- a fourth file was added to it after the ruling, and that read is the only one in this arm that can say so.  Read the saw/want sets above before touching anything else."
    fail_gate A-row16
  }
  SEATASSERT_N=$(( SEATASSERT_N + 1 ))
fi
...
if [ "${NEWBEH:-0}" = "0" ]; then
  ...
  #   would refuse every healthy launch of THIS one, whose sixteen rows add no behavioral project at all
  #   (row 9 RE-EMITS an existing guard -- seven MODIFIED files, zero additions -- which is why its class
...
#     tree is the stale-figure class.  MEASURED 2026-09-13 at a02ac3df3 over all SIXTEEN seat tips: NO
...
stamp "ROW 8 CLOSING NOTE :: ${SEAT8_REF} @${SEAT8_SHA} re-mints CORPUS METADATA that MASTER also moved.  A-row8 read BOTH halves at the union BY CONTENT -- master's licensing hunks and this row's own -- because both diffs are pure insertions and removals in nearby regions of the same ten files: git reports a clean merge whichever one it loses, the tree still compiles, and no ancestry check can see the subtraction.  That is the syscall.Uname class, and CONTENT is the only instrument that answers it."
stamp "ROW 10 CLOSING NOTE :: ${SEAT10_REF} @${SEAT10_SHA} was cut from an OLDER base than this train's (measured at derive time: its merge-base with origin/master is not the base, and its two-dot file count reads in the thousands while its THREE-DOT count reads twelve).  Every read of this row -- its class shape, its forbidden set, LEG D's per-seat prediction and A-row10 -- uses the THREE-DOT form for exactly that reason: a two-dot query against a base a seat forked before returns MASTER's own newer content, REVERSED, and reads as a seat that changed the world."
...
stamp "=== ${LABEL} ASSEMBLE DONE head=$(git rev-parse --short HEAD) base=$BASE route=$ROUTE seats=$SEATS_DONE (of $SEATS_LISTED listed; skippedPending=$SEATS_SKIPPED) requireAll=$REQUIRE_ALL seatShas=[$SEAT1_SHA $SEAT2_SHA $SEAT3_SHA $SEAT4_SHA $SEAT5_SHA $SEAT6_SHA $SEAT7_SHA $SEAT8_SHA $SEAT9_SHA $SEAT10_SHA $SEAT11_SHA $SEAT12_SHA $SEAT13_SHA $SEAT14_SHA $SEAT15_SHA $SEAT16_SHA] owed=[golib=$OWED_GOLIB gen=$OWED_GEN converter=$OWED_CONV behavioral=$OWED_BEH corpus=$OWED_CORPUS golibtests=$OWED_GT docs=$OWED_DOCS claudemd=$OWED_CLAUDEMD manifest=$OWED_MANIFEST] seatAssertArms=$SEATASSERT_N legD=$LEGD_VERDICTS legKrows=[$LEGK_ROWS] newBehavioralProjects=$NEWBEH newBehavioralProjectsAllDepths=$NEWBEH_ALL overallFailed=$FAILED ==="
