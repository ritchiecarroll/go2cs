#!/bin/bash
# ============================================================================================
# TRAIN 47 LAND.  DERIVED from coord-train47-land.sh: same skeleton -- read the assembly's own stdout
...
BASE_DECL=$(grep -aE "^EXPECT_46='" "$LAND_SELFDIR/coord-train47-assemble.sh" 2>/dev/null | head -1 | sed -E "s/^EXPECT_46='([^']*)'.*/\1/")
BASE_CONTAINS=$(grep -aE "^EXPECT_CONTAINS='" "$LAND_SELFDIR/coord-train47-assemble.sh" 2>/dev/null | head -1 | sed -E "s/^EXPECT_CONTAINS='([^']*)'.*/\1/")
[ -n "$BASE_DECL" ] && [ "$BASE_DECL" != "PENDING" ] || { echo "LAND BASE UNREADABLE/PENDING :: the assembly script declares EXPECT_46=[${BASE_DECL:-absent}].  A landing cannot be gated against a placeholder base -- ABORT"; exit 2; }
...
ASM="${LAND_ASM:-$SELFDIR/coord-train47-assemble-run1.stdout}"
case "$ASM" in /*|[A-Za-z]:[\\/]*) ;; *) ASM="$(cd "$(dirname "$ASM")" 2>/dev/null && pwd)/$(basename "$ASM")" ;; esac
LOG="$SELFDIR/coord-train47-land.log"
...
DONE=$(grep -a '=== train47 ASSEMBLE DONE' "$ASM" | tail -1 | tr -d '\r')
...
#     EXPECT_46=[$BASE_DECL]; when that is a REVSPEC rather than a SHA, the SHA the battery ran on is
...
#     `TRAIN47_REQUIRE_ALL=1` is what makes a PENDING row ABORT instead, so a landing record must
...
[ "${REQALL:-x}" = "1" ] || { stamp "  ^ ABORT: the record reads requireAll=${REQALL:-ABSENT}.  A landing run sets TRAIN47_REQUIRE_ALL=1, which is what makes a PENDING row REFUSE rather than be skipped -- without it this record describes a PARTIAL train, however internally consistent its numbers are."; exit 4; }
...
# ⚠ THE FLOOR IS **FIFTEEN**, AND IT IS A FACT ABOUT THIS TRAIN'S TABLE RATHER THAN A GUESS: the
#   table was RE-FILLED to SIXTEEN rows on 2026-09-13 and then RE-NUMBERED TO FIFTEEN the same day,
#   when the coordinator UNSEATED the former row 8 (seat 6, claude/g-unfreeze-handown-recut) after run
#   1 went RED at LEG 2 in packages only that row touches.  ALL FIFTEEN remaining rows carry a resolved
#   SHA and NONE reads PENDING, so with requireAll=1 (asserted above) a landing record must carry all
#   fifteen; the floor below is the weaker, always-true statement that catches a record from a partial
#   run whose requireAll field was itself mis-read.  The row-count equality on the next line is the
#   strong form, and it reads whatever the assembly listed.
#   ⚠ THE FLOOR MOVES WITH THE TABLE -- IN BOTH DIRECTIONS.  It was NINE while the table held eleven
#   rows and ELEVEN while five rows still read PENDING; leaving it below the filled count admits a
#   record short of the announced set, and leaving it ABOVE the filled count refuses every healthy
#   landing of this train.  Fifteen is the filled count as this file stands.
[ "${SEATSN:-0}" -ge 15 ] || { stamp "  ^ ABORT: the record reads seats=$SEATSN where this train's FIFTEEN filled rows are unconditional."; exit 4; }
...
#   ⚠ EVERY ANCHOR BELOW WAS READ OUT OF coord-train47-assemble.sh's OWN STAMP STRINGS.  None is
...
# --- LEG U, row 1's own seat gate.  ⚠ ITS THREE ARMS ARE REQUIRED SEPARATELY, because the pair is the
...
# — the former row 8's `A-row8 hand-own metadata unfreeze` anchor stood here and was DELETED with its
#   seat (coordinator, 2026-09-13: claude/g-unfreeze-handown-recut UNSEATED after run 1 went red at
#   LEG 2).  An anchor whose ARM no longer exists can never be satisfied, and a landing that refuses a
#   healthy battery is how an operator learns to reach for the override.  What stands in its place is
#   the SURVIVING row 8's own both-halves assertion -- the same silent-subtraction class (a converter
#   hunk and its RE-BASELINED golden, which git merges independently and can subtract one of).
req 'A-row8 generic cross-package alias qualifier, BOTH HALVES AT THE UNION ::' 1 "row 8's SILENT-SUBTRACTION assertion RAN.  ⚠ The converter rule and the re-baselined behavioral golden are TWO HALVES that git merges independently: a union that kept the emission and dropped the golden (or the reverse) compiles, runs, and re-baselines nothing, and no ancestry check can see it.  The arm compares the BLOBS at the union"
...
stamp "delta by directory :: src/go2cs=$D_CONV src/go2cs.slnx=$D_SLNX src/core/golib=$D_GOLIB src/gen=$D_GEN src/tests/GolibTests=$D_GT src/tests/Behavioral=$D_BEH src/core=$D_CORE docs=$D_DOCS :: behavioral files MODIFIED or DELETED other than the four MSTest classes=$D_BEHMOD"
...
if [ "${LAND_VERIFY_ONLY:-0}" = 1 ]; then stamp "=== TRAIN 47 LAND VERIFY-ONLY DONE head=$HEAD (nothing pushed) ==="; exit 0; fi
...
stamp "=== TRAIN 47 LAND DONE master=$NM seats=$SEATN ==="
