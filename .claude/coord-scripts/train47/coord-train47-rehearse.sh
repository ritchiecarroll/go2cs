...
#           NOVET=1 bash coord-train47-rehearse.sh         # skip the per-seat build+vet, STATED in the summary
#           REHEARSE_WT=<dir> bash coord-train47-rehearse.sh   # the throwaway worktree (default: under .claude)
#           bash coord-train47-rehearse.sh --union-out <path>  # write the union's own facts to a file
...
WT="${REHEARSE_WT:-$G/.claude/worktrees/coord-t47-rehearse}"
TAG=t47rehearse
...
ASM="$SELFDIR/coord-train47-assemble.sh"
RESROOT="$SELFDIR/coord-train47-resolutions"
...
#   one of them.  The assembly refuses three things about TRAIN47_SKIP_PENDING before it merges
#   anything; the rehearsal read `${TRAIN47_SKIP_PENDING:-0}` at the PENDING row and nothing else --
...
#     (2) a DECLARED-BUT-EMPTY `TRAIN47_SKIP_PENDING=`.  `${X:-0}` reads an empty override as unset,
#         so a blank authorisation silently becomes "no" -- the same class the assembly refuses.
#     (3) `TRAIN47_REQUIRE_ALL=1` AND `TRAIN47_SKIP_PENDING=1` TOGETHER.  The assembly calls that a
...
if [ "${SKIP_PENDING+x}" = "x" ]; then
  say "REFUSED: SKIP_PENDING is in the ENVIRONMENT reading [${SKIP_PENDING:-<empty>}].  That is NOT this script's switch -- the switch is TRAIN47_SKIP_PENDING -- so an export of it configures NOTHING here AND ABORTS THE ASSEMBLY, which would make this rehearsal's verdict a statement about a run that cannot happen.  Re-launch with TRAIN47_SKIP_PENDING=<0|1>."
  exit 2
fi
if [ "${REQUIRE_ALL+x}" = "x" ]; then
  say "REFUSED: REQUIRE_ALL is in the ENVIRONMENT reading [${REQUIRE_ALL:-<empty>}].  That is NOT the switch -- the switch is TRAIN47_REQUIRE_ALL -- and the assembly ABORTS on this environment, so a rehearsal run under it would describe a train nobody can assemble."
  exit 2
fi
if [ "${TRAIN47_SKIP_PENDING+x}" = "x" ]; then
  if [ -z "${TRAIN47_SKIP_PENDING:-}" ]; then
    say "REFUSED: TRAIN47_SKIP_PENDING is DECLARED but EMPTY.  An empty authorisation is not 'unset' and must not be read as either answer -- name it or do not declare it."
    exit 2
  fi
  case "$TRAIN47_SKIP_PENDING" in
    1) say "TRAIN47_SKIP_PENDING=1 :: a PENDING row is SKIPPED WITH A STAMP and THIS REHEARSAL IS OF A **PARTIAL UNION** -- the skipped row's collisions with every other row are UNMEASURED, which is not the same as clean." ;;
    0) say "TRAIN47_SKIP_PENDING=0 :: stated explicitly; a PENDING row REFUSES by name." ;;
    *) say "REFUSED: TRAIN47_SKIP_PENDING reads [$TRAIN47_SKIP_PENDING], which is neither 0 nor 1 -- an authorisation this script cannot read is not an authorisation."; exit 2 ;;
  esac
else
  say "TRAIN47_SKIP_PENDING is not in the environment :: a PENDING row REFUSES by name (the same default the assembly takes)."
fi
if [ "${TRAIN47_REQUIRE_ALL+x}" = "x" ] && [ -z "${TRAIN47_REQUIRE_ALL:-}" ]; then
  say "REFUSED: TRAIN47_REQUIRE_ALL is DECLARED but EMPTY.  The assembly refuses this environment before it merges anything, so the rehearsal refuses it too -- one switch, both instruments."
  exit 2
fi
if [ "${TRAIN47_REQUIRE_ALL:-0}" = "1" ] && [ "${TRAIN47_SKIP_PENDING:-0}" = "1" ]; then
  say "REFUSED: TRAIN47_REQUIRE_ALL=1 AND TRAIN47_SKIP_PENDING=1 are a CONTRADICTION: one demands every announced row and the other permits a gap.  The assembly refuses this pair rather than ordering them by precedence, and a rehearsal that accepted it would be rehearsing an environment the assembly will not run -- name ONE of them."
  exit 2
fi
...
if [ "${VERIFY:-0}" = "1" ]; then
  say "=== TRAIN 47 RESOLUTION VERIFY :: $RESROOT ==="
...
case "$BASE_EXPECT" in
  origin/master)
    ...
    CPIN=$(grep -aE "^EXPECT_CONTAINS='" "$ASM" 2>/dev/null | head -1 | sed -E "s/^EXPECT_CONTAINS='([^']*)'.*/\1/")
    if [ -z "${CPIN:-}" ]; then
      say "⚠ NOTE: the assembly declares base=origin/master but no EXPECT_CONTAINS pin could be read out of it, so this rehearsal's base is UNCHECKED against anything."
...
while IFS='|' read -r n b s c m a <&3; do
  ...
  if [ "$s" = "PENDING" ]; then
    ...
    if [ "${TRAIN47_SKIP_PENDING:-0}" != "1" ]; then
      say "REFUSED: seat $n ($c) $b reads PENDING and TRAIN47_SKIP_PENDING is not 1.  Rehearsing a subset of a FILLED table produces a CLEAN verdict about a union nobody announced -- opt into the partial rehearsal BY NAME, or fill the row."
      exit 2
    fi
    SKIPPED=$(( SKIPPED + 1 )); SKIPNAMES="$SKIPNAMES $n($b)"
    say "seat $n ($c) $b :: **SKIPPED -- PENDING** (TRAIN47_SKIP_PENDING=1).  ⚠ This rehearsal is of a PARTIAL union: every verdict below describes the rows that merged, and the skipped row's collisions with all of them are UNMEASURED."
    continue
  fi
