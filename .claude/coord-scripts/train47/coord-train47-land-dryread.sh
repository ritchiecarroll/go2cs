#!/bin/bash
# ============================================================================================
# TRAIN 47 LAND -- DRY READ.  It runs NOTHING that lands and touches NO tree: it checks that the land
...
LAND="$SELFDIR/coord-train47-land.sh"
...
say "⚠ TRAIN 47's LAND READS **ONE** RECORD.  Train 45's read two -- the assembly's and a LEG D"
say "  STANDALONE re-measurement -- because its wired LEG D refused at a seeding control and measured"
say "  nothing.  That leg is CORRECTED in the train-47 assembly, so there is no second record and no"
...
#   form used `TokenValueTagRefusalTests`, which the train-47 assembly legitimately MENTIONS in a dated
...
#   the control WORKING rather than a control being weakened.  Train 47 seats `claude/c2-census-reader`,
#   whose deliverable IS `src/core/golib/Q44RegistryCensus.cs`, so the assembly's own row-9 content
...
for bad in 'FinalizerBindingTests' 'coord-doctrine-batch18' 'AliasNamespaceShadow' 'SliceTypeParamNil' 'RefLoweredDeferChain' 'FatalReportTests' 'TokenLatticeShadowTests' 'WindowsNewCallback' 'laneR-golib-sema'; do
...
CTL=$(mktemp /tmp/t47-dryread-ctl.XXXXXX)
...
HEALTHY=$(mktemp /tmp/t47-dryread-ok.XXXXXX)
