...
if [ "$LEGD_OK" = "1" ]; then
  for goos in $LEGD_TARGETS; do
    ...
    EXTRA=$(comm -13 <(sort -u "$LEGD_EXPSET") "$GOTSET" | grep -ac . || true)
    MISSING=$(comm -23 <(sort -u "$LEGD_EXPSET") "$GOTSET" | grep -ac . || true)
    if [ "${EXTRA:-0}" != "0" ]; then
      MET=0; WHY="$WHY [$EXTRA file(s) differ that the prediction does not name]"
      legd_stamp "  ^ LEG D $goos: files OUTSIDE the prediction:"; comm -13 <(sort -u "$LEGD_EXPSET") "$GOTSET" | head -10 | sed 's/^/    /' | tee -a "$ASMLOG"
    fi
    if [ "${MISSING:-0}" != "0" ]; then
      MET=0; WHY="$WHY [$MISSING predicted file(s) did NOT differ]"
      legd_stamp "  ^ LEG D $goos: predicted files that did NOT move.  ⚠ On a PER-GOOS package this can be legitimate -- a file that exists only under another flavour cannot move here -- so each is NAMED rather than counted:"; comm -23 <(sort -u "$LEGD_EXPSET") "$GOTSET" | head -10 | sed 's/^/    /' | tee -a "$ASMLOG"
    fi
